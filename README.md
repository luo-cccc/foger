...
- **生产工具去污染**：清掉所有从真实付费测试书渗入生产源码的剧情细节（审计降级分支、hook 词表、角色豁免表、提示词示例、Studio 占位符全部改为题材中立）；建书提示词新增**世界命名硬约束**——世界名必须从标题与设定派生、禁止"XX界/XX大陆/XX域"占位命名、brief 指定则沿用、确立后全剧不得改名。
- **模板感收敛**：writer 新增节拍变奏指令（慢热/过渡/后效集允许反转替换、情绪问题低音量、爽点类型轮换），标题句式结构轮换检测（连续 3 集同句式壳自动换名）。
- **题材去模板化**：15 题材疲劳词全部唯一（通用 AI 词由 auditor 统一检查）、爽点池扩到 8-10 类、审计维度题材差异化，配 `genre-config.test.ts` 防复发。
- **污染守卫**：新增 `scripts/audit-contamination.mjs` 拒绝测试夹具/付费书专名进入生产源码，接入 `pnpm verify`；新增 `pnpm audit:contamination` 命令。
- 验证：core 148 文件 1448 测试全绿。
