...
- 无人值守写作现在按真实 Cron 时间执行，停止 daemon 会取消并等待当前章节安全退出；多书任务不会因并发上限长期饥饿，每日章节上限也能在并发写作时准确生效。
- Studio 在快速切换书籍时不会再显示或操作上一页的过期数据；重复点击建书会被前后端共同拦截，避免同名书重复生成。
- 基础设定重修会连同结构化 Canon 一起事务提交，生成或抽取失败时保留原设定，不再留下新旧设定混用的中间状态。
...
当前完成质量边界：本地章节生产、结构化状态、章节/工作流事务恢复和完整确定性测试已经达到稳定可用基线；隔离 Studio E2E、跨进程竞争和真实强杀恢复保持全绿。2026-07-16 的 DeepSeek 官方 20 章真实前后端联动已连续持久化 15 章，第 16 章因 resync 后伏笔 ID 漂移未提交；规范化修复已进入离线门禁，但完整 20/20 章仍待重跑。最新精确测试计数和真实模型边界只在[当前架构与开发优先级](docs/current-architecture-and-priorities.md#11-实际完成度与质量矩阵)维护，避免 README 随测试增删失真。
...
这样 `brief`、卷纲、书级规则、当前任务不再混成一坨 prompt，而是先编译，再写作。运行时还会记录 Prompt Assembly Trace：每个 source 的字符数、估算 token、上下文层级、是否压缩和内容指纹都可以进入诊断报告；上下文按 `verbatim`、`semantic`、`compressible` 三层治理。
...
InkOS 提供三种交互方式，Studio、CLI、Chat 和外部 agent 的受控写作路径共享 core command、锁、恢复和事务合同：
...
`write next` 现在默认走 `plan -> compose -> write` 的输入治理链路，审计后的自动修订轮数默认是 2。若你需要控制成本或回退到旧的 prompt 拼装路径，可在 `inkos.json` 中显式设置：
...
真实章节成本复盘示例：`inkos analytics --chapters 4-6 --llm-report --save-report --max-total-tokens 600000 --max-chapter-tokens 200000 --max-prompt-tokens 16000 --max-retry-rate 0.2 --max-audit-calls 2 --max-revision-calls 1 --max-normalize-calls 2 --max-settle-calls 1`。报告以持久化 operation telemetry 为准，同时显示章节索引 token 与 telemetry 的差额、每章治理调用数和自动审校终止原因；历史无 operationId 的恢复调用会单独标记，不能静默漏算。
...
- **P1（当前）**：重跑修复后的真实 20 章联动，要求 20/20 持久化、truth/快照一致、Doctor 全部关联且报告可靠收口；随后按业务域拆分 Studio server 与 PipelineRunner，并继续按需加载 Mermaid、Shiki 和 WASM 重依赖。
