...
InkOS 将原长篇小说生产链改造成统一的漫剧剧本工作流。系统默认规划 100 集，每集约 90 秒，并以结构化 `EpisodeScript JSON` 作为权威创作真源，同时生成便于阅读和交付的分镜 Markdown。
...
- 全项目切换为 Episode v2 漫剧生产语义，默认 100 集、单集 90 秒、每集 1～3 个场景和 6～12 个镜头。
- 剧本采用统一结构化合同，显式保存进入状态、目标、反对力量、因果升级、局部兑现、出去压力、信息权限和交接状态。
- 增加分镜执行性、时长、反转因果、情绪牵引、跨集交接和全剧完本门禁。
- 正常单集流程压缩为最多三次模型调用，并增加上下文快照、调用预算、缓存统计、证据型审查和性能报告。
- 旧小说项目不再继续写作；检测到旧格式时返回 `UNSUPPORTED_LEGACY_FORMAT`。
...
- 1～3 个场景，6～12 个镜头。
- 目标时长 90 秒；75～105 秒为软区间，60～120 秒为硬区间。
- 每个镜头必须有可制作的视觉信息。
...
`episodes/*.json` 是剧本权威真源。Markdown 是阅读和导出投影。`story/state/*.json` 保存结构化运行状态，`story/runtime/` 保存单次操作的计划、上下文、规则、审查、性能和交接数据。
