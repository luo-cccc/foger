...
当前完成质量边界：本地章节生产、结构化状态、章节/工作流事务恢复和完整确定性测试已经达到稳定可用基线；隔离 Studio E2E 完整套件为 10/10，preparing/committed 两个真实子进程强杀/重启 recovery 场景合并连续运行 5 轮共 10/10。`pnpm stress:process` 已通过 8 worker、400 次竞争 mutation 和 30 轮真实强杀恢复，发布门禁保持全绿。Studio、CLI、Chat 与 sub-agent 的已知写入旁路、revise mode 校验、恢复前置和跨进程配置写入均已收敛。最新精确测试计数和真实模型边界只在[当前架构与开发优先级](docs/current-architecture-and-priorities.md#11-实际完成度与质量矩阵)维护，避免 README 随测试增删失真。
...
- **P1（当前）**：在 report v2 和 Prompt Assembly Trace 上完成修复后的真实 3-5 章基线，建立跨 agent token/context 预算与稳定缓存；随后按业务域拆分 Studio server 与 PipelineRunner，并继续按需加载 Mermaid、Shiki 和 WASM 重依赖。
