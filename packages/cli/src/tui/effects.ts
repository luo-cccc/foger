...
export function intentToBadge(intent: string, locale: TuiLocale = resolveTuiLocale()): string {
  const labels = locale === "en"
    ? {
        write_next: " WRITE ",
        revise_episode: " REVISE ",
        rewrite_episode: " REWRITE ",
        update_focus: " FOCUS ",
    ...
    : {
        ...
        select_book: " 选择 ",
        rename_entity: " 改名 ",
        patch_episode_text: " 修补 ",
        edit_truth: " 真相 ",
      };
  const backgrounds: Record<string, string> = {
    write_next: bgMagenta,
    revise_episode: bgBlue,
    rewrite_episode: bgBlue,
    update_focus: bgCyan,
...
export function intentToTheme(intent: string): string {
  const map: Record<string, string> = {
    write_next: "writing",
    revise_episode: "revising",
    rewrite_episode: "revising",
    ...
    return [
      {
        title: "Writing",
        commands: [
          ["/write", "Write the next episode (full pipeline)"],
          ["/rewrite <n>", "Rewrite episode N from scratch"],
        ],
      },
      {
        title: "Navigation",
