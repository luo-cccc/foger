...
function summarizeEvent(event: InteractionEvent, copy: TuiCopy): string {
  ...
  if (event.bookId && event.episodeNumber !== undefined) {
    const episodeLabel = copy.locale === "zh-CN"
      ? `第 ${event.episodeNumber} 集`
      : `ch.${event.episodeNumber}`;
    return `${base} (${event.bookId} ${episodeLabel})`;
  }
  if (event.bookId) {
