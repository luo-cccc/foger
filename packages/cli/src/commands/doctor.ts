...
import { fetchWithProxy, KNOWN_MODEL_ROUTING_AGENTS, PHASE7_MODEL_ROUTING_AGENTS } from "@actalk/inkos-core";
import {
  ...
  .action(async (opts: { repairNodeRuntime?: boolean }) => {
    ...
    // 5. Check effective LLM config (Studio project base + env/CLI overlay, or legacy env)
    {
      ...
      try {
        ...
        const overrides = configResult.config.modelOverrides ?? {};
        const overrideCount = Object.keys(overrides).length;
        const phase7Covered = PHASE7_MODEL_ROUTING_AGENTS.filter((agent) => agent in overrides);
        checks.push({
          name: "Agent Model Routing",
          ok: true,
          detail: overrideCount > 0
            ? `${overrideCount}/${KNOWN_MODEL_ROUTING_AGENTS.length} override(s); Phase 7 agents covered: ${phase7Covered.length > 0 ? phase7Covered.join(", ") : "none"}`
            : `No overrides; ${KNOWN_MODEL_ROUTING_AGENTS.length} agents use effective default model ${configResult.llm.model}`,
        });
      } catch {
