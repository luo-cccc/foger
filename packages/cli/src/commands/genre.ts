...
name: ${params.name}
id: ${params.id}
episodeTypes: ["progression", "setup", "transition", "payoff"]
...
pacingRule: "A clear advance or payoff every 2-3 episodes"
...
genreCommand
  ...
  .action(async (id: string) => {
    try {
      ...
      log(`  Episode types:      ${profile.episodeTypes.join(", ")}`);
...
genreCommand
  ...
  .action(async (id: string, opts) => {
    try {
      ...
      log(`Edit the file to customize episode types, fatigue words, rules, etc.`);
    } catch (e) {
