...
import { ConsolidatorAgent, executeCoreMutation, StateManager } from "@actalk/inkos-core";
import { loadConfig, buildPipelineConfig, findProjectRoot, resolveBookId, log, logError } from "../utils.js";

export const consolidateCommand = new Command("consolidate")
  .description("Consolidate episode summaries into volume-level summaries (reduces context for long books)")
  ...
  .action(async (bookIdArg: string | undefined, opts) => {
    try {
      ...
      if (!opts.json) log(`Consolidating episode summaries for "${bookId}"...`);

      const result = await executeCoreMutation({
        state,
        pipeline: {
          consolidateBook: async (targetBookId) => await consolidator.consolidate(state.bookDir(targetBookId)),
        },
      }, { kind: "consolidate-book", bookId });
      ...
      if (opts.json) {
        ...
        if (result.archivedVolumes === 0) {
          ...
          log(`Retained ${result.retainedEpisodes} recent episode summaries.`);
