  ...
  .action(async (key: string, value: string) => {
    ...
    try {
      ...
      const KNOWN_KEYS = new Set([
        ...
        "daemon.maxConcurrentBooks", "daemon.episodesPerCycle",
        "daemon.retryDelayMs", "daemon.cooldownAfterEpisodeMs",
        "daemon.maxEpisodesPerDay",
      ...
      await mutateProjectConfig(root, (config) => {
        ...
        for (let i = 0; i < keys.length - 1; i++) {
          const k = keys[i]!;
          if (!(k in target) || typeof target[k] !== "object" || target[k] === null || Array.isArray(target[k])) {
            target[k] = {};
          }
...
configCommand
  ...
  .argument("<agent>", `Agent name (${KNOWN_MODEL_ROUTING_AGENTS.join(", ")})`)
  ...
  .action(async (agent: string, model: string, opts: { baseUrl?: string; provider?: string; apiKeyEnv?: string; stream?: boolean }) => {
    if (!KNOWN_MODEL_ROUTING_AGENTS.includes(agent as typeof KNOWN_MODEL_ROUTING_AGENTS[number])) {
      logError(`Unknown agent "${agent}". Valid agents: ${KNOWN_MODEL_ROUTING_AGENTS.join(", ")}`);
      process.exit(1);
    }
    ...
    try {
      await mutateProjectConfig(root, (config) => {
        const overrides = config.modelOverrides && typeof config.modelOverrides === "object" && !Array.isArray(config.modelOverrides)
          ? config.modelOverrides as Record<string, unknown>
          : {};
  ...
  .action(async (agent: string) => {
    ...
    try {
      const removed = await mutateProjectConfig(root, (config) => {
        ...
        if (!overrides || typeof overrides !== "object" || Array.isArray(overrides) || !(agent in overrides)) {
          return false;
        }
        const { [agent]: _, ...rest } = overrides as Record<string, unknown>;
...
configCommand
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      log("");
      const usingDefault = KNOWN_MODEL_ROUTING_AGENTS.filter((a) => !(a in overrides));
