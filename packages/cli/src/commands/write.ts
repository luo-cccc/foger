...
import { PipelineRunner, StateManager, executeChapterMutation, resolveChapterReviewMode } from "@actalk/inkos-core";
...
import { withChapterCheckpoint } from "../chapter-checkpoint.js";
  ...
  .action(async (bookIdArg: string | undefined, opts) => {
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (!opts.json) {
          const recoveryNotice = formatChapterRecoveryNotice(language, result.recovery);
      ...
      if (opts.json) {
        log(JSON.stringify(results.map(withChapterCheckpoint), null, 2));
      } else {
...
writeCommand
  ...
  .action(async (args: ReadonlyArray<string>, opts) => {
    ...
    try {
      ...
      const result = await executeChapterMutation({ state, pipeline }, {
        kind: "rewrite",
        bookId,
      ...
      if (opts.json) {
        log(JSON.stringify(withChapterCheckpoint(result), null, 2));
      } else {
        const recoveryNotice = formatChapterRecoveryNotice(language, result.recovery);
        if (recoveryNotice) log(recoveryNotice);
        for (const line of formatWriteNextResultLines(language, {
