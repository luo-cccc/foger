  ...
  .action(async (opts) => {
    ...
    try {
      ...
      const scheduler = new Scheduler({
        ...
        episodesPerCycle: config.daemon.episodesPerCycle,
        retryDelayMs: config.daemon.retryDelayMs,
        cooldownAfterEpisodeMs: config.daemon.cooldownAfterEpisodeMs,
        maxEpisodesPerDay: config.daemon.maxEpisodesPerDay,
        ...
        onEpisodeComplete: (bookId, episode, status) => {
          const icon = status === "ready-for-review"
            ...
            : status === "state-degraded"
              ? "x"
              : "!";
          log(`  [${icon}] ${bookId} Ep.${episode} — ${status}`);
        },
