...
function createSession(): InteractionSession {
  return {
    ...
    activeBookId: "harbor",
    activeEpisodeNumber: 12,
    automationMode: "semi",
    currentExecution: {
      status: "writing",
      bookId: "harbor",
      episodeNumber: 12,
      stageLabel: "writing episode",
    },
    pendingDecision: {
      kind: "review",
      bookId: "harbor",
      episodeNumber: 12,
      summary: "Review episode 12 before publishing.",
    },
    messages: [
      { role: "user", content: "continue current book", timestamp: 1 },
      { role: "assistant", content: "Working on episode 12.", timestamp: 2 },
    ],
    events: [
      {
        kind: "task.started",
...
describe("ink dashboard", () => {
  it("renders a codex-like single column with compact status and composer", async () => {
    ...
    expect(frame).toContain("writing episode");
