...
vi.mock("@actalk/inkos-core", () => ({
  ...
  // packages/core/src/__tests__/episode-review-mode.test.ts.
  resolveEpisodeReviewMode: (
...
vi.mock("../localization.js", () => ({
  formatEpisodeRecoveryNotice: vi.fn(() => null),
...
describe("inkos write next review mode", () => {
  ...
  it("passes book-level reviewMode into the pipeline config", async () => {
    ...
    expect(buildPipelineConfigMock).toHaveBeenCalledWith(
      expect.anything(),
      "/project",
      expect.objectContaining({ episodeReviewMode: "manual" }),
    );
  ...
  it("adds a stable checkpoint to JSON write results", async () => {
    writeNextEpisodeMock.mockResolvedValue({
      ...
      recovery: { kind: "rolled-back", episodeNumber: 4, rolledBackTo: 3, operationId: "interrupted-operation" },
    });
  ...
  it("stops a multi-episode batch after audit-failed without starting another episode", async () => {
    loadBookConfigMock.mockResolvedValue({ language: "zh" });
    loadConfigMock.mockResolvedValue({ llm: {}, writing: { reviewRetries: 1 } });
