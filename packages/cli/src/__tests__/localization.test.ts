...
describe("CLI localization", () => {
  ...
  it("explains chapter persistence recovery in terminal output", () => {
    expect(formatChapterRecoveryNotice("en", {
      kind: "rolled-back",
      chapterNumber: 4,
      rolledBackTo: 3,
    })).toBe("Recovered an incomplete chapter 4 write and rolled back to chapter 3 before continuing.");
    expect(formatChapterRecoveryNotice("en", {
      kind: "committed-cleanup",
      chapterNumber: 5,
    })).toBe("Cleared the recovery marker left by the committed chapter 5 write.");
    expect(formatChapterRecoveryNotice("zh", {
      kind: "rolled-back",
      chapterNumber: 4,
      rolledBackTo: 3,
    })).toContain("4");
    expect(formatChapterRecoveryNotice("en", undefined)).toBeNull();
  });
