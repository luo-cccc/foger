...
vi.mock("@actalk/inkos-core", () => ({
  ...
  executeCoreMutation: async (
    dependencies: { pipeline?: { auditDraft?: (bookId: string, chapterNumber?: number) => Promise<unknown> } },
    command: { kind: string; bookId: string; chapterNumber?: number },
  ) => {
    if (command.kind === "audit-chapter" && dependencies.pipeline?.auditDraft) {
      return await dependencies.pipeline.auditDraft(command.bookId, command.chapterNumber);
    }
