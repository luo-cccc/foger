...
export function applyEpisodeRuntimeStateDelta(params: {
  ...
  if (delta.episodeSummary && delta.episodeSummary.episode !== delta.episode) {
    throw new Error(`episode summary ${delta.episodeSummary.episode} does not match delta episode ${delta.episode}`);
  }
  ...
  if (delta.episodeSummary
    && snapshot.episodeSummaries.rows.some((row) => row.episode === delta.episodeSummary?.episode)
    && !allowReapply) {
    throw new Error(`duplicate summary row for episode ${delta.episodeSummary.episode}`);
  }
...
function applySummaryDelta(
  ...
  if (!delta.episodeSummary) {
    return { rows: [...state.rows].sort((left, right) => left.episode - right.episode) };
  }
  return {
    rows: [
      ...(allowReapply ? state.rows.filter((row) => row.episode !== delta.episodeSummary!.episode) : state.rows),
      delta.episodeSummary,
