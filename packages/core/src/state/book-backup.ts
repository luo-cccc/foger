...
export interface BookBackupState {
  ...
  recoverIncompleteEpisodePersistence?(bookId: string): Promise<unknown>;
...
export async function executeBookBackupCommand(
  ...
  try {
    if (command.kind === "backup-book") {
      await state.recoverIncompleteEpisodePersistence?.(command.bookId);
...
async function validateBookDirectory(bookDir: string, expectedBookId: string): Promise<void> {
  ...
  if (await pathExists(indexPath)) {
    ...
    if (!Array.isArray(index)) throw new Error(`Invalid episode index in backup for "${expectedBookId}".`);
  }
}
