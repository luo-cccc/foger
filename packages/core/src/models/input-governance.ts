...
export const ChapterMemoSchema = z.object({
  ...
  shotMin: z.number().int().min(1).max(12).optional(),
  shotMax: z.number().int().min(1).max(12).optional(),
});

type ParsedChapterMemo = z.infer<typeof ChapterMemoSchema>;
export type ChapterMemo = Omit<ParsedChapterMemo, "volumeKrRefs" | "volumeKrRationale"> & {
  readonly volumeKrRefs?: ParsedChapterMemo["volumeKrRefs"];
  readonly volumeKrRationale?: ParsedChapterMemo["volumeKrRationale"];
};
