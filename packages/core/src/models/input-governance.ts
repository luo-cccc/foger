...
export const EpisodeMemoSchema = z.object({
  episode: z.number().int().min(1),
  goal: z.string().min(1).max(50),
  isGoldenOpening: z.boolean().default(false),
  body: z.string().min(1),
  threadRefs: z.array(z.string()).default([]),
  volumeKrRefs: z.array(z.string()).default([]),
  volumeKrRationale: z.string().default(""),
  payoff: z.string().optional(),
  relationshipPressure: z.string().optional(),
  reversalSetup: z.string().optional(),
  reversalTurn: z.string().optional(),
  reversalConsequence: z.string().optional(),
  emotionalHook: z.string().optional(),
  endState: z.string().optional(),
  incomingState: z.string().optional(),
  objective: z.string().optional(),
  opposition: z.string().optional(),
  causalEscalation: z.string().optional(),
  turn: z.string().optional(),
  localDramaticResult: z.string().optional(),
  outgoingPressure: z.string().optional(),
  handoffState: z.string().optional(),
  informationPermissions: z.string().optional(),
  sceneLimit: z.number().int().min(1).max(3).optional(),
  shotMin: z.number().int().min(1).max(30).optional(),
  shotMax: z.number().int().min(1).max(30).optional(),
});

type ParsedEpisodeMemo = z.infer<typeof EpisodeMemoSchema>;
export type EpisodeMemo = Omit<ParsedEpisodeMemo, "volumeKrRefs" | "volumeKrRationale"> & {
  readonly volumeKrRefs?: ParsedEpisodeMemo["volumeKrRefs"];
  readonly volumeKrRationale?: ParsedEpisodeMemo["volumeKrRationale"];
};
