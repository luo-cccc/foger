...
export const QualityGatesSchema = z.object({
  ...
  maxEpisodeTokens: z.number().int().min(1).default(100_000),
  ...
  maxFallbacksPerEpisode: z.number().int().min(0).default(0),
...
export const ProjectConfigSchema = z.object({
  ...
  daemon: z.object({
    ...
    episodesPerCycle: z.number().int().min(1).max(20).default(1),
    retryDelayMs: z.number().int().min(0).default(30_000),
    cooldownAfterEpisodeMs: z.number().int().min(0).default(10_000),
    maxEpisodesPerDay: z.number().int().min(1).default(50),
