 ...
 *
 * Nothing here mutates truth files. Episode-level state deltas (runtime-state.ts)
...
export const CanonClaimSchema = z.object({
  ...
  // Deterministic canon lifecycle: claims settle when the episode state
  // establishes the fact (see deriveEpisodeCanonUpdates). Old claims files
  // without these fields keep the "active" default with zero migration.
  status: z.enum(["active", "resolved", "superseded"]).default("active"),
  statusUpdatedAtEpisode: z.number().int().min(1).optional(),
  // Asset-facing seam for the future image/video production chain. Structure
  // only this round; nothing generates or fills these references yet.
  assetRefs: z.preprocess(
    (value) => value === null ? undefined : value,
    z.object({
      character: z.array(z.string()).default([]),
      location: z.array(z.string()).default([]),
      prop: z.array(z.string()).default([]),
    }).optional(),
  ),
});
...
// ---------------------------------------------------------------------------
// Asset registry — structured seam for the future image/video production
// chain. This round only establishes the schema and storage; no generator
// consumes it yet, and empty registries are valid.
// ---------------------------------------------------------------------------
...
export const AssetRegistrySchema = z.object({
  ...
  characters: z.array(z.object({
    ...
    propStates: z.array(z.string()).default([]),
  })).default([]),
...
export function emptyAssetRegistry(): AssetRegistry {
  return { version: 1, characters: [], locations: [], props: [] };
}

// ---------------------------------------------------------------------------
// Unclaimed episode facts — deterministic candidates for a later canon
// refresh. Facts that no existing claim covers after an episode settles.
// ---------------------------------------------------------------------------

export const UnclaimedFactsFileSchema = z.object({
  version: z.literal(1),
  updatedAt: z.string(),
  facts: z.array(z.object({
    fact: z.string().min(1),
    sourceEpisode: z.number().int().min(1),
  })).default([]),
});
export type UnclaimedFactsFile = z.infer<typeof UnclaimedFactsFileSchema>;
