...
import { defaultEpisodeLength } from "../utils/length-metrics.js";
...
import { loadEpisodesFromPath } from "./episode-import-source.js";
import type { AgentContext } from "../agents/base.js";
import { ActionPayloadSchema, type ActionPayload } from "../interaction/action-envelope.js";
import { executeCoreMutation } from "../pipeline/episode-mutations.js";
...
function errorResult<T = undefined>(text: string, details?: T): AgentToolResult<T> {
  return {
    content: [{ type: "text", text }],
    details: details as T,
    isError: true,
  } as AgentToolResult<T> & { readonly isError: true };
}
...
type ProposedActionTargetRoute = "import:episodes" | "import:canon";
...
function proposedActionTargetRoute(action: ProposeActionParamsType["action"]): ProposedActionTargetRoute | undefined {
  if (action === "continuation_import") return "import:episodes";
  return undefined;
}
...
const SubAgentParams = Type.Object({
  ...
  ], { description: "Sub-agent to run. architect initialises a new book; writer writes the next episode; auditor reviews; reviser revises; exporter exports." }),
  ...
  episodeNumber: Type.Optional(Type.Number({ description: "Target episode number for auditor / reviser." })),
  ...
  episodeDurationSeconds: Type.Optional(Type.Number({ description: "architect only: duration per episode in seconds. Default: 150" })),
  ...
  format: Type.Optional(Type.Union([
    Type.Literal("screenplay-md"),
    Type.Literal("screenplay-json"),
    Type.Literal("dialogue"),
  ], { description: "exporter only: screenplay-md, screenplay-json, or dialogue" })),
  approvedOnly: Type.Optional(Type.Boolean({ description: "exporter only: export only approved episodes. Default: false" })),
});
...
const ArchitectCreateSubAgentParams = Type.Object({
  ...
  episodeDurationSeconds: Type.Optional(Type.Number({ description: "Confirmed duration per episode in seconds. Default: 150" })),
});
  ...
  return {
    name: "sub_agent",
      ...
      : "Delegate a heavy operation to a specialised sub-agent. " +
        "Use agent='architect' to initialise a new book, 'writer' to write the next episode, " +
        "'auditor' to audit quality, 'reviser' to revise a episode, 'exporter' to export.",
    label: "Sub-Agent",
    ...
    async execute(
      ...
      try {
        if (options.architectCreateOnly && agent !== "architect") {
          throw new Error("This confirmed book-creation turn can only run the architect. Open the created book or use the book session to write episodes.");
        }
        if (!activeBookId && agent !== "architect") {
          return errorResult("No active book. Only the architect agent can create a book from this session.");
        }
        ...
        switch (agent) {
          case "architect": {
            ...
            if (revise) {
              if (!activeBookId) {
                return errorResult("Open the book first before revising its foundation.");
              }
            ...
            if (!resolvedTitle) {
              return errorResult('Error: title is required for the architect agent.');
            }
            ...
            await pipeline.initBook(
              {
                ...
                targetEpisodes: createBookPayload?.targetEpisodes ?? targetEpisodes ?? 100,
                episodeDurationSeconds: createBookPayload?.episodeDurationSeconds ?? episodeDurationSeconds ?? EPISODE_DURATION_TARGET_SECONDS,
                createdAt: now,
                updatedAt: now,
              },
          ...
          case "writer": {
            ...
            progress(`Writing next episode for "${targetBookId}"...`);
            const result = await pipeline.writeNextEpisode(targetBookId, episodeDurationSeconds);
            progress(`Writer finished episode for "${targetBookId}".`);
            const resultStatus = (result as any).status;
            const resultDurationSeconds = (result as any).episodeDurationSeconds ?? "unknown";
            const episodeNumberResult = (result as any).episodeNumber ?? (result as any).episodeNumber;
            const titleResult = (result as any).title;
            const episodeLabel = episodeNumberResult ?? "latest";
            const message = resultStatus !== "ready-for-review"
              ? (sessionIsZh
                  ? `已为 ${targetBookId} 写出第 ${episodeLabel} 集，时长 ${resultDurationSeconds} 秒，但审稿未通过，状态 ${resultStatus}，需要复核后再继续。`
                  : `Episode output for "${targetBookId}" ended with status "${resultStatus}" and needs review before it is treated as complete. Duration: ${resultDurationSeconds}s.`)
              : (sessionIsZh
                  ? `已为 ${targetBookId} 完成第 ${episodeLabel} 集，时长 ${resultDurationSeconds} 秒。`
                  : `Episode written for "${targetBookId}". Duration: ${resultDurationSeconds}s.`);
            const details = {
              kind: "episode_written",
              bookId: targetBookId,
              episodeNumber: episodeNumberResult,
              title: titleResult,
              episodeDurationSeconds: resultDurationSeconds,
              status: resultStatus,
            };
            return resultStatus === "ready-for-review"
              ? textResult(message, details)
          ...
          case "auditor": {
            ...
            if (!projectRoot) {
              throw new Error("Episode audit requires a project root for the shared mutation command.");
            }
            progress(`Auditing episode ${episodeNumber ?? "latest"} for "${targetBookId}"...`);
            ...
            const issueLines = (audit.issues ?? [])
              ...
              .join("\n");
            return textResult(
              `Audit episode ${audit.episodeNumber}: ${audit.passed ? "PASSED" : "FAILED"}, ${(audit.issues ?? []).length} issue(s).` +
              (issueLines ? `\n${issueLines}` : ""),
            );
          }
          ...
          case "reviser": {
            ...
            progress(`Revising "${targetBookId}" episode ${episodeNumber ?? "latest"} in ${resolvedMode} mode...`);
            const result = await pipeline.reviseDraft(targetBookId, episodeNumber, resolvedMode, instruction);
            const applied = result.applied !== false;
            const resultEpisode = result.episodeNumber ?? episodeNumber;
            const details = {
              kind: "episode_revision",
              bookId: targetBookId,
              episodeNumber: resultEpisode,
              mode: resolvedMode,
              applied,
            ...
            if (!applied) {
              ...
              const diagnosticText = diagnostics
                ...
                : "";
              return errorResult(
                `Revision not applied for "${targetBookId}" episode ${resultEpisode ?? "latest"}: ${result.skippedReason ?? result.status ?? "pipeline kept the original episode"}.${diagnosticText}`,
                details,
              );
            }
            progress(`Revision complete for "${targetBookId}".`);
            return textResult(
              `Revision (${resolvedMode}) complete for "${targetBookId}" episode ${resultEpisode ?? "latest"}.`,
              details,
            );
          }
          ...
          case "exporter": {
            ...
            if (!projectRoot) return errorResult("Error: exporter requires projectRoot.");
            const inferredFormat = format ?? "screenplay-md";
            const exportApprovedOnly = approvedOnly ?? false;
            const state = new StateManager(projectRoot);
            ...
            return textResult(
              `Exported "${targetBookId}": ${result.episodesExported} episodes, ${result.totalDurationSeconds}s → ${result.outputPath}`,
            );
          }

          default:
            return errorResult(`Unknown agent: ${agent}`);
        }
      } catch (err: any) {
        ...
        return errorResult(`[sub_agent] "${agent}" failed: ${err?.message ?? String(err)}`);
      }
    },
  };
}
...
const ImportEpisodesParams = Type.Object({
  bookId: Type.Optional(Type.String({
  ...
  sourcePath: Type.String({
    description: "Local path of the EpisodeScript source: either the stored_path from the Uploaded Files block or an absolute path on this machine that the user provided. A directory imports each .md/.json file as one episode in filename order; a single Markdown file is split into episodes automatically by heading lines.",
  }),
  splitPattern: Type.Optional(Type.String({
    description: "Single-file mode only: custom JavaScript regex source matching episode heading lines. Omit to use the default pattern, which matches \"第X集\" and \"Episode N\" headings.",
  })),
  resumeFrom: Type.Optional(Type.Number({
    description: "Resume an interrupted import from episode N (1-based). Required when the book already has episodes: replay starts at episode N and earlier episodes are kept. Omit for a fresh import into an empty book.",
  })),
...
type ImportEpisodesParamsType = Static<typeof ImportEpisodesParams>;

export function createImportEpisodesTool(
  pipeline: PipelineRunner,
  activeBookId: string | null,
  projectRoot: string,
): AgentTool<typeof ImportEpisodesParams> {
  return {
    name: "import_episodes",
    description:
      "Import existing EpisodeScript JSON or screenplay Markdown from a local file or directory into an InkOS book as real episodes (not reference material). " +
      "InkOS reverse-engineers foundation/truth files from the imported text and replays every episode to rebuild story state, so the book can be continued afterwards.",
    label: "Import Episodes",
    parameters: ImportEpisodesParams,
    async execute(
      _toolCallId: string,
      params: ImportEpisodesParamsType,
      _signal?: AbortSignal,
      onUpdate?: AgentToolUpdateCallback,
    ): Promise<AgentToolResult<unknown>> {
      const targetBookId = resolveToolBookId("import_episodes", params.bookId, activeBookId);

      const state = new StateManager(projectRoot);
      const existingEpisodeCount = (await state.getNextEpisodeNumber(targetBookId)) - 1;
      if (existingEpisodeCount > 0 && params.resumeFrom === undefined) {
        throw new Error(
          `Book "${targetBookId}" already has ${existingEpisodeCount} episode(s). ` +
          `Pass resumeFrom=<n> to resume/append from episode n, or ask the user to clear the existing episodes first.`,
        );
      }
      ...
      onUpdate?.(textResult(`Reading episodes from ${resolvedSourcePath}...`));
      const episodes = await loadEpisodesFromPath(resolvedSourcePath, params.splitPattern);

      onUpdate?.(textResult(`Found ${episodes.length} episode(s); importing into "${targetBookId}"...`));
      ...
      return textResult(
        [
          `Imported ${result.importedCount} episode(s) into book "${result.bookId}".`,
          `Total imported duration: ${result.totalDurationSeconds}s. Next episode to write: ${result.nextEpisode}.`,
          regeneratedFoundation
            ? "Foundation and truth files were reverse-engineered from the imported text; episode files and the episode index were rebuilt by sequential replay."
            : `Resumed replay from episode ${params.resumeFrom}; earlier episodes and the existing foundation were kept.`,
  ...
  return {
    ...
    async execute(_toolCallId, params): Promise<AgentToolResult<undefined>> {
      try {
        ...
        return errorResult(`write_truth_file failed: ${err?.message ?? String(err)}`);
      }
    },
  };
}
...
export function createRenameEntityTool(
  ...
  return {
    name: "rename_entity",
    description: "Rename an entity across truth files and episodes using deterministic edit control.",
    label: "Rename Entity",
...
const PatchEpisodeTextParams = Type.Object({
  bookId: Type.Optional(Type.String({ description: "Book ID. Omit to use the active book." })),
  episodeNumber: Type.Number({ description: "Episode number to patch." }),
  targetText: Type.String({ description: "Exact text to replace." }),
  replacementText: Type.String({ description: "Replacement text." }),
});

export function createPatchEpisodeTextTool(
  pipeline: PipelineRunner,
  projectRoot: string,
  activeBookId: string | null,
): AgentTool<typeof PatchEpisodeTextParams> {
  const tools = createDeterministicInteractionTools(pipeline, projectRoot);
  return {
    name: "patch_episode_text",
    description: "Apply a deterministic local text patch to an episode and mark it for review.",
    label: "Patch Episode",
    parameters: PatchEpisodeTextParams,
    async execute(_toolCallId, params): Promise<AgentToolResult<undefined>> {
      const bookId = resolveToolBookId("patch_episode_text", params.bookId, activeBookId);
      const result = await tools.patchEpisodeText(
        bookId,
      ...
      const summary = result.__interaction?.responseText ?? `Patched episode ${params.episodeNumber} for "${bookId}".`;
      return textResult(summary);
    },
  };
}

const ReplaceEpisodeTextParams = Type.Object({
  bookId: Type.Optional(Type.String({ description: "Book ID. Omit to use the active book." })),
  episodeNumber: Type.Number({ description: "Episode number to replace." }),
  fullText: Type.String({ description: "The complete replacement episode markdown/text supplied by the user." }),
});

export function createReplaceEpisodeTextTool(
  pipeline: PipelineRunner,
  projectRoot: string,
  activeBookId: string | null,
): AgentTool<typeof ReplaceEpisodeTextParams> {
  const tools = createDeterministicInteractionTools(pipeline, projectRoot);
  return {
    name: "replace_episode_text",
    description:
      "Replace a whole existing episode with user-supplied full episode text and mark it for review. " +
      "Use only when the user provides the complete replacement episode; for model-generated rewrites use sub_agent reviser.",
    label: "Replace Episode",
    parameters: ReplaceEpisodeTextParams,
    async execute(_toolCallId, params): Promise<AgentToolResult<undefined>> {
      const bookId = resolveToolBookId("replace_episode_text", params.bookId, activeBookId);
      const result = await tools.replaceEpisodeText(
        bookId,
        params.episodeNumber,
        params.fullText,
      ) as {
      ...
      const summary = result.__interaction?.responseText ?? `Replaced episode ${params.episodeNumber} for "${bookId}".`;
      return textResult(summary);
    },
  };
}
  ...
  return {
    ...
    async execute(
      ...
      try {
        ...
        return errorResult(`Failed to read "${params.path}": ${err?.message ?? String(err)}`);
      }
    },
  };
}
...
export function createGrepTool(projectRoot: string): AgentTool<typeof GrepParams> {
  ...
  return {
    name: "grep",
    description:
      "Search for a text pattern across a book's story/ and episodes/ directories. Returns matching lines.",
    label: "Search",
    ...
    async execute(
      ...
      try {
        ...
        await Promise.all([
          ...
          searchDir(join(bookDir, "episodes"), "episodes/"),
        ]);
        ...
        return errorResult(`Grep failed: ${err?.message ?? String(err)}`);
      }
    },
  };
}
...
const LsParams = Type.Object({
  ...
  subdir: Type.Optional(
    Type.String({ description: "Subdirectory within the book, e.g. 'story', 'episodes', 'story/runtime'" }),
  ),
});
...
export function createLsTool(projectRoot: string): AgentTool<typeof LsParams> {
  ...
  return {
    name: "ls",
    description: "List files in a book directory. Optionally specify a subdirectory like 'story' or 'episodes'.",
    label: "List Files",
    parameters: LsParams,
    async execute(
      _toolCallId: string,
      ...
      try {
        ...
        return errorResult(`Failed to list "${params.bookId}/${params.subdir ?? ""}": ${err?.message ?? String(err)}`);
      }
    },
  };
}
