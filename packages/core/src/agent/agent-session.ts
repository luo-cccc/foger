...
function looksLikeUnsavedEpisodeProse(text: string): boolean {
  const trimmed = text.trim();
  if (trimmed.length < 800) return false;
  if (looksLikeEpisodeRevisionPlan(trimmed)) return false;
  return /(^|\n)\s{0,3}#{1,3}\s*(?:第\s*[0-9一二三四五六七八九十百千]+\s*集|Episode\s+\d+)/i.test(trimmed);
}

function looksLikeEpisodeRevisionPlan(text: string): boolean {
  const headingLines = text
    .split(/\n+/)
    .filter((line) => /^\s{0,3}#{1,3}\s*(?:第\s*[0-9一二三四五六七八九十百千]+\s*集|Episode\s+\d+)/i.test(line))
    .slice(0, 8);
...
function bookRawEpisodeBoundaryText(language: string): string {
  return language === "zh"
    ? "这次模型输出了疑似剧集分镜的聊天文本，但没有调用落盘工具。InkOS 不会把聊天文本当成已保存剧集：如果要续写，请发送“继续写下一集”；如果要修改已有剧集，请发送“重写/修订第 N 集 + 具体要求”，系统会走 Reviser/Writer 管线并校验 EpisodeScript 后落盘。"
    : "The model produced episode-like prose in chat without calling a persistence tool. InkOS will not treat chat prose as a saved episode. Ask to write the next episode only when you want to append; ask to rewrite/revise episode N with concrete requirements when you want to change existing episodes.";
}
...
function createAgentToolsForMode(params: {
  ...
  const importEpisodesTool = createImportEpisodesTool(params.pipeline, params.bookId, params.projectRoot);
  const isConfirmed = (
  ...
  if (params.sessionKind === "chat") {
    return [proposalTool, importEpisodesTool];
  }
  ...
  const bookTools = [
    ...
    createPatchEpisodeTextTool(params.pipeline, params.projectRoot, params.bookId),
    createReplaceEpisodeTextTool(params.pipeline, params.projectRoot, params.bookId),
    importEpisodesTool,
  ...
  if (params.sessionKind === "edit") {
    return bookTools.filter((tool) => !["sub_agent", "import_episodes"].includes(tool.name));
  }
...
async function runAgentSessionUnlocked(
  ...
  const persistAgentEvent = async (event: AgentEvent): Promise<void> => {
    ...
    if (role === "assistant" && sessionKind === "book" && !successfulProductionToolResultSeen) {
      ...
      if (looksLikeUnsavedEpisodeProse(extractTextFromAssistant(assistant))) {
        replaceAssistantText(assistant, bookRawEpisodeBoundaryText(language));
      }
    }
