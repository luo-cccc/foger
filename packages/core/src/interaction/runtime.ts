...
type InteractionReviseMode = "spot-fix" | "rewrite";
...
export interface InteractionRuntimeTools {
  ...
  readonly exportBook?: (bookId: string, options: {
    readonly format?: "screenplay-md" | "screenplay-json" | "dialogue";
  ...
  readonly writeNextEpisode: (bookId: string) => Promise<unknown>;
  readonly reviseDraft: (bookId: string, episodeNumber: number, mode: InteractionReviseMode) => Promise<unknown>;
  readonly patchEpisodeText: (
    bookId: string,
    episodeNumber: number,
    targetText: string,
    replacementText: string,
  ) => Promise<unknown>;
  readonly replaceEpisodeText: (
    bookId: string,
    episodeNumber: number,
    fullText: string,
  ) => Promise<unknown>;
...
interface InteractionToolMetadata {
  ...
  readonly activeEpisodeNumber?: number;
...
function extractToolMetadata(value: unknown): InteractionToolMetadata {
  const episodeNumber = typeof value === "object" && value !== null && "episodeNumber" in value
    ...
    ? (value as { episodeNumber: number }).episodeNumber
    : undefined;

  if (!value || typeof value !== "object" || !("__interaction" in value)) {
    return {
      ...(episodeNumber !== undefined ? { activeEpisodeNumber: episodeNumber } : {}),
    };
  }
  ...
  return {
    ...interaction,
    ...(interaction?.activeEpisodeNumber === undefined && episodeNumber !== undefined
  ...
  switch (request.intent) {
    ...
    case "continue_book":
      return {
        ...
        episodeNumber: session.activeEpisodeNumber,
        stageLabel: localize(language, {
          zh: "准备剧集输入",
          en: "preparing episode inputs",
        }),
      };
    case "create_book":
      return {
    ...
    case "rewrite_episode":
      return {
        status: "repairing",
        bookId: request.bookId ?? session.activeBookId,
        episodeNumber: request.episodeNumber ?? session.activeEpisodeNumber,
        stageLabel: request.intent === "rewrite_episode"
          ? localize(language, { zh: "重写剧集", en: "rewriting episode" })
          : localize(language, { zh: "修订剧集", en: "revising episode" }),
      };
    case "update_focus":
...
function shouldWaitForHuman(
  ...
  const contentIntent = request.intent === "write_next"
    ...
    || request.intent === "revise_episode"
    || request.intent === "rewrite_episode"
    || request.intent === "patch_episode_text"
    || request.intent === "replace_episode_text";
  ...
  switch (request.intent) {
    ...
    case "create_book": {
      ...
      const toolResult = await tools.createBook({
        ...
        targetEpisodes: request.targetEpisodes ?? effectiveDraft?.targetEpisodes,
        episodeDurationSeconds: request.episodeDurationSeconds ?? effectiveDraft?.episodeDurationSeconds,
...
export async function runInteractionRequest(params: {
  ...
  const addEvent = (
    ...
    episodeNumber: nextSession.activeEpisodeNumber,
    detail,
  });
  ...
  switch (request.intent) {
    ...
    case "continue_book": {
      ...
      const toolResult = await params.tools.writeNextEpisode(bookId);
      const metadata = extractToolMetadata(toolResult);
      ...
      const pendingDecision = metadata.pendingDecision ?? buildPendingDecision(
        ...
        request,
        language,
        metadata.activeEpisodeNumber,
      );
      const completed = pendingDecision
        ? {
            ...session,
            pendingDecision,
            currentExecution: metadata.currentExecution ?? buildWaitingExecution(session, request, language, metadata.activeEpisodeNumber),
          }
        : {
    ...
    case "rewrite_episode": {
      ...
      if (!request.episodeNumber) {
        throw new Error(localize(language, {
          zh: "修订剧集需要剧集编号。",
          en: "Episode number is required for episode revision.",
        }));
      }
      const mode: InteractionReviseMode = request.intent === "rewrite_episode" ? "rewrite" : "spot-fix";
      const toolResult = await params.tools.reviseDraft(bookId, request.episodeNumber, mode);
      const metadata = extractToolMetadata(toolResult);
      ...
      const completed = pendingDecision
        ? {
            ...session,
            pendingDecision,
            currentExecution: metadata.currentExecution ?? buildWaitingExecution(session, request, language, episodeNumber),
          }
        : {
            ...markCompleted(session),
            currentExecution: metadata.currentExecution ?? markCompleted(session).currentExecution,
      ...
      return {
        session: addEvent(completed, "task.completed", "completed", localize(language, {
          zh: request.intent === "rewrite_episode"
            ? `已为 ${bookId} 完成剧集重写。`
            : `已为 ${bookId} 完成剧集修订。`,
        ...
        responseText: metadata.responseText ?? (
          pendingDecision
            ? localize(language, {
                zh: request.intent === "rewrite_episode"
                  ? `已为 ${bookId} 完成剧集重写，等待你的下一步决定。`
                  : `已为 ${bookId} 完成剧集修订，等待你的下一步决定。`,
    ...
    case "patch_episode_text": {
      ...
      if (!request.episodeNumber || !request.targetText || !request.replacementText) {
        throw new Error(localize(language, {
          zh: "剧本修补需要剧集编号、目标文本和替换文本。",
          en: "Episode patch requires episode number, target text, and replacement text.",
        }));
      }
      const toolResult = await params.tools.patchEpisodeText(
        bookId,
      ...
      return {
        session: addEvent(completed, "task.completed", "completed", localize(language, {
          zh: `已修补 ${bookId} 的第 ${episodeNumber} 集。`,
          en: `Patched episode ${episodeNumber} for ${bookId}.`,
        })),
        responseText: metadata.responseText ?? (
          pendingDecision
            ? localize(language, {
                zh: `已修补 ${bookId} 的第 ${episodeNumber} 集，等待你的下一步决定。`,
                en: `Patched episode ${episodeNumber} for ${bookId}; waiting for your next decision.`,
              })
            : localize(language, {
                zh: `已修补 ${bookId} 的第 ${episodeNumber} 集。`,
                en: `Patched episode ${episodeNumber} for ${bookId}.`,
    ...
    case "replace_episode_text": {
      ...
      if (!request.episodeNumber || !request.fullText) {
        throw new Error(localize(language, {
          zh: "整集替换需要剧集编号和完整 EpisodeScript 或分镜 Markdown。",
          en: "Whole-episode replacement requires episode number and fullText.",
        }));
      }
      const toolResult = await params.tools.replaceEpisodeText(
        bookId,
        request.episodeNumber,
        request.fullText,
      );
      ...
      return {
        session: addEvent(completed, "task.completed", "completed", localize(language, {
          zh: `已替换 ${bookId} 的第 ${episodeNumber} 集。`,
          en: `Replaced episode ${episodeNumber} for ${bookId}.`,
        })),
        responseText: metadata.responseText ?? (
          pendingDecision
            ? localize(language, {
                zh: `已替换 ${bookId} 的第 ${episodeNumber} 集，等待你的下一步决定。`,
                en: `Replaced episode ${episodeNumber} for ${bookId}; waiting for your next decision.`,
              })
            : localize(language, {
                zh: `已替换 ${bookId} 的第 ${episodeNumber} 集。`,
                en: `Replaced episode ${episodeNumber} for ${bookId}.`,
    ...
    case "chat": {
      ...
      const responseText = metadata.responseText ?? (
        /^(hi|hello|hey|你好|嗨|哈喽)$/i.test(prompt)
          ? (bookId
              ? localize(language, {
                  zh: `你好。当前作品是 ${bookId}。你可以让我继续写、修订剧集，或者解释当前卡住的原因。`,
                  en: `Hi. Active book is ${bookId}. Ask me to continue, revise a episode, or explain what is blocked.`,
                })
              : localize(language, {
          ...
          : (bookId
              ? localize(language, {
                  zh: `我在。当前作品是 ${bookId}。你可以让我继续写、修订剧集、重写、调整焦点，或者查看流水线为何停止。`,
                  en: `I’m here. Active book is ${bookId}. You can ask me to continue, revise a episode, rewrite, change focus, or inspect why the pipeline stopped.`,
                })
              : localize(language, {
