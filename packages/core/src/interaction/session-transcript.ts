...
const APPEND_STATE_TTL_MS = 5 * 60 * 1000;

interface TranscriptAppendState {
  queue: Promise<void>;
...
const appendStates = new Map<string, TranscriptAppendState>();
let appendStateCleanupTimer: ReturnType<typeof setInterval> | null = null;

function assertSafeSessionId(sessionId: string): string {
  return SessionIdSchema.parse(sessionId);
}

function ensureAppendStateCleanupTimer(): void {
  if (appendStateCleanupTimer) return;
  appendStateCleanupTimer = setInterval(() => {
    const cutoff = Date.now() - APPEND_STATE_TTL_MS;
    for (const [key, state] of appendStates) {
      if (state.lastActive < cutoff) appendStates.delete(key);
    }
    if (appendStates.size === 0 && appendStateCleanupTimer) {
      clearInterval(appendStateCleanupTimer);
      appendStateCleanupTimer = null;
    }
  }, 60_000);
  appendStateCleanupTimer.unref?.();
}
...
export function transcriptPath(projectRoot: string, sessionId: string): string {
  return join(sessionsDir(projectRoot), `${assertSafeSessionId(sessionId)}.jsonl`);
}

export function legacyBookSessionPath(projectRoot: string, sessionId: string): string {
  return join(sessionsDir(projectRoot), `${assertSafeSessionId(sessionId)}.json`);
}

export function evictTranscriptAppendState(projectRoot: string, sessionId: string): void {
  const safeSessionId = assertSafeSessionId(sessionId);
  appendStates.delete(`${projectRoot}:${safeSessionId}`);
}
...
export async function appendTranscriptEvents(
  ...
  const safeSessionId = assertSafeSessionId(sessionId);
  const key = `${projectRoot}:${safeSessionId}`;
  const state = appendStates.get(key) ?? {
    queue: Promise.resolve(),
    lastActive: Date.now(),
  };
  state.lastActive = Date.now();
  appendStates.set(key, state);
  ensureAppendStateCleanupTimer();

  const previous = state.queue;
  let result: TranscriptEvent[] = [];

  const next = previous.then(async () => {
    const events = state.events ?? await readTranscriptEvents(projectRoot, safeSessionId);
    state.events = events;
    state.nextSeq ??= events.reduce((max, event) => Math.max(max, event.seq), 0) + 1;
    ...
    await appendFile(
      transcriptPath(projectRoot, safeSessionId),
      `${result.map((event) => JSON.stringify(event)).join("\n")}\n`,
      "utf-8",
    );
    events.push(...result);
    state.nextSeq = Math.max(...result.map((event) => event.seq)) + 1;
    state.lastActive = Date.now();
  });
