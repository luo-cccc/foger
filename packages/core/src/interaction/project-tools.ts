...
type PipelineLike = Pick<PipelineRunner, "writeNextEpisode" | "reviseDraft"> & {
  readonly initBook?: (
    book: BookConfig,
    options?: {
...
type StateLike = Pick<StateManager, "ensureControlDocuments" | "bookDir" | "loadBookConfig" | "loadEpisodeIndex" | "saveEpisodeIndex" | "listBooks" | "acquireBookLock">;
...
function buildBookConfig(input: {
  ...
  const targetEpisodes = input.targetEpisodes ?? 100;
  return {
    ...
    format: "screenplay",
    targetEpisodes,
    episodeDurationSeconds: input.episodeDurationSeconds ?? EPISODE_DURATION_TARGET_SECONDS,
...
export function buildEpisodeFileLookup(files: ReadonlyArray<string>): ReadonlyMap<number, string> {
  const lookup = new Map<number, string>();
  for (const file of files) {
    if (!file.endsWith(".md") || !/^\d{4}/.test(file)) {
      continue;
    }
    const episodeNumber = parseInt(file.slice(0, 4), 10);
    if (!lookup.has(episodeNumber)) {
      lookup.set(episodeNumber, file);
    }
  }
  return lookup;
}
...
function mapStageMessageToStatus(message: string): InteractionEvent["status"] | undefined {
  ...
  if (
    lower.includes("planning next episode")
    || lower.includes("generating foundation")
    || lower.includes("reviewing foundation")
    || lower.includes("preparing episode inputs")
    || message.includes("规划下一集意图")
    || message.includes("生成基础设定")
    || message.includes("审核基础设定")
    || message.includes("准备剧集输入")
  ) {
    return "planning";
  }
  if (
    lower.includes("composing episode runtime context")
    || message.includes("组装剧集运行时上下文")
  ) {
    return "composing";
  }
  if (
    lower.includes("writing episode draft")
    || message.includes("撰写剧集草稿")
  ) {
    return "writing";
  }
  if (
  ...
  if (
    ...
    || lower.includes("revising episode")
    || lower.includes("rewrite")
    || lower.includes("repair")
    || message.includes("自动修复")
    || message.includes("整集改写")
...
async function withPipelineInteractionTelemetry<T extends { episodeNumber?: number }>(
    ...
    return {
      ...
      __interaction: {
        ...
        ...(typeof result.episodeNumber === "number"
          ? { activeEpisodeNumber: result.episodeNumber }
          : {}),
      },
    };
  } finally {
  ...
  return {
    ...
    exportBook: async (bookId, options) => {
      ...
      return {
        ...result,
        __interaction: {
          responseText: `Exported ${bookId} to ${result.outputPath} (${result.episodesExported} episodes).`,
          details: {
    ...
    writeNextEpisode: (bookId) => withPipelineInteractionTelemetry(
      instrumentedPipeline,
      bookId,
      () => pipeline.writeNextEpisode(bookId),
    ),
    reviseDraft: (bookId, episodeNumber, mode) => withPipelineInteractionTelemetry(
      instrumentedPipeline,
      bookId,
      () => pipeline.reviseDraft(bookId, episodeNumber, mode as ReviseMode),
    ),
    patchEpisodeText: async (bookId, episodeNumber, targetText, replacementText) => {
    ...
    replaceEpisodeText: async (bookId, episodeNumber, fullText) => {
      const result = await executeCoreMutation({ state }, {
        kind: "save-episode",
        bookId,
        episodeNumber,
