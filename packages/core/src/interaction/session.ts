...
export const SessionKindSchema = z.enum(["chat", "book-create", "book", "edit"]);
...
export const InteractionSessionSchema = z.object({
  ...
  activeEpisodeNumber: z.number().int().min(1).optional(),
...
export function createBookSession(
  ...
  const safeSessionId = SessionIdSchema.parse(
