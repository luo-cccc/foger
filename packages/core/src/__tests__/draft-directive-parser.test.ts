...
describe("parseDraftDirectives", () => {
  ...
  it("extracts multiple fields of different types", () => {
    const raw = [
      ...
      ":::",
      "",
      ':::number{key="targetEpisodes" label="目标章数"}',
      "300",
      ":::",
    ].join("\n");
    ...
    expect(result.fields["targetEpisodes"]).toBe("300");
  });
  ...
  it("extracts fields nested inside a :::group", () => {
    const raw = [
      ...
      "300",
      ":::",
      ':::number{key="episodeLength" label="每章字数"}',
      "3000",
      ":::",
      ":::",
      "",
    ...
    expect(result.fields["targetEpisodes"]).toBe("300");
    expect(result.fields["episodeLength"]).toBe("3000");
  ...
  it("does not include group labels in summary (only leaf fields)", () => {
    const raw = [
      ':::group{label="篇幅设置"}',
      ':::number{key="episodeCount" label="总章数"}',
      "200",
      ":::",
      ":::",
    ].join("\n");
