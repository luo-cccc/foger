...
function compiled(overrides: Partial<CompiledEpisodeClaims>): CompiledEpisodeClaims {
  return {
    episodeNumber: 5,
    usable: [],
    revealNow: [],
...
describe("claim gates", () => {
  it("flags hidden claim references in the episode memo", () => {
    const secret = claim({
      id: "s-1",
  ...
  it("does not connect unrelated maintenance bypass words to broad world rules elsewhere in the episode", () => {
    const hardRules = [
      claim({
        id: "world-memory",
        claimType: "objective_rule",
