...
describe("StateValidatorAgent", () => {
  ...
  it("treats missing truth updates as blocking even when the model says PASS", async () => {
    ...
    vi.spyOn(agent as unknown as { chat: (...args: unknown[]) => Promise<unknown> }, "chat")
      .mockResolvedValue({
        content: "PASS\n[missing_state_change] Episode 2 changed the goal, but the state card stayed on episode 1.",
        usage: ZERO_USAGE,
      });

    const result = await agent.validate(
      "Episode 2 changes the goal.",
      2,
      "old state",
  ...
  it("passes authority truth context into the cross-file validation prompt", async () => {
    ...
    await agent.validate(
      ...
      "zh",
      {
        ...
        episodeSummaries: "第1章：发现第五条规则的漏洞。",
      },
    );
  ...
  it("does not silently truncate episode or authority context before validation", async () => {
    const agent = new StateValidatorAgent({
      client: {
        provider: "openai",
        apiFormat: "chat",
    ...
    await agent.validate(
      ...
      "zh",
      {
        ...
        episodeSummaries: `${"摘要".repeat(4000)}\nEPISODE_SUMMARIES_TAIL_MARKER`,
      },
    );

    const messages = chatSpy.mock.calls[0]?.[0] as Array<{ role: string; content: string }>;
    expect(messages[1]?.content).toContain("EPISODE_TAIL_MARKER");
    expect(messages[1]?.content).toContain("STORY_FRAME_TAIL_MARKER");
    expect(messages[1]?.content).toContain("BOOK_RULES_TAIL_MARKER");
    expect(messages[1]?.content).toContain("EPISODE_SUMMARIES_TAIL_MARKER");
  ...
  it("deterministically rejects a hook advanced this episode whose note denies movement", async () => {
    const agent = new StateValidatorAgent({
      client: {
        provider: "openai",
        apiFormat: "chat",
  ...
  it("deterministically rejects deferred hooks that refresh lastAdvancedEpisode", async () => {
    const agent = new StateValidatorAgent({
      client: {
        provider: "openai",
        apiFormat: "chat",
    ...
    const oldHooks = [
      "| hook_id | 起始章节 | 类型 | 状态 | 最近推进 | 预期回收 | 备注 |",
      "| --- | --- | --- | --- | --- | --- | --- |",
      "| H004 | 1 | 信息 | progressing | 3 | 查明替代名单 | 第3集获得部分明文 |",
    ].join("\n");
    const newHooks = [
      "| hook_id | 起始剧集 | 类型 | 状态 | 最近推进 | 预期回收 | 备注 |",
      "| --- | --- | --- | --- | --- | --- | --- |",
      "| H004 | 1 | 信息 | deferred | 4 | 查明替代名单 | 第3集获得部分明文 |",
    ].join("\n");

    const result = await agent.validate("正文没有推进替代名单。", 4, "old", "new", oldHooks, newHooks, "zh");

    expect(result.passed).toBe(false);
    expect(result.warnings[0]?.description).toContain("延后必须保留此前的推进剧集");
    expect(chatSpy).not.toHaveBeenCalled();
  });
