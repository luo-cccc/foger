...
function createValidAuditMemoBody(): string {
  return [
    "## Current task",
    "Advance the chapter clue without revealing any protected canon truth.",
    "",
    "## What the reader is waiting for right now",
    "The reader expects a concrete clue and a meaningful change in pressure.",
    "",
    "## To pay off / to keep buried",
    "Pay off the immediate search beat while keeping the sect truth buried.",
    "",
    "## What the slow / transitional beats carry",
    "Use transitions to preserve location, motive, and the current investigation rhythm.",
    "",
    "## Three-question check on the key choice",
    "The choice is motivated, costly enough, and changes the next available action.",
    "",
    "## Required end-of-chapter change",
    "End with a usable clue and a narrower set of safe next actions.",
    "",
    "## Hook ledger for this chapter",
    "No hook is resolved or opened; preserve the existing hook ledger unchanged.",
    "",
    "## Volume KR binding",
    "Buffer chapter: preserve the current volume objective without a visible KR advance.",
    "",
    "## Do not",
    "Do not reveal protected canon truth.",
  ].join("\n");
}
...
function createLongFormFoundationOutput() {
  return {
    storyBible: "# Story Bible\n\nThe old city hides a mentor debt under ordinary commerce.\n",
    volumeOutline: "# Volume Outline\n\n## 第一卷 暗账初开\n林月从雨夜账本追到内城旧契，逐步确认师门失踪不是单一仇杀。\n",
    storyFrame: [
      "# Story Frame",
      "",
      "林月在旧城雨巷追索师门债契，核心冲突是个人承诺与宗门账册之间的拉扯。",
      "世界铁律：誓契只记录真实承诺，不能伪造，也不能替他人偿还。",
    ].join("\n"),
    volumeMap: [
      "# Volume Map",
      "",
      "## 第一卷 暗账初开",
      "开篇聚焦旧城账房、失踪师父和被截断的誓契线索。每一章推进一条可验证线索。",
      "",
      "收束节奏：现场行动先于解释，线索兑现先于新谜题扩张。",
    ].join("\n"),
    roles: [
      {
        name: "林月",
        ...
        content: [
          ...
          "",
          "## Core",
          "欠师门一笔未明的债，外冷内急，面对账册证据时先查验再表态。",
          "",
          "## Current_State",
          "在旧城码头持有半枚誓契铜片，尚未公开身份。",
        ].join("\n"),
      },
    ],
    bookRules: [
    ...
    currentState: createStateCard({
      chapter: 0,
      location: "旧城码头",
      protagonistState: "林月握有半枚誓契铜片。",
      goal: "查清师父失踪前留下的账本去向。",
      conflict: "账房封门，城门巡丁开始搜查陌生修士。",
    }),
    pendingHooks: [
      "# Pending Hooks",
      "",
      "| hook_id | start_chapter | type | status | last_advanced_chapter | target_chapter | description |",
      "| --- | ---: | --- | --- | ---: | ---: | --- |",
      "| mentor-ledger | 0 | mystery | open | 0 | 4 | 师父失踪前留下的账本被谁拿走。 |",
    ].join("\n"),
  };
}
...
async function waitForFile(root: string, relativePath: string, timeoutMs: number = 1000): Promise<string> {
  ...
  throw lastError instanceof Error ? lastError : new Error(`Timed out waiting for ${absolutePath}`);
}

async function waitForDirectoryEntries(root: string, relativePath: string, timeoutMs: number = 1000): Promise<string[]> {
  const absolutePath = join(root, relativePath);
  const deadline = Date.now() + timeoutMs;
  let lastError: unknown;
...
describe("PipelineRunner", () => {
  ...
  it("runs the long-form creation flow from foundation to chapter artifacts and export", async () => {
    const root = await mkdtemp(join(tmpdir(), "inkos-long-form-flow-test-"));
    const bookId = "long-form-flow";
    ...
    const state = new StateManager(root);
    const chapterBody = [
      "雨停在子时前。林月推开旧账房的侧门，木柜里只剩半截湿纸。她没有急着翻找，而是先用铜片压住门缝，看见泥水里有一枚细小鞋印，鞋底纹路来自内城巡丁。",
      "",
      "掌柜躲在灯影后说账本早被烧毁。林月把湿纸放到火旁，墨迹慢慢浮出师父的签名。她问掌柜昨夜谁来过，掌柜只盯着门外的水沟。水沟里漂着一根红绳，正是师门封账用的旧结。",
      "",
      "她收起铜片，决定天亮前去内城桥下找第二个见证人。桥下若还有誓契灰烬，师父失踪前的最后一笔账就能重新接上。",
    ].join("\n");

    const generateFoundation = vi.spyOn(ArchitectAgent.prototype, "generateFoundation").mockResolvedValue(
      createLongFormFoundationOutput(),
    );
    const writeChapter = vi.spyOn(WriterAgent.prototype, "writeChapter").mockResolvedValue(
      createWriterOutput({
        ...
        updatedState: createStateCard({
          chapter: 1,
          location: "内城桥下",
          protagonistState: "林月取得湿纸上的师父签名。",
          goal: "寻找第二个见证人并核对誓契灰烬。",
          conflict: "巡丁鞋印证明内城势力已经介入。",
        }),
        ...
        updatedHooks: [
          ...
          "| hook_id | start_chapter | type | status | last_advanced_chapter | target_chapter | description |",
          "| --- | ---: | --- | --- | ---: | ---: | --- |",
          "| mentor-ledger | 0 | mystery | open | 1 | 4 | 账本线索推进到内城桥下见证人。 |",
        ].join("\n"),
        chapterSummary: "| 1 | 雨夜账本 | 林月、掌柜 | 湿纸显出师父签名 | 目标转向内城桥下见证人 | mentor-ledger advanced | tense | investigation |",
        updatedSubplots: "# Subplot Board\n\n| id | status |\n| --- | --- |\n| ledger-trail | active |\n",
        updatedEmotionalArcs: "# Emotional Arcs\n\n| 角色 | 章节 | 情绪状态 | 触发事件 | 强度(1-10) | 弧线方向 |\n| --- | ---: | --- | --- | ---: | --- |\n| 林月 | 1 | 克制紧绷 | 师父签名浮现 | 6 | 上扬 |\n",
        updatedCharacterMatrix: "# Character Matrix\n\n## 林月\n\n- chapter 1: 以证据推进师门账本线。\n",
      }),
    );
    ...
    try {
      ...
      const chapterFiles = (await readdir(chaptersDir)).filter((file) => file.endsWith(".md"));
      const chapterMarkdown = await readFile(join(chaptersDir, chapterFiles.find((file) => file.startsWith("0001_"))!), "utf-8");
      const exported = await buildExportArtifact(state, bookId, { format: "md" });
      ...
      expect(await readFile(join(storyDir, "outline", "story_frame.md"), "utf-8")).toContain("师门债契");
      expect(await readFile(join(storyDir, "roles", "主要角色", "林月.md"), "utf-8")).toContain("Current_State");
      expect(JSON.parse(await readFile(join(storyDir, "canon", "claims.json"), "utf-8"))).toMatchObject({
        claims: expect.any(Array),
      });
      expect(await readFile(join(runtimeDir, "chapter-0001.intent.md"), "utf-8")).toContain("# Chapter Intent");
      expect(JSON.parse(await readFile(join(runtimeDir, "chapter-0001.context.json"), "utf-8"))).toMatchObject({
        chapter: 1,
      });
      expect(await readFile(join(runtimeDir, "chapter-0001.rule-stack.yaml"), "utf-8")).toContain("story_frame");
      expect(chapterMarkdown).toContain("# 第1章 雨夜账本");
      expect(chapterMarkdown).toContain("墨迹慢慢浮出师父的签名");
      expect(await readFile(join(storyDir, "current_state.md"), "utf-8")).toContain("内城桥下");
      expect(await readFile(join(storyDir, "pending_hooks.md"), "utf-8")).toContain("mentor-ledger");
      expect(await readFile(join(storyDir, "chapter_summaries.md"), "utf-8")).toContain("雨夜账本");
      expect(await stat(join(storyDir, "snapshots", "1", "current_state.md"))).toBeTruthy();
      expect(exported).toMatchObject({
        format: "md",
        chaptersExported: 1,
        totalWords: result.wordCount,
      });
      expect(exported.payload.toString()).toContain("# 长篇流程测试");
      expect(exported.payload.toString()).toContain("# 第1章 雨夜账本");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("refuses to accept a deterministically invalid foundation scale after retries", async () => {
    ...
    try {
      ...
      expect(generate.mock.calls[1]?.[0]).toContain("## 硬性阻断项");
      expect(generate.mock.calls[1]?.[0]).toContain("目标5章只能规划1卷");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("does not begin draft persistence when cancellation arrives after generation", async () => {
    const controller = new AbortController();
    const { root, runner, state, bookId } = await createRunnerFixture({
      signal: controller.signal,
    });
    const beginPersistence = vi.spyOn(state, "beginChapterPersistence");
    vi.spyOn(WriterAgent.prototype, "writeChapter").mockImplementation(async () => {
      controller.abort(new DOMException("Operation cancelled by user", "AbortError"));
      return createWriterOutput({
        chapterNumber: 1,
        content: "This generated body must never be persisted.",
        wordCount: "This generated body must never be persisted.".length,
      });
    });

    try {
      await expect(runner.writeDraft(bookId)).rejects.toMatchObject({ name: "AbortError" });
      expect(beginPersistence).not.toHaveBeenCalled();
      await expect(state.loadChapterIndex(bookId)).resolves.toEqual([]);
      await expect(readdir(join(state.bookDir(bookId), "chapters"))).resolves.toEqual([]);
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  }, SLOW_PIPELINE_TEST_TIMEOUT_MS);
  ...
  it("persists per-book llm telemetry and forwards it to the external callback", async () => {
    const forwarded: LLMCallTelemetry[] = [];
    const { root, runner } = await createRunnerFixture({
      onCallTelemetry: (telemetry) => forwarded.push(telemetry),
    });
    const ctx = runner.createAgentContext("writer", "test-book");
    ...
    const telemetryJsonl = await waitForFile(root, ".inkos/runtime/llm-calls/test-book.jsonl");
    const parsed = JSON.parse(telemetryJsonl.trim()) as LLMCallTelemetry;

    expect(forwarded).toHaveLength(1);
    expect(forwarded[0]!.agent).toBe("writer");
  ...
  it("writes partial llm reports alongside telemetry records", async () => {
    const { root, runner } = await createRunnerFixture();
    const ctx = runner.createAgentContext("planner", "test-book");
    ...
    ctx.onCallTelemetry?.({
      ...
      partialContentLength: 12,
      partialContent: "partial plan",
      timestamp: "2026-07-10T01:02:03.000Z",
    });

    await waitForFile(root, ".inkos/runtime/llm-calls/test-book.jsonl");
    const partialFiles = await waitForDirectoryEntries(root, ".inkos/runtime/llm-partials/test-book");
    const partialContent = await readFile(
      join(root, ".inkos", "runtime", "llm-partials", "test-book", partialFiles[0]!),
      "utf-8",
    );

    expect(partialContent).toContain("# LLM Partial Report");
    expect(partialContent).toContain("partial plan");
    expect(partialContent).toContain("planner");
  ...
  it("keeps full-write and standalone audit claim blockers aligned", async () => {
    const { root, runner, state, bookId } = await createRunnerFixture({
      inputGovernanceMode: "v2",
    ...
    vi.spyOn(PlannerAgent.prototype, "planChapter").mockImplementation(async (input) => {
      const runtimeDir = join(input.bookDir, "story", "runtime");
      const runtimePath = join(runtimeDir, `chapter-${String(input.chapterNumber).padStart(4, "0")}.intent.md`);
      const intentMarkdown = "# Chapter Intent\n\n## Goal\nAdvance the next clue without revealing protected canon.\n";
      await mkdir(runtimeDir, { recursive: true });
      await writeFile(runtimePath, intentMarkdown, "utf-8");
      return {
        intent: {
          chapter: input.chapterNumber,
          goal: "Advance the next clue without revealing protected canon.",
          mustKeep: ["Keep protected truths hidden."],
          mustAvoid: ["Do not leak character knowledge."],
          styleEmphasis: [],
        },
        memo: {
          chapter: input.chapterNumber,
          goal: "Advance the next clue.",
          isGoldenOpening: false,
          body: createValidAuditMemoBody(),
          threadRefs: [],
          volumeKrRefs: [],
          volumeKrRationale: "Buffer chapter.",
        },
        intentMarkdown,
        plannerInputs: [runtimePath],
        runtimePath,
    ...
    await Promise.all([
      ...
      writeFile(join(storyDir, "volume_outline.md"), "# Volume Outline\n\n## Chapter 1\nTrack the guild trail.\n", "utf-8"),
      writeFile(join(storyDir, "current_state.md"), "# Current State\n\n- Lin Yue knows nothing about the sect truth.\n", "utf-8"),
    ...
    vi.spyOn(WriterAgent.prototype, "writeChapter").mockResolvedValue(
      createWriterOutput({
        chapterNumber: 1,
        content: "林月停在旧账房门外，忽然确认宗门高层早已知道真相。",
        wordCount: "林月停在旧账房门外，忽然确认宗门高层早已知道真相。".length,
      }),
    );
    const auditChapter = vi.spyOn(ContinuityAuditor.prototype, "auditChapter").mockResolvedValue(
      createAuditResult({
    ...
    try {
      const writeResult = await runner.writeNextChapter(bookId);
      const standaloneResult = await runner.auditDraft(bookId, 1);
      const fullWriteCriticalCategories = writeResult.auditResult.issues
        .filter((issue) => issue.severity === "critical")
        .map((issue) => issue.category)
        .sort();
      const standaloneCriticalCategories = standaloneResult.issues
        .filter((issue) => issue.severity === "critical")
        .map((issue) => issue.category)
        .sort();
      const savedIndex = await state.loadChapterIndex(bookId);

      expect(writeResult.auditResult.passed).toBe(false);
      expect(standaloneResult.passed).toBe(false);
      expect(writeResult.auditResult.issues).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ category: "claim-character-knowledge-leak", severity: "critical" }),
        ]),
      );
      expect(standaloneCriticalCategories).toEqual(fullWriteCriticalCategories);
      ...
      expect(auditChapter.mock.calls[1]?.[4]).toEqual(expect.objectContaining({
        chapterMemo: expect.objectContaining({ chapter: 1 }),
        contextPackage: expect.objectContaining({ chapter: 1 }),
        ruleStack: expect.objectContaining({ layers: expect.any(Array) }),
      }));
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  ...
  it("adds hook-ledger 揭1埋1 violations from the memo to the audit result", async () => {
    ...
    const storyDir = join(state.bookDir(bookId), "story");
    await Promise.all([
      writeFile(join(storyDir, "current_focus.md"), "# Current Focus\n\nClose the token thread.\n", "utf-8"),
      writeFile(join(storyDir, "volume_outline.md"), "# Volume Outline\n\n## Chapter 1\nTrack the guild trail.\n", "utf-8"),
      writeFile(join(storyDir, "current_state.md"), "# Current State\n\n- 林秋 still holds the servant token.\n", "utf-8"),
    ...
    vi.spyOn(PlannerAgent.prototype, "planChapter").mockImplementation(async function (this: PlannerAgent, input) {
      const planned = await originalPlanChapter.call(this, input);
      return {
        ...planned,
        memo: {
          chapter: input.chapterNumber,
    ...
    vi.spyOn(WriterAgent.prototype, "writeChapter").mockResolvedValue(
      createWriterOutput({
        chapterNumber: 1,
        content: "林秋翻看胖虎借条，随后当众摘下杂役腰牌。",
        wordCount: "林秋翻看胖虎借条，随后当众摘下杂役腰牌。".length,
      }),
    );
    vi.spyOn(ContinuityAuditor.prototype, "auditChapter").mockResolvedValue(
      createAuditResult({ passed: true, issues: [], summary: "clean", overallScore: 90 }),
    );
  ...
  it("logs explicit stage messages during writeNextChapter", async () => {
    ...
    try {
      await runner.writeNextChapter(bookId, 20);
  ...
  it("logs English stage messages during writeNextChapter for English books", async () => {
    ...
    try {
      await runner.writeNextChapter(bookId, 3);
  ...
  it("records a length warning when a single normalize pass still misses the hard range", async () => {
    ...
    try {
      ...
      expect(chapterMeta?.status).toBe("audit-failed");
      await expect(state.getNextChapterNumber(bookId)).resolves.toBe(1);
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("keeps the last actionable audit issues when re-audit returns failed with no issues", async () => {
    ...
    try {
      ...
      await expect(readFile(join(storyDir, "current_state.md"), "utf-8")).resolves.toContain(
        "Current Chapter | 0",
      );
      await expect(readFile(join(storyDir, "pending_hooks.md"), "utf-8")).resolves.toBe("# Pending Hooks\n");
      await expect(state.getNextChapterNumber(bookId)).resolves.toBe(1);
  ...
  it("persists truth files derived from the final revised chapter", async () => {
    ...
    await runner.writeNextChapter(bookId, "Final revised body.".length);
  ...
  it("persists structured runtime state and rendered projections from writer delta output", async () => {
    ...
    const result = await runner.writeNextChapter(
      bookId,
      countChapterLength("Lin Yue follows the debt into the river-port ledger.", "en_words"),
    );
  ...
  it("repairs chapter-number drift in writer delta before persisting runtime state", async () => {
    ...
    vi.spyOn(WriterAgent.prototype, "writeChapter").mockResolvedValue(
      createWriterOutput({
        ...
        runtimeStateDelta: {
          chapter: 0,
          currentStatePatch: {
            currentGoal: "Recover the broken chapter body.",
          },
          hookOps: {
            upsert: [],
            mention: [],
          ...
          chapterSummary: {
            ...
            characters: "Lin Yue",
            events: "Broken chapter body.",
            stateChanges: "Goal is updated.",
            hookActivity: "none",
            mood: "quiet",
            chapterType: "test",
          },
    ...
    const result = await runner.writeNextChapter(bookId, 3);
  ...
  it("rolls back persisted runtime state when writer delta contains natural-language numeric drift", async () => {
    ...
    await expect(runner.writeNextChapter(bookId, 3)).rejects.toThrow();
  ...
  it("degrades to state-degraded when state validation errors instead of aborting", async () => {
    ...
    const result = await runner.writeNextChapter(
      bookId,
      "Healthy chapter body with the copper token in his coat.".length,
    );
  ...
  it("replays state through the chapter analyzer without a second settlement call", async () => {
    const diagnostics: Array<{ kind: string; chapterNumber?: number; details?: Record<string, unknown> }> = [];
    const { root, runner, state, bookId } = await createRunnerFixture({
      inputGovernanceMode: "legacy",
    ...
    const settleChapterState = vi.spyOn(
      WriterAgent.prototype as unknown as {
        settleChapterState: (input: Record<string, unknown>) => Promise<WriteChapterOutput>;
      },
      "settleChapterState",
    ...
    const analyzeChapter = vi.spyOn(ChapterAnalyzerAgent.prototype, "analyzeChapter").mockResolvedValue(
      createAnalyzedOutput({
        ...
        title: "Test Chapter",
        content: "Healthy chapter body with the copper token in his coat.",
        updatedState: "analyzer rebuilt state",
        updatedHooks: [
          "| hook_id | start_chapter | type | status | last_advanced | expected_payoff | notes |",
          "| --- | --- | --- | --- | --- | --- | --- |",
          "| H027 (Old Li's note) | 1 | clue | open | 1 | 2 | Old Li's note remains actionable |",
        ].join("\n"),
        chapterSummary: "| 1 | Test Chapter | Lin Yue | Copper token retained | State rebuilt | none | tense | investigation |",
      }),
    );
    vi.spyOn(StateValidatorAgent.prototype, "validate")
      .mockResolvedValueOnce({
    ...
    expect(diagnostics).toContainEqual(expect.objectContaining({
      ...
      chapterNumber: 1,
      details: { settlementAttempts: 1 },
    }));
    await expect(readFile(join(storyDir, "current_state.md"), "utf-8")).resolves.toBe("analyzer rebuilt state");
    await expect(readFile(join(storyDir, "pending_hooks.md"), "utf-8")).resolves.toContain(
      "| H027 | 1 | clue | open | 1 | 2 | Old Li's note remains actionable |",
    );
    const structuredHooks = JSON.parse(
      await readFile(join(storyDir, "state", "hooks.json"), "utf-8"),
    ) as { hooks: Array<{ hookId: string; notes: string }> };
    expect(structuredHooks.hooks).toContainEqual(expect.objectContaining({
      hookId: "H027",
      notes: "Old Li's note remains actionable",
    }));
  ...
  it("blocks writing a new chapter when the latest persisted chapter is audit-failed", async () => {
    ...
    await Promise.all([
      ...
      writeFile(
        join(chaptersDir, "0001_Failed_Audit.md"),
        "# 第1章 Failed Audit\n\nbody",
        "utf-8",
      ),
    ]);
  ...
  it("keeps an audit-passed chapter blocked when its durable snapshot is missing", async () => {
    const { root, runner, state, bookId } = await createRunnerFixture({
      inputGovernanceMode: "legacy",
    });
    const now = "2026-03-19T00:00:00.000Z";
    ...
    expect(result.issues).toEqual([
      expect.objectContaining({ category: "state-sync-required", severity: "warning" }),
    ]);
    expect(savedIndex[0]?.status).toBe("state-degraded");
  ...
  it("repairs the latest state-degraded chapter from persisted body without rewriting it", async () => {
    ...
    const validationSpy = vi.spyOn(StateValidatorAgent.prototype, "validate").mockResolvedValue({
      passed: true,
      warnings: [],
    });
    ...
    const savedIndex = await state.loadChapterIndex(bookId);
    const savedBook = await state.loadBookConfig(bookId);
    ...
    expect(savedBook.status).toBe("active");

    await rm(root, { recursive: true, force: true });
  });

  it("returns a degraded repair result so unattended recovery can continue to resync", async () => {
    const { root, runner, state, bookId } = await createRunnerFixture({
      inputGovernanceMode: "legacy",
    });
    const now = "2026-03-19T00:00:00.000Z";
    ...
    const settleSpy = vi.spyOn(
      ...
      createWriterOutput({
        ...
        content: "Healthy chapter body with the copper token in his coat.",
        wordCount: 55,
        updatedState: "empty replacement state",
        updatedHooks: "empty replacement hooks",
        updatedLedger: "empty replacement ledger",
      }),
    );
    vi.spyOn(StateValidatorAgent.prototype, "validate")
      .mockResolvedValueOnce({
        ...
        warnings: [{
          category: "missing_state_change",
          description: "The repair removed the current chapter state.",
        }],
      })
      .mockResolvedValueOnce({
        passed: false,
        warnings: [{
          category: "missing_state_change",
          description: "The settlement retry still removed the current chapter state.",
        }],
      });
  ...
  sqliteIt("normalizes snapshot-only repair output before rebuilding narrative memory", async () => {
    ...
    await Promise.all([
      ...
      writeFile(join(storyDir, "particle_ledger.md"), "stable ledger", "utf-8"),
      writeFile(
        join(bookDir, "chapters", "0001_Snapshot_Only.md"),
        "# 第1章 Snapshot Only\n\nLin Yue finds the mentor ledger beside the ferry seal.",
        "utf-8",
      ),
      state.saveChapterIndex(bookId, [{
        number: 1,
        ...
        createdAt: now,
        updatedAt: now,
        auditIssues: ["[warning] structured state was incomplete"],
        lengthWarnings: [],
        reviewNote: JSON.stringify({
          kind: "state-degraded",
          baseStatus: "ready-for-review",
          injectedIssues: ["[warning] structured state was incomplete"],
        }),
      }]),
    ]);
    ...
    vi.spyOn(
      ...
      chapterNumber: 1,
      title: "Snapshot Only",
      content: "Lin Yue finds the mentor ledger beside the ferry seal.",
      updatedState: createStateCard({
        chapter: 1,
        location: "Ashen ferry crossing",
        protagonistState: "Lin Yue carries the recovered mentor ledger.",
        goal: "Trace the ledger seal.",
        conflict: "Guild watchers are closing the ferry gate.",
      }),
      updatedHooks: [
        "# Pending Hooks",
        "",
        "| hook_id | start_chapter | type | status | last_advanced | expected_payoff | notes |",
        "| --- | --- | --- | --- | --- | --- | --- |",
        "| mentor-ledger | 1 | mystery | open | 1 | Reveal who altered the seal. | Ledger recovered at the ferry. |",
        "",
      ].join("\n"),
      chapterSummary: "| 1 | Snapshot Only | Lin Yue | The mentor ledger is recovered. | The seal trail opens. | mentor-ledger advanced | tense | investigation |",
      runtimeStateSnapshot: {
        ...
        hooks: {
          hooks: [{
            hookId: "mentor-ledger",
            startChapter: 1,
            type: "mystery",
            status: "open",
            lastAdvancedChapter: 1,
            expectedPayoff: "Reveal who altered the seal.",
            notes: "Snapshot is intentionally ahead of its manifest.",
          }],
        },
    ...
    expect(capture.warnings.join("\n")).not.toContain("Narrative memory sync skipped");

    const memory = new MemoryDB(bookDir);
    try {
      expect(memory.getSummaries(1, 1)).toEqual([
        expect.objectContaining({ chapter: 1, title: "Snapshot Only" }),
      ]);
      expect(memory.getActiveHooks()).toEqual([
        expect.objectContaining({ hookId: "mentor-ledger", lastAdvancedChapter: 1 }),
      ]);
    } finally {
      memory.close();
    }
  ...
  it.each([
    ...
    await Promise.all([
      writeFile(join(storyDir, "current_state.md"), "baseline state", "utf-8"),
      writeFile(join(storyDir, "pending_hooks.md"), "baseline hooks", "utf-8"),
      writeFile(join(storyDir, "particle_ledger.md"), "baseline ledger", "utf-8"),
    ]);
    await state.snapshotState(bookId, 0);
    await mkdir(join(storyDir, "state"), { recursive: true });
    ...
    await Promise.all([
      ...
      writeFile(join(storyDir, "state", "hooks.json"), JSON.stringify({ hooks: [] }), "utf-8"),
      writeFile(join(storyDir, "state", "chapter_summaries.json"), JSON.stringify({ rows: [] }), "utf-8"),
  ...
  it.each([
    { maxSettlementCalls: undefined, expectedSettlementCalls: 2, expectedValidationCalls: 2 },
    { maxSettlementCalls: 1, expectedSettlementCalls: 1, expectedValidationCalls: 1 },
  ])("rebuilds resync truth after $expectedSettlementCalls incomplete settlement attempt(s)", async ({
    maxSettlementCalls,
    expectedSettlementCalls,
    ...
    const { root, runner, state, bookId } = await createRunnerFixture({
      inputGovernanceMode: "legacy",
      onPipelineDiagnostic: (diagnostic) => diagnostics.push(diagnostic),
      ...(maxSettlementCalls === undefined
        ? {}
        : { governanceCallLimits: { maxSettlementCallsPerChapter: maxSettlementCalls } }),
    });
    const now = "2026-03-19T00:00:00.000Z";
    const bookDir = state.bookDir(bookId);
    const storyDir = join(bookDir, "story");
    ...
    await state.snapshotState(bookId, 0);
    await Promise.all([
      writeFile(join(storyDir, "current_state.md"), "live chapter state", "utf-8"),
      writeFile(join(storyDir, "pending_hooks.md"), "live chapter hooks", "utf-8"),
      writeFile(join(bookDir, "chapters", "0001_Empty.md"), "# 第1章 Empty\n\n正文发生了明确变化。", "utf-8"),
      state.saveChapterIndex(bookId, [{
        number: 1,
        title: "Empty",
    ...
    const settleChapterState = vi.spyOn(
      ...
      title: "Empty",
      content: "正文发生了明确变化。",
      updatedState: "baseline state",
      updatedHooks: "baseline hooks",
      chapterSummary: "",
    }));
    const analyzeChapter = vi.spyOn(ChapterAnalyzerAgent.prototype, "analyzeChapter").mockResolvedValue(
      createAnalyzedOutput({
        ...
        content: "正文发生了明确变化。",
        updatedState: "analyzer rebuilt state",
        updatedHooks: "analyzer rebuilt hooks",
        chapterSummary: "| 1 | Empty | 林越 | 状态发生变化 | 状态已重建 | 无 | 紧张 | 调查 |",
      }),
    );
    const validate = vi.spyOn(StateValidatorAgent.prototype, "validate").mockResolvedValue({
      passed: true,
      warnings: [],
    });
    ...
    expect(result.status).toBe("ready-for-review");
    expect(settleChapterState).toHaveBeenCalledTimes(expectedSettlementCalls);
    expect(analyzeChapter).toHaveBeenCalledTimes(1);
    expect(validate).toHaveBeenCalledTimes(expectedValidationCalls);
    expect(diagnostics).toContainEqual(expect.objectContaining({
      kind: "resync-analyzer-fallback",
      chapterNumber: 1,
      details: { settlementAttempts: expectedSettlementCalls },
    }));
    await expect(readFile(join(storyDir, "current_state.md"), "utf-8")).resolves.toBe("analyzer rebuilt state");
    await expect(readFile(join(storyDir, "pending_hooks.md"), "utf-8")).resolves.toBe("analyzer rebuilt hooks");
    expect(savedIndex[0]?.status).toBe("ready-for-review");

    await rm(root, { recursive: true, force: true });
  });

  it("restores live truth when settlements and analyzer fallback are all incomplete", async () => {
    const { root, runner, state, bookId } = await createRunnerFixture({
      inputGovernanceMode: "legacy",
    });
    const now = "2026-03-19T00:00:00.000Z";
    ...
    await expect(runner.resyncChapterArtifacts(bookId, 1)).rejects.toThrow(/resync|状态结算|settlement/i);
    await expect(readFile(join(storyDir, "current_state.md"), "utf-8")).resolves.toBe("live chapter state");
    await expect(readFile(join(storyDir, "pending_hooks.md"), "utf-8")).resolves.toBe("live chapter hooks");
    expect((await state.loadChapterIndex(bookId))[0]?.status).toBe("ready-for-review");

    await rm(root, { recursive: true, force: true });
  });

  it("accepts an explicit unchanged state when resync still returns hooks and a chapter summary", async () => {
    const { root, runner } = await createRunnerFixture({
      inputGovernanceMode: "legacy",
    });
    ...
    const validation = (
      runner as unknown as {
        validateResyncSettlementCompleteness: (
          output: WriteChapterOutput,
          language: "zh" | "en",
        ) => { passed: boolean } | null;
      }
    ).validateResyncSettlementCompleteness(createWriterOutput({
      updatedState: "baseline state",
      updatedHooks: "baseline hooks",
      chapterSummary: "| 1 | Empty | 林越 | 本章无状态变化 | 无 | 无 | 平静 | 过渡 |",
    }), "zh");
  ...
  it("records visible volume KR refs from the final written chapter and refreshes dashboards", async () => {
    const { root, runner, state, bookId } = await createRunnerFixture({
      inputGovernanceMode: "v2",
      writingReviewRetries: 0,
    });
    ...
    const runtimeDir = join(storyDir, "runtime");
    const finalBody = [
      "Mara recovered the sealed invoice in front of the dock clerk.",
      "The recovery turned the harbor rumor into evidence.",
    ].join(" ");

    await mkdir(join(storyDir, "outline"), { recursive: true });
    await Promise.all([
      writeFile(join(storyDir, "current_focus.md"), "# Current Focus\n\nRecover the sealed invoice.\n", "utf-8"),
      writeFile(join(storyDir, "outline", "story_frame.md"), "# Story Frame\n\nThe harbor ledger cannot be forged.\n", "utf-8"),
      ...
      writeFile(join(storyDir, "current_state.md"), "# Current State\n\nMara is still outside the archive.\n", "utf-8"),
      writeFile(join(storyDir, "story_bible.md"), "# Story Bible\n\nThe sealed invoice is admissible evidence.\n", "utf-8"),
      writeFile(join(storyDir, "pending_hooks.md"), "# Pending Hooks\n", "utf-8"),
      writeFile(join(storyDir, "chapter_summaries.md"), "# Chapter Summaries\n", "utf-8"),
    ]);
    ...
    vi.spyOn(PlannerAgent.prototype, "planChapter").mockImplementation(async (input) => {
      ...
      return {
        intent: {
          ...
          outlineNode: "Recover the sealed invoice.",
          mustKeep: [],
          mustAvoid: [],
          styleEmphasis: [],
        },
    ...
    try {
      await runner.writeNextChapter(bookId, 120);

      const progress = JSON.parse(await readFile(join(runtimeDir, "volume-progress.json"), "utf-8")) as {
        entries: Array<{ chapter: number; krRefs: string[]; visibleKrRefs: string[]; attemptedKrRefs: string[] }>;
      };
      ...
      await expect(readFile(join(runtimeDir, "volume-dashboard.md"), "utf-8")).resolves.toContain(
        "| V1-KR1 | advanced | ch1 | ch1 | - | Recover the sealed invoice. |",
      );
      await expect(readFile(join(runtimeDir, "volume-001.dashboard.md"), "utf-8")).resolves.toContain(
        "| V1-KR1 | advanced | ch1 | ch1 | - | Recover the sealed invoice. |",
      );
    } finally {
      await rm(root, { recursive: true, force: true });
  ...
  it("adds post-write volume gate issues to the audit result when a bound KR is not visibly advanced", async () => {
    // End-to-end companion to volume-contract.test.ts (unit) and the sibling
    // claim-gate / hook-ledger e2e tests: prove a memo that binds a volume KR
    // but whose prose never advances it surfaces the volume-kr-not-visible
    // warning through runPostWriteChecks into the audit result.
    const { root, runner, state, bookId } = await createRunnerFixture({
      inputGovernanceMode: "v2",
      writingReviewRetries: 0,
    });
    const storyDir = join(state.bookDir(bookId), "story");
    const runtimeDir = join(storyDir, "runtime");
    // Prose deliberately avoids every KR keyword (invoice / sealed / recover /
    // clerk / guild) so neither visible nor attempted KR refs are detected.
    const finalBody = "Mara wandered the empty pier at dusk, thinking about nothing in particular.";
    ...
    try {
      ...
      expect(result.auditResult.issues).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ category: "volume-kr-not-visible", severity: "warning" }),
        ]),
      );
    } finally {
      await rm(root, { recursive: true, force: true });
  ...
  async function createRevisionGateFixture(revisionGate?: "strict" | "lenient" | "always") {
    ...
    return { ...fixture, storyDir, chaptersDir, revisedBody };
  }
  ...
  it("always applies manual revisions when revisionGate is always, even when the audit worsens", async () => {
    const { root, runner, state, bookId, storyDir, chaptersDir, revisedBody } = await createRevisionGateFixture("always");
    const truthBefore = await readFile(join(storyDir, "current_state.md"), "utf-8");
    ...
    try {
      ...
      expect((await readFile(join(storyDir, "current_state.md"), "utf-8")).trim()).toBe(truthBefore.trim());
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  }, SLOW_PIPELINE_TEST_TIMEOUT_MS);

  it("keeps truth unchanged when a telemetry-backed manual revision misses the hard length range", async () => {
    const { root, runner, state, bookId } = await createRunnerFixture({ revisionGate: "always" });
    const storyDir = join(state.bookDir(bookId), "story");
    const chaptersDir = join(state.bookDir(bookId), "chapters");
    const originalBody = "原稿保留了稳定状态。".repeat(12);
    const revisedBody = "过长修订。".repeat(120);
    const originalState = createStateCard({
      chapter: 1,
      location: "旧码头",
      protagonistState: "林越仍持有旧钥匙。",
      goal: "找到失踪导师。",
      conflict: "旧债尚未解决。",
    });
    await Promise.all([
    ...
    vi.spyOn(ReviserAgent.prototype, "reviseChapter").mockResolvedValue(createReviseOutput({
      ...
      updatedState: createStateCard({
        chapter: 1,
        location: "错误的新地点",
        protagonistState: "错误状态",
        goal: "错误目标",
        conflict: "错误冲突",
      }),
      updatedHooks: "# Pending Hooks\n- incorrect hook",
    }));
    vi.mocked(LengthNormalizerAgent.prototype.normalizeChapter).mockImplementation(
      async ({ chapterContent, lengthSpec }) => ({
        normalizedContent: chapterContent,
        finalCount: countChapterLength(chapterContent, lengthSpec.countingMode),
        applied: false,
    ...
    try {
      ...
      expect(result.applied).toBe(true);
      expect(result.status).toBe("audit-failed");
      expect(result.lengthWarnings?.length).toBeGreaterThan(0);
      expect(savedIndex[0]?.status).toBe("audit-failed");
      expect((await readFile(join(storyDir, "current_state.md"), "utf-8")).trim()).toBe(originalState.trim());
      await expect(readFile(join(storyDir, "pending_hooks.md"), "utf-8")).resolves.toBe("# Pending Hooks\n");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  ...
  it("re-audits revisions against updated state overrides instead of stale on-disk truth files", async () => {
    ...
    await Promise.all([
      ...
      writeFile(join(storyDir, "chapter_summaries.md"), [
        ...
        "| Chapter | Title | Characters | Key Events | State Changes | Hook Activity | Mood | Chapter Type |",
        "| --- | --- | --- | --- | --- | --- | --- | --- |",
        "| 1 | First | Taryn | Taryn reaches the archive lower hall. | The archive is compromised. | none | tense | investigation |",
        "",
      ].join("\n"), "utf-8"),
    ]);
    await state.saveChapterIndex(bookId, [
    ...
    try {
      ...
      const savedIndex = await state.loadChapterIndex(bookId);
      const savedSummaries = await readFile(join(storyDir, "chapter_summaries.md"), "utf-8");
      const runtimeState = await loadRuntimeStateSnapshot(state.bookDir(bookId));
      ...
      expect(savedSummaries).toContain("| 2 | Test Chapter |");
      expect(savedSummaries).toContain("He checked the seal again before he moved.");
      expect(runtimeState.manifest.lastAppliedChapter).toBe(2);
      expect(runtimeState.currentState.chapter).toBe(2);
      expect(runtimeState.chapterSummaries.rows.map((row) => row.chapter)).toEqual([1, 2]);
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  }, SLOW_PIPELINE_TEST_TIMEOUT_MS);
