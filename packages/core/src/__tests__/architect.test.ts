...
describe("ArchitectAgent", () => {
  ...
  it("uses English prompts when generating foundation from imported English episodes", async () => {
    ...
    expect(messages[0]?.content).not.toContain("3-5 episodes per KR");
    expect(messages[0]?.content).not.toContain("every 5 episodes");
    expect(messages[0]?.content).not.toContain("last episode roughly feels");
  ...
  it("keeps episode-zero seed hooks dormant even when the model labels them open", async () => {
    const agent = new ArchitectAgent({
      client: {
        provider: "openai",
        apiFormat: "chat",
