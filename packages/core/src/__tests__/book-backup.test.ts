...
describe("book backup and restore", () => {
  ...
  it("publishes validated backups atomically and omits transient lock files", async () => {
    ...
    await writeFile(join(bookDir, ".episode-persistence.json"), "transient", "utf-8");
    ...
    await expect(readFile(join(backupDir, "episodes", "0001_起风.md"), "utf-8"))
      .resolves.toBe("第一章原文。");
    await expect(access(join(backupDir, ".write.lock"))).rejects.toThrow();
    await expect(access(join(backupDir, ".episode-persistence.json"))).rejects.toThrow();
  ...
  it("restores the whole book and preserves the replaced state in a pre-restore backup", async () => {
    ...
    await writeFile(join(bookDir, "episodes", "0001_起风.md"), "改坏了的第一章。", "utf-8");
    await writeFile(join(bookDir, "episodes", "0002_多余.md"), "多写的一章。", "utf-8");
    ...
    await expect(readFile(join(bookDir, "episodes", "0001_起风.md"), "utf-8"))
      .resolves.toBe("第一章原文。");
    await expect(access(join(bookDir, "episodes", "0002_多余.md"))).rejects.toThrow();
    const preRestoreDir = join(bookBackupsDir(state, "harbor"), "20260715-090000-pre-restore");
    await expect(readFile(join(preRestoreDir, "episodes", "0001_起风.md"), "utf-8"))
      .resolves.toBe("改坏了的第一章。");
  });

  it("rolls an interrupted restore back automatically when the next book lock is acquired", async () => {
    const bookDir = await setupBook(projectRoot, "harbor");
    await writeFile(join(bookDir, "episodes", "0001_起风.md"), "恢复前内容。", "utf-8");
    ...
    await writeFile(join(bookDir, "episodes", "0001_起风.md"), "只恢复了一半。", "utf-8");
  ...
  it("keeps a committed restore and only removes its leftover transaction", async () => {
    const bookDir = await setupBook(projectRoot, "harbor");
    await writeFile(join(bookDir, "episodes", "0001_起风.md"), "已经恢复的新内容。", "utf-8");
    const restoreDir = join(projectRoot, ".inkos", "book-restore", "harbor");
    await mkdir(restoreDir, { recursive: true });
    await writeFile(join(restoreDir, "transaction.json"), JSON.stringify({
      version: 1,
