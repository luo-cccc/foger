...
async function writeEpisode(bookDir: string, episode: number, title: string, body: string): Promise<void> {
  const filename = `${String(episode).padStart(4, "0")}_${title}.md`;
  await writeFile(
    join(bookDir, "episodes", filename),
    `# 第${episode}章 ${title}\n\n${body}\n`,
    "utf-8",
  );
}
...
describe("analyzeLongSpanFatigue", () => {
  it("warns when the last three episode types are identical", async () => {
    const bookDir = await createBookDir("inkos-long-span-type-test-");

    await Promise.all([
      writeEpisode(bookDir, 1, "铺陈", "城门口下着雨。林越压低斗笠，慢慢走进旧巷。风从墙缝里钻出来。"),
      writeEpisode(bookDir, 2, "潜伏", "午后的石街很亮。林越在茶棚外停了一下，随后绕向后院。铜铃轻轻响了一声。"),
      writeFile(
        join(bookDir, "story", "episode_summaries.md"),
        [
          "# 章节摘要",
          "",
    ...
    try {
      const result = await analyzeLongSpanFatigue({
        bookDir,
        episodeNumber: 3,
        episodeContent: "夜色像潮水一样漫到院墙根。林越没有立刻翻墙，而是先贴着墙根听了一阵。最后，他把手按在那道旧债印上。",
        episodeSummary: "| 3 | 试探 | 林越 | 继续潜伏 | 目标未变 | 债印未解 | 克制 | 布局 |",
        language: "zh",
      });
  ...
  it("warns in English when recent episode endings are highly similar", async () => {
    const bookDir = await createBookDir("inkos-long-span-ending-test-");

    await Promise.all([
      writeEpisode(bookDir, 1, "Debt", "The rain had finally stopped. The harbor lights thinned behind him. He knew the debt had only grown heavier."),
      writeEpisode(bookDir, 2, "Weight", "Morning fog crawled over the quay. No one called his name. He knew the debt had only grown heavier tonight."),
    ]);
    ...
    try {
      const result = await analyzeLongSpanFatigue({
        bookDir,
        episodeNumber: 3,
        episodeContent: "The alley was empty by the time he turned back. Even the dogs had gone quiet. He knew the debt had only grown heavier again.",
        language: "en",
      });

      expect(result.issues.some((issue) => issue.category === "Ending Pattern Repetition")).toBe(true);
      expect(result.issues.some((issue) => issue.description.includes("last 3 episode endings"))).toBe(true);
    } finally {
      await rm(join(bookDir, ".."), { recursive: true, force: true });
    }
  });
  ...
  it("builds an English variance brief with phrase, opening, ending, and scene guidance", async () => {
    ...
    await Promise.all([
      writeEpisode(bookDir, 1, "Ledger", "Mara kept the ledger close to her chest. The corridor stayed quiet after the bell. There it was again."),
      writeEpisode(bookDir, 2, "Ash", "Mara kept the ledger close to her chest while the ash fell. The corridor stayed quiet until Taryn stopped. There it was again."),
      writeEpisode(bookDir, 3, "Harbor", "Mara kept the ledger close to her chest near the harbor gate. The corridor stayed quiet while the guards changed. There it was again."),
      writeFile(
        join(bookDir, "story", "episode_summaries.md"),
        [
          "# Episode Summaries",
  ...
  it("warns when title focus collapses and high-tension mood never releases", async () => {
    ...
    await Promise.all([
      writeEpisode(bookDir, 1, "名单之前", "风贴着走廊吹。周谨川没有停，手指一直压着那份发潮的薄册。"),
      writeEpisode(bookDir, 2, "名单之后", "楼道里只有脚步声。周谨川顺着裂灯往下走，肩背始终绷着。"),
      writeFile(
        join(bookDir, "story", "episode_summaries.md"),
        [
          "# 章节摘要",
    ...
    try {
      const result = await analyzeLongSpanFatigue({
        bookDir,
        episodeNumber: 3,
        episodeContent: "墙角的灰一直没落定。周谨川盯着名单最后一行，喉结很轻地滚了一下，还是没有把气松出来。",
        episodeSummary: "| 3 | 名单未落 | 周谨川 | 名单追查继续推进 | 目标未变 | 名单线继续发酵 | 压迫、窒息 | 调查章 |",
        language: "zh",
      });
