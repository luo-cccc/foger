...
import { renderBookRulesMarkdown as readPlannerBookRules } from "../agents/planner-context.js";
...
describe("Phase 5 cleanup (1) — volume_outline.md mirror removed", () => {
  ...
  it("isCompleteBookDirectory accepts the new outline/ layout (no legacy mirror needed)", async () => {
    ...
    try {
      ...
      await mkdir(join(targetBookDir, "episodes"), { recursive: true });

      await Promise.all([
        writeFile(join(targetBookDir, "book.json"), JSON.stringify(baseBook()), "utf-8"),
        ...
        writeFile(join(targetBookDir, "episodes/index.json"), "[]", "utf-8"),
      ]);
  ...
  it("isCompleteBookDirectory rejects the pre-cleanup legacy layout", async () => {
    ...
    try {
      ...
      await Promise.all([
        writeFile(join(targetBookDir, "book.json"), JSON.stringify({ ...baseBook(), id: "legacy-book" }), "utf-8"),
      ...
      const state = new StateManager(projectRoot);
      await expect(state.isCompleteBookDirectory(targetBookDir)).resolves.toBe(false);
    } finally {
      await rm(projectRoot, { recursive: true, force: true });
    }
  });
...
describe("Phase 5 cleanup (3) — book_rules is authoritative Markdown", () => {
  ...
  it("readBookRules() extracts numerical and era constraints from markdown rules", async () => {
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
    await writeFile(
      join(storyDir, "book_rules.md"),
