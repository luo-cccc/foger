...
//      roles/*.Current_State + pending_hooks startEpisode=0 rows when the
...
describe("Phase 5 consolidation — 7→5 sections, prompt contract", () => {
  ...
  it("the prompt forbids duplication across sections (dedup rule)", async () => {
    ...
    expect(system).toContain("环境/时代锚");
  ...
  it("the premise-device contract requirement exists in both the zh and en foundation prompts (P2-4)", async () => {
    const agent = buildAgent();
    const chat = vi.spyOn(agent as unknown as { chat: (...args: unknown[]) => Promise<unknown> }, "chat")
      .mockResolvedValue({ content: CONSOLIDATED_RESPONSE, usage: ZERO_USAGE });
    ...
    await agent.generateFoundation(baseBook());
    const zhSystem = (chat.mock.calls[0]?.[0] as Array<{ content: string }>)[0]?.content ?? "";
    expect(zhSystem).toContain("前提装置");
    expect(zhSystem).toContain("装置契约");
    expect(zhSystem).toContain("能力范围、失效条件、可见代价、规则可靠性");

    chat.mockClear();
    await agent.generateFoundation({ ...baseBook(), language: "en" });
    const enSystem = (chat.mock.calls[0]?.[0] as Array<{ content: string }>)[0]?.content ?? "";
    expect(enSystem).toContain("premise device");
    expect(enSystem).toContain("device contract");
    expect(enSystem).toContain("capability range, failure conditions, visible cost, and rule reliability");
    expect(enSystem).not.toContain("前提装置");
  });
});
...
describe("Phase 5 consolidation — readCurrentStateWithFallback derives initial state", () => {
  ...
  it("returns a derived block built from roles/*.Current_State + seed hooks when current_state.md is a seed placeholder", async () => {
    ...
    // Seed hook row startEpisode=0 surfaces in the derived block.
