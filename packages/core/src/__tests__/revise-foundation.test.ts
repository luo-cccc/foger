...
describe("architect generateFoundation with reviseFrom option", () => {
  ...
  it("injects legacy content into the system prompt when reviseFrom is supplied", async () => {
    ...
    expect(systemMsg.content).toContain("既有架构稿修订模式");
    expect(systemMsg.content).toContain("一部已有漫剧");
...
describe("reviseFoundation re-extracts structured canon", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it("re-runs canon extraction after a foundation revise so story/canon reflects the new prose", async () => {
    const { mkdtemp, writeFile, mkdir, rm, readFile } = await import("node:fs/promises");
    const { tmpdir } = await import("node:os");
    const { join } = await import("node:path");
    const { PipelineRunner } = await import("../pipeline/runner.js");
    ...
    const root = await mkdtemp(join(tmpdir(), "inkos-revise-canon-"));
    const bookDir = join(root, "books", "canon-book");
    ...
    try {
      ...
      await writeFile(join(bookDir, "story", "book_rules.md"), "## 规则", "utf-8");
      await writeFile(join(bookDir, "story", "character_matrix.md"), "## 角色", "utf-8");
      // 一份过时的 canon —— revise 后必须被覆盖，不能残留
      await mkdir(join(bookDir, "story", "canon"), { recursive: true });
      await writeFile(
        join(bookDir, "story", "canon", "claims.json"),
        JSON.stringify({ claims: [{
          id: "STALE-1", domain: "world", claimType: "objective_rule",
          content: "过时设定", scope: { appliesTo: [] },
          authority: { source: "旧稿", priority: "hard" }, visibility: {},
        }] }),
        "utf-8",
      );
      await writeFile(join(bookDir, "book.json"), JSON.stringify({
        id: "canon-book", title: "书", platform: "qidian", genre: "xuanhuan",
        status: "active", schemaVersion: "inkos-episode-v2" as const, format: "screenplay" as const, targetEpisodes: 50, episodeDurationSeconds: 90, language: "zh",
        createdAt: "2026-01-01T00:00:00.000Z", updatedAt: "2026-04-01T00:00:00.000Z",
      }), "utf-8");
      ...
      const freshCanon: ExtractedCanon = {
        claims: [{
          id: "FRESH-1", domain: "world", claimType: "objective_rule",
          content: "修订后的新世界铁律", scope: { appliesTo: [] },
          status: "active",
      ...
      const extractSpy = vi.spyOn(CanonExtractor.prototype, "extract").mockResolvedValue(freshCanon);
      ...
      await runner.reviseFoundation("canon-book", "换个世界观");

      expect(extractSpy).toHaveBeenCalledTimes(1);
      const claims = JSON.parse(await readFile(join(bookDir, "story", "canon", "claims.json"), "utf-8"));
      expect(claims.claims.map((c: { id: string }) => c.id)).toEqual(["FRESH-1"]);
      expect(JSON.stringify(claims)).not.toContain("STALE-1");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("rolls back the foundation and canon when canon extraction fails", async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), "inkos-revise-canon-fail-"));
    const bookDir = join(root, "books", "canon-fail-book");

    try {
      await mkdir(join(bookDir, "story"), { recursive: true });
      await writeFile(join(bookDir, "story", "story_bible.md"), "# 架构稿", "utf-8");
      await writeFile(join(bookDir, "story", "volume_outline.md"), "## 卷一", "utf-8");
      await writeFile(join(bookDir, "story", "book_rules.md"), "## 规则", "utf-8");
      await writeFile(join(bookDir, "story", "character_matrix.md"), "## 角色", "utf-8");
      await mkdir(join(bookDir, "story", "canon"), { recursive: true });
