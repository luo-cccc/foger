...
describe("volume contract", () => {
  it("extracts Objective, KRs, irreversible event, and episode range from volume_map", () => {
  ...
  it("extracts volume-level supply fields for stage, world, relationship, and hook governance", () => {
    const contract = extractVolumeContracts([
      "## Volume 1 Harbor Ledger (Episodes 1-20)",
  ...
  it("accepts a buffer rationale when no KR is bound", () => {
    ...
    const memo: EpisodeMemo = {
      ...
      threadRefs: [],
      volumeKrRefs: [],
      volumeKrRationale: "buffer episode that stages the dock route for the next invoice recovery",
    };
  ...
  it("warns when a 3-5 episode mini-cycle has no KR advancement", () => {
    const contract = extractVolumeContracts([
      "## Volume 1 Harbor Ledger (Episodes 1-20)",
      "Objective: Pin the harbor ledger trail to a named guild.",
      "KR1: Recover the sealed invoice.",
    ...
    const memo: EpisodeMemo = {
      ...
      threadRefs: [],
      volumeKrRefs: [],
      volumeKrRationale: "buffer episode that delays the invoice recovery",
    };
  ...
  it("blocks volume end when KRs or the irreversible event are missing", () => {
    const contract = extractVolumeContracts([
      "## Volume 1 Harbor Ledger (Episodes 1-5)",
      "Objective: Pin the harbor ledger trail to a named guild.",
      "KR1: Recover the sealed invoice.",
      "KR2: Force the guild clerk to make a visible mistake.",
      "Irreversible Event: Mara burns her safe identity at the dock gate.",
  ...
  it("does not re-fire the volume-end gate on episodes past the planned end", () => {
    // Volume planned as episodes 1-5. When writing overruns (episode 6+) and the
    // contract is reused for out-of-range episodes, the volume-end critical suite
    // must NOT fire again — it belongs to the last episode only.
    const contract = extractVolumeContracts([
      "## Volume 1 Harbor Ledger (Episodes 1-5)",
      "Objective: Pin the harbor ledger trail to a named guild.",
      "KR1: Recover the sealed invoice.",
    ...
    const memo: EpisodeMemo = {
      ...
      threadRefs: [],
      volumeKrRefs: [],
      volumeKrRationale: "cool-down episode after the volume already ended at episode 5",
    };
  ...
  it("records visible KR refs without losing planned memo bindings", async () => {
    ...
    try {
      ...
      await expect(readFile(join(runtimeDir, `${contract.volumeId}.dashboard.md`), "utf-8")).resolves.toContain(
        "# Volume Dashboard: volume-001 Harbor Ledger (Episodes 1-5)",
      );
      await expect(readFile(join(runtimeDir, `${contract.volumeId}.dashboard.md`), "utf-8")).resolves.toContain(
        "| V1-KR1 | advanced | ch2 | ch2 | - | Recover the sealed invoice. |",
      );
  ...
  it("preserves post-write visible refs when the same episode is re-planned", async () => {
    const root = await mkdtemp(join(tmpdir(), "inkos-volume-replan-"));
    try {
      const bookDir = join(root, "book");
      const storyDir = join(bookDir, "story");
      const runtimeDir = join(storyDir, "runtime");
      ...
      // Re-planning the same episode (e.g. a rewrite) must not wipe the
      // visible evidence recorded after the first write.
      await recordVolumeProgressEntry(bookDir, {
        episode: 3,
        volumeId: contract.volumeId,
        volumeNumber: contract.volumeNumber,
        krRefs: ["KR2"],
        rationale: "replanned beat",
        memoGoal: "force clerk mistake",
        recordedAt: "2026-07-08T02:00:00.000Z",
      });
