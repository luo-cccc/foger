...
function createDelta(overrides: Partial<EpisodeRuntimeStateDelta> = {}): EpisodeRuntimeStateDelta {
  return {
    ...
    episodeSummary: overrides.episodeSummary,
    subplotOps: [],
    emotionalArcOps: [],
    characterMatrixOps: [],
    notes: [],
...
describe("arbitrateEpisodeRuntimeStateDeltaHooks", () => {
  it("maps a duplicate-family candidate back onto the matched existing hook", () => {
    const result = arbitrateEpisodeRuntimeStateDeltaHooks({
      hooks: [
        createHook({
  ...
  it("maps English information candidates onto existing Chinese information hooks", () => {
    ...
    expect(result.resolvedDelta.hookOps.upsert).toEqual([
      expect.objectContaining({ hookId: "H004", lastAdvancedEpisode: 3 }),
    ]);
        ...
        newHookCandidates: [
          {
            ...
            notes: "A fresh unresolved rule around the seal appears in this episode.",
          },
        ],
      }),
    });
  ...
  it("ignores a fabricated long id on a new upsert and assigns a derived id", () => {
    ...
    expect(result.resolvedDelta.episodeSummary?.hookActivity).toBe("D001 opened");
  });
  ...
  it("maps a separator-drifted legacy id back to the unique existing hook", () => {
    ...
    expect(result.resolvedDelta.hookOps.upsert).toEqual([
      expect.objectContaining({ hookId: "anonymous-source-scope", lastAdvancedEpisode: 6 }),
    ]);
  });
      ...
      delta: createDelta({
        ...
        hookOps: {
          upsert: [createHook({
            ...
            notes: "A new pursuit order activates this episode.",
          })],
          mention: [],
          resolve: [],
          defer: [],
