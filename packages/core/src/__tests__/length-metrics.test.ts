...
describe("length metrics", () => {
  it("counts Chinese episode length using zh_chars", () => {
    expect(countEpisodeLength("他抬头看天。", "zh_chars")).toBe(6);
  });

  it("counts English episode length using en_words", () => {
    expect(countEpisodeLength("He looked at the sky.", "en_words")).toBe(5);
  });

  it("defaults episode length to the language-native unit", () => {
    expect(defaultEpisodeLength("zh")).toBe(3000);
    expect(defaultEpisodeLength("en")).toBe(2000);
    expect(defaultEpisodeLength()).toBe(3000);
  });

  it("counts prose only for markdown-shaped Chinese episodes", () => {
    const markdownEpisode = [
      "---",
      "title: 第1章 归来",
      "---",
      "",
    ...
    expect(countEpisodeLength(markdownEpisode, "zh_chars")).toBe("陈风抬头看天。".length);
  });

  it("builds a conservative length spec for Chinese episodes", () => {
  ...
  it("builds a conservative length spec for English episodes", () => {
