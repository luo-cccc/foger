...
describe("context-filter", () => {
  it("filters old episode summary rows even when titles start with 'Episode'", () => {
    const summaries = [
      "# Episode Summaries",
      "",
      "| 1 | Episode 1 | Lin Yue | Old event | state-1 | side-quest-1 | tense | drama |",
      "| 97 | Episode 97 | Lin Yue | Recent event | state-97 | side-quest-97 | tense | drama |",
      "| 98 | Episode 98 | Lin Yue | New event | state-98 | side-quest-98 | tense | drama |",
      "| 100 | Episode 100 | Lin Yue | Latest event | state-100 | mentor-oath advanced | tense | drama |",
    ].join("\n");

    const filtered = filterSummaries(summaries, 101);

    expect(filtered).not.toContain("| 1 | Episode 1 |");
    expect(filtered).not.toContain("| 97 | Episode 97 |");
    expect(filtered).toContain("| 98 | Episode 98 |");
    expect(filtered).toContain("| 100 | Episode 100 |");
  });
