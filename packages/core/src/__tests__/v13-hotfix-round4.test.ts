 ...
 *   Issue 2: per-episode promotion pass
...
describe("Issue 2 — rerunPromotionPass", () => {
  it("promotes hook when mentioned in 2+ episodes of episode_summaries", () => {
  ...
  it("does not promote when mentioned in only 1 episode", () => {
    const hooks: StoredHook[] = [
      {
        hookId: "H06",
        startEpisode: 5,
...
describe("Issue 2 — per-episode promotion persists to pending_hooks.md", () => {
  let bookDir: string;
  ...
  it("after writing episode where H05 is mentioned 2nd time, pending_hooks.md shows H05 promoted=true", async () => {
    const hooks: StoredHook[] = [
      {
        hookId: "H05",
        startEpisode: 3,
    ...
    // Simulate episode_summaries with H05 mentioned in 2 episodes
    ...
    const summariesRaw = await readFile(join(storyDir, "episode_summaries.md"), "utf-8");
