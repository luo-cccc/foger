...
function episodeArtifact(episode: number, title: string): string {
  return renderEpisodeScriptMarkdown(EpisodeScriptSchema.parse({
    ...
    contract: {
      ...
      objective: { character: "林己", desiredChange: "取得证据", whyNow: "封锁即将开始" },
      ...
      causalEscalation: [{
        ...
        countermove: "守门人封锁出口",
        stateChange: "真相范围缩小",
        nextPressure: "林己必须立刻脱身",
      }],
      localDramaticResult: { goalOutcome: "取得部分证据", stateChange: "调查方向确定", costPaid: "身份暴露" },
      outgoingPressure: { startedDecisionDangerOrQuestion: "守门人开始追捕林己", whyItFollows: "林己已经触发警报" },
      handoffState: { knowledge: ["调查方向确定"], power: ["守门人控制出口"], relationship: ["双方公开敌对"], physical: [], activeAction: ["林己正在脱身"] },
      informationPermissions: [{ subject: "证据", audience: "观众", known: ["林己"], suspected: ["守门人"], mistaken: [], unknown: [] }],
    },
    scenes: [{
      id: "S1",
      location: "码头",
