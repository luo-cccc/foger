...
import { parseMemo } from "../utils/episode-memo-parser.js";
  ...
  it("returns a complete architect foundation contract for foundation prompts", () => {
    const response = stubChatCompletion(
      [
        ...
        { role: "user", content: "Target episodes: 12\nGenerate the complete foundation." },
      ],
      "stub-model",
    );
    ...
    expect(response.content).toContain("## Volume 1 (Episodes 1-12)");
    expect(response.content).not.toContain("## Volume 2");
    expect(response.content).toContain("### Compact Episode Beat Contract");
    expect(response.content).toContain("Episode 12: Goal=");
  ...
  it("returns a valid verdict for state validator prompts", () => {
    const response = stubChatCompletion(
      [
        {
          role: "system",
          content: "You are a continuity validator for an episode writing system. First line: exactly PASS or FAIL.",
        },
        {
          role: "user",
