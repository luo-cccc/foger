...
describe("analyzeAITells", () => {
  ...
  it("detects uniform paragraph lengths (dim 20)", () => {
    ...
    // dim 20 must report concrete paragraph locations/lengths
    expect(paraIssues[0]!.description).toContain("涉及段落");
    expect(paraIssues[0]!.description).toContain("第1段");
  });
  ...
  it("detects high hedge word density (dim 21)", () => {
    ...
    // dim 21 must locate specific occurrences
    expect(hedgeIssues[0]!.description).toContain("位置：");
    expect(hedgeIssues[0]!.description).toContain("第1段");
    expect(hedgeIssues[0]!.description).toContain("似乎");
  });
  ...
  it("detects formulaic transition repetition (dim 22)", () => {
    ...
    // dim 22 must cite locations of the repeated word
    expect(transIssues[0]!.description).toContain("位置：");
    expect(transIssues[0]!.description).toContain("第1段");
  });
  ...
  it("detects list-like sentence structure (dim 23)", () => {
    ...
    // dim 23 must quote the actual consecutive run
    expect(listIssues[0]!.description).toContain("连续句：");
    expect(listIssues[0]!.description).toContain("他看着远方的山峰");
  });
