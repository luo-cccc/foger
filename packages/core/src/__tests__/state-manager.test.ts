...
describe("StateManager", () => {
  ...
  describe("chapter persistence transaction recovery", () => {
    it("rolls back partial chapter, index, and truth writes from a preparing transaction", async () => {
      ...
      await manager.beginChapterPersistence(bookId, 1, "operation-transaction");
      await writeFile(join(chaptersDir, "0001_partial.md"), "partial", "utf-8");
      ...
      await expect(manager.recoverIncompleteChapterPersistence(bookId)).resolves.toEqual({
        kind: "rolled-back",
        chapterNumber: 1,
        rolledBackTo: 0,
      ...
      await expect(stat(join(chaptersDir, "0001_partial.md"))).rejects.toThrow();
      await expect(manager.loadChapterIndex(bookId)).resolves.toEqual([]);
    ...
    it("cleans up a committed marker without removing committed artifacts", async () => {
      ...
      await mkdir(chaptersDir, { recursive: true });
      await mkdir(storyDir, { recursive: true });
      await writeFile(join(chaptersDir, "0001_committed.md"), "committed chapter", "utf-8");
      ...
      await expect(stat(join(bookDir, ".chapter-persistence.json"))).rejects.toThrow();
      await expect(readFile(join(chaptersDir, "0001_committed.md"), "utf-8")).resolves.toBe("committed chapter");
      await expect(readFile(join(storyDir, "current_state.md"), "utf-8")).resolves.toBe("committed truth");
      await expect(manager.loadChapterIndex(bookId)).resolves.toHaveLength(1);
      const recoveryDiagnostic = JSON.parse(await readFile(join(storyDir, "runtime", "recovery.json"), "utf-8"));
      expect(recoveryDiagnostic).toMatchObject({
        kind: "committed-cleanup",
        chapterNumber: 1,
    ...
    it("recovers a preparing transaction through a new manager after process restart", async () => {
      ...
      await manager.saveChapterIndex(bookId, []);
      await manager.snapshotState(bookId, 0);
      await manager.beginChapterPersistence(bookId, 1, "operation-restart");
      await writeFile(join(chaptersDir, "0001_interrupted.md"), "interrupted", "utf-8");
      ...
      await expect(restartedManager.recoverIncompleteChapterPersistence(bookId)).resolves.toEqual({
        kind: "rolled-back",
        chapterNumber: 1,
        rolledBackTo: 0,
      ...
      await expect(stat(join(chaptersDir, "0001_interrupted.md"))).rejects.toThrow();
      await expect(restartedManager.loadChapterIndex(bookId)).resolves.toEqual([]);
    ...
    it("reports when no persistence transaction needs recovery", async () => {
      await expect(manager.recoverIncompleteChapterPersistence("no-transaction-book")).resolves.toEqual({ kind: "none" });
    });
  });
