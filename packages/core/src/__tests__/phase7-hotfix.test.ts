...
describe("Phase 7 hotfix 1 — half_life roundtrip", () => {
  ...
  it("renders the 半衰期 column and parses it back with the original value", () => {
    ...
    expect(hExplicit.halfLifeEpisodes).toBe(45);
    const hImplicit = parsed.find((h) => h.hookId === "H-implicit")!;
    expect(hImplicit.halfLifeEpisodes).toBeUndefined();
  });
  ...
  it("legacy 11-column pending_hooks.md still parses (half_life stays undefined)", () => {
    ...
    expect(h.halfLifeEpisodes).toBeUndefined();
    // promoted should also be undefined on legacy 11-col data.
    expect(h.promoted).toBeUndefined();
  });

  it("hook-stale-detection honors explicit halfLifeEpisodes after roundtrip", () => {
    const hooks: StoredHook[] = [
      {
        hookId: "H-late",
        startEpisode: 5,
    ...
    const diag = computeHookDiagnostics({ hooks: parsed, currentEpisode: 45 }).get("H-late")!;
...
describe("Phase 7 hotfix 2 — consolidator re-promotes advancedCount>=2", () => {
  ...
  it("flips promoted=false → true when episode_summaries mentions the hook in 2+ episodes", async () => {
    // Seed ledger: H-slept was architect-emitted as 否 (no rule firing at
    // seed time), but subsequent episode summaries mention it 3 times.
    ...
    // episode_summaries mentions H-slept in 3 episodes (>=2 → promote).
    // H-cold mentioned in only 1 episode (below threshold).
    await writeFile(
      join(bookDir, "story", "episode_summaries.md"),
      [
        "| 章节 | 标题 | 出场人物 | 关键事件 | 状态变化 | 伏笔动态 | 情绪基调 | 章节类型 |",
        "| --- | --- | --- | --- | --- | --- | --- | --- |",
        "| 4 | 旧信 | 林乙 | 收到家书 | 回忆浮现 | H-slept 推进 | 压抑 | 铺垫 |",
        "| 5 | 湿巷 | 林乙 | 遇到熟人 | 警戒上升 | H-cold 提及 | 紧绷 | 铺垫 |",
        "| 7 | 码头 | 林乙 | 查账 | 线索汇合 | H-slept 推进 | 紧绷 | 推进 |",
        "| 9 | 雨夜 | 林乙 | 回到住所 | 情绪积压 | H-slept 推进 | 压抑 | 内省 |",
      ].join("\n"),
      "utf-8",
    );
...
describe("Phase 7 hotfix 2 — reviewer gates critical severity on promoted", () => {
  it("zh reviewer prompt references 升级=是 as critical gate and the 已阻 N 集 token", async () => {
    ...
    try {
      ...
      await auditor.auditEpisode(bookDirLocal, "章节正文。", 1, "urban", {
        episodeContextSnapshot: await loadEpisodeContextSnapshot({
          bookDir: bookDirLocal,
          episode: 1,
          model: "test-model",
          service: "test",
        }),
      ...
      // The 已阻 N 集 literal token reviewer reads verbatim (from hotfix 3).
  ...
  it("en reviewer prompt gates on promoted=true and references 'blocked N episodes'", async () => {
    const { ContinuityAuditor } = await import("../agents/continuity.js");
    ...
    try {
      ...
      await auditor.auditEpisode(bookDirLocal, "Episode body.", 1, "other", {
        episodeContextSnapshot: await loadEpisodeContextSnapshot({
          bookDir: bookDirLocal,
          episode: 1,
          model: "test-model",
...
describe("Phase 7 hotfix 3 — blocked distance embeds 已阻 N 集 token", () => {
  it("reports blocked distance = currentEpisode - upstream.startEpisode when upstream is planted but unresolved", () => {
    ...
    expect(renderHookDiagnosticMarker(diag, "zh")).toContain("已阻 9 集");
    expect(renderHookDiagnosticMarker(diag, "en")).toContain("blocked 9 episodes");
  });

  it("uses hook.startEpisode as the reference when upstream is missing from the ledger entirely", () => {
    ...
    expect(renderHookDiagnosticMarker(diag, "zh")).toContain("已阻 7 集");
  });
  ...
  it("blockedDistance is 0 when hook is not blocked", () => {
    ...
    const diag = computeHookDiagnostics({ hooks: [upstream, downstream], currentEpisode: 10 }).get("D")!;
