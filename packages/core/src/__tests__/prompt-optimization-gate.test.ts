...
function buildSystemPrompt(language: "zh" | "en", episodeNumber: number): string {
  ...
  it("removes duplicated governed context without dropping quality contracts", () => {
    const contextPackage: ContextPackage = {
      ...
      selectedContext: [
        ...
        {
          source: "story/episode_summaries.md#6",
    ...
    const optimizedPrompt = (agent as unknown as {
      ...
      externalContext: "PER_EPISODE_USER_INSTRUCTION",
      lengthSpec,
      language: "en",
    ...
    expect(optimizedPrompt).toContain("Hard range: 90-210 seconds");
    ...
    expect(occurrences(legacyDuplicatedAssembly, "EPISODE_TASK_REQUIRED")).toBeGreaterThan(1);
    ...
    expect(getContextSourceTier("story/episode_summaries.md#6")).toBe("compressible");
  });
});
