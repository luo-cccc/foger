...
## 卷级 KR 绑定
- 绑定：KR1
- 推进方式：账册到手推动本卷锁定旧港债务证据。

## 不要做
- 不要出现母亲真实姓名
- 不要让账册被反派看到`;

function buildPlan(episode: number): PlanEpisodeOutput {
  return {
    intent: {
      episode,
      goal: "取回账册离开旧港",
      outlineNode: "Episode 1: return",
    ...
    memo: {
      ...
      threadRefs: ["H1"],
      volumeKrRefs: ["KR1"],
      volumeKrRationale: "账册到手推动本卷锁定旧港债务证据。",
      body: MEMO_BODY,
    },
...
describe("persisted-governed-plan round trip", () => {
  it("savePersistedPlan + loadPersistedPlan returns equal memo", async () => {
    ...
    await writeFile(
      join(dir, "story", "runtime", "episode-0001.intent.md"),
      "# Episode Intent\n\n## Goal\n取回账册离开旧港\n",
      "utf-8",
    );
    ...
    const persisted = await readFile(join(dir, "story", "runtime", "episode-0001.plan.md"), "utf-8");
    expect(persisted.trimStart()).not.toMatch(/^---\s*\n/);
    expect(persisted).toContain("## 本集目标");
...
`;
    await writeFile(
      join(dir, "story", "runtime", "episode-0001.plan.md"),
      corrupt,
      "utf-8",
    );
  ...
  it("returns null when episode number does not match", async () => {
    const dir = await mkdtemp(join(tmpdir(), "inkos-plan-"));
    await mkdir(join(dir, "story", "runtime"), { recursive: true });
  ...
  it("does not reuse a fallback memo left by an interrupted write", async () => {
    ...
    const historicalInput = await loadPersistedGovernedEpisodeInput(dir, 1);
