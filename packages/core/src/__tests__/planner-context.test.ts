...
| S001 | 三担保排查 | 主角 | ep1 | ep38 | 0 | 推进 | 核心旧账线 | 1章 |
| S007 | 货款线 | 主角 | ep3 | ep4 | 34 | 暂挂 | 冻结 | 4-6章 |
`;

describe("composeCurrentArcProse reads episode from emotional_arcs column 1", () => {
  it("filters rows by episode from column index 1 (剧集), not column 0 (角色)", () => {
  ...
  it("excludes rows at or beyond the current episode", () => {
    const prose = composeCurrentArcProse(
      SUBPLOT_BOARD_SAMPLE,
      EMOTIONAL_ARCS_SAMPLE,
      37,
...
`;
  ...
  it("derives stale subplot and high-intensity emotion as next planning focus", () => {
    const subplots = `
| subplot_id | 名称 | 负责人 | 起始 | 最近触达 | 沉默集数 | 状态 | 压力 |
|------------|------|--------|------|----------|----------|------|------|
| S001 | 三担保排查 | 主角 | ep1 | ep34 | 4 | 推进 | 核心旧账线 |
| S002 | 门卫证词 | 主角 | ep33 | ep38 | 0 | 推进 | 现场证据线 |
`;
  ...
  it("uses the status column, not any cell, to decide subplot activity", () => {
    ...
    const subplots = `
| subplot_id | 名称 | 负责人 | 起始 | 最近触达 | 沉默集数 | 状态 | 进度概述 | 回收ETA |
|------------|------|--------|------|----------|----------|------|----------|---------|
| S100 | 活跃线 | 主角 | ep1 | ep34 | 4 | 推进 | 稳步推进中 | ep40 |
| S900 | 已收线 | 配角 | ep1 | ep20 | 18 | 已完结 | 曾经推进到高压后收束 | - |
`;
    const snapshot = buildCurrentArcSnapshot(subplots, EMOTIONAL_ARCS_SAMPLE, 39);
  ...
  it("renders a structured current arc prose block while preserving legacy section names", () => {
    ...
    expect(prose).toContain("活跃支线");
    expect(prose).toContain("近期情感线");
    expect(prose).toContain("下一集规划焦点");
    expect(prose).toContain("S001");
    expect(prose).toContain("周谨川：ep36 紧绷");
  });
});
...
| hook_id | 状态 | 最近推进 |
|---------|------|----------|
| H001 | activating | ep38 |
| H002 | dormant | ep20 |
| H003 | partial_payoff | ep37 |
`;
    const subplots = `
| S001 | 主线追查 | 推进 |
| S007 | 旁线 | 暂挂 |
`;
  ...
  it("surfaces deferred hooks scheduled for the current episode as existing hooks", () => {
    const hooks = `
| hook_id | 起始剧集 | 类型 | 状态 | 预期回收 |
|---------|----------|------|------|----------|
