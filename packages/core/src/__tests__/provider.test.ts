...
vi.mock("@mariozechner/pi-ai", async (importOriginal) => {
  ...
  it("retries after a timeout abort with a fresh signal", async () => {
    mockCompleteSimple
      .mockImplementationOnce(async (_model: unknown, _context: unknown, opts: { signal?: AbortSignal } | undefined) => {
      ...
      .mockResolvedValueOnce(makeAssistantMessage("retry success"));

    const result = await chatCompletion(makeClient(0.7, { stream: false }), "test-model", [{ role: "user", content: "write" }], {
      timeoutMs: 5,
    });

    expect(result.content).toBe("retry success");
    expect(mockCompleteSimple).toHaveBeenCalledTimes(2);
    expect(mockCompleteSimple.mock.calls[0]?.[2]?.signal).toBeInstanceOf(AbortSignal);
    expect(mockCompleteSimple.mock.calls[1]?.[2]?.signal).toBeInstanceOf(AbortSignal);
    expect(mockCompleteSimple.mock.calls[0]?.[2]?.signal).not.toBe(mockCompleteSimple.mock.calls[1]?.[2]?.signal);
  });
});
...
describe("chatCompletion via pi-ai", () => {
  ...
  it("retries transient socket termination errors before failing the episode pipeline", async () => {
    mockStreamSimple
  ...
  it("uses OpenRouter reasoning fields for custom openai-compatible non-stream responses that omit content", async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        choices: [{
          message: {
            content: "",
            reasoning: "OpenRouter reasoning fallback",
            reasoning_details: [{ text: "details fallback" }],
          },
        }],
        usage: { prompt_tokens: 3, completion_tokens: 2, total_tokens: 5 },
      }),
    ...
    const result = await chatCompletion(client, "tencent/hy3:free", [{ role: "user", content: "nihao" }]);

    expect(result.content).toBe("OpenRouter reasoning fallback");
    expect(fetchMock).toHaveBeenCalledOnce();
  ...
  it("uses OpenRouter reasoning_details for custom openai-compatible streams that omit content deltas", async () => {
    const encoder = new TextEncoder();
    const sse = [
      "data: {\"choices\":[{\"delta\":{\"content\":\"\",\"reasoning_details\":[{\"text\":\"你\"}]}}]}\n\n",
      "data: {\"choices\":[{\"delta\":{\"content\":\"\",\"reasoning_details\":[{\"text\":\"好\"}]}}]}\n\n",
      "data: {\"usage\":{\"prompt_tokens\":3,\"completion_tokens\":2,\"total_tokens\":5}}\n\n",
      "data: [DONE]\n\n",
    ].join("");
    const fetchMock = vi.fn().mockResolvedValue({
