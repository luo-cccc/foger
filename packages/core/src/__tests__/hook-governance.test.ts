...
describe("collectStaleHookDebt", () => {
  it("returns unresolved stale hooks and excludes resolved or deferred hooks", () => {
    const result = collectStaleHookDebt({
      hooks: [
        createHook({ hookId: "H001", status: "open", lastAdvancedEpisode: 3 }),
        createHook({ hookId: "H002", status: "progressing", lastAdvancedEpisode: 8 }),
        createHook({ hookId: "H003", status: "deferred", lastAdvancedEpisode: 1 }),
        createHook({ hookId: "H004", status: "resolved", lastAdvancedEpisode: 2 }),
      ],
      episodeNumber: 20,
...
describe("classifyHookDisposition", () => {
  it("classifies mention, advance, resolve, and defer with strict priority", () => {
    ...
    expect(classifyHookDisposition({
      ...
      delta: createDelta({
        episode: 12,
        hookOps: {
          upsert: [createHook({ hookId: "H002", lastAdvancedEpisode: 12 })],
          mention: [],
          resolve: [],
          defer: [],
        },
    ...
    expect(classifyHookDisposition({
      ...
      delta: createDelta({
        hookOps: {
          upsert: [createHook({ hookId: "H003", lastAdvancedEpisode: 12 })],
          mention: ["H003"],
          resolve: ["H003"],
          defer: [],
        },
  ...
  it("returns none when the hook is untouched in the episode delta", () => {
    expect(classifyHookDisposition({
      hookId: "H099",
      delta: createDelta(),
    })).toBe("none");
  ...
  it("reports defer when resolve and defer both target the same hook", () => {
    expect(classifyHookDisposition({
      ...
      delta: createDelta({
        episode: 12,
        hookOps: {
          upsert: [createHook({ hookId: "H021", lastAdvancedEpisode: 12 })],
          mention: ["H021"],
          resolve: ["H021"],
          defer: ["H021"],
        },
