...
describe("validatePostWrite", () => {
  ...
  it("flags a purely atmospheric opening as a local blocking issue", () => {
    ...
    const result = detectEpisodeBoundaryQuality(content, "zh");
  ...
  it("allows atmosphere when an immediate event enters the opening window", () => {
    ...
    expect(findRule(detectEpisodeBoundaryQuality(content, "zh"), "空开头")).toBeUndefined();
  });
  ...
  it("flags an abstract summary ending as a local blocking issue", () => {
    ...
    for (const ending of endings) {
      ...
      expect(findRule(detectEpisodeBoundaryQuality(content, "zh"), "总结式结尾")).toBeDefined();
    }

    const content = `林越收起地图，离开仓库。\n\n${endings[0]}`;

    const violation = findRule(detectEpisodeBoundaryQuality(content, "zh"), "总结式结尾");
  ...
  it("allows concrete evidence, decisions, and discoveries at the ending", () => {
    ...
    for (const content of endings) {
      expect(findRule(detectEpisodeBoundaryQuality(content, "zh"), "总结式结尾")).toBeUndefined();
    }
  });
  ...
  it("detects English empty openings and summary endings", () => {
    ...
    expect(findRule(detectEpisodeBoundaryQuality(emptyOpening, "en"), "Empty opening")?.repairScope).toBe("local");
    expect(findRule(detectEpisodeBoundaryQuality(summaryEnding, "en"), "Summary ending")?.repairScope).toBe("local");
  });
  ...
  it("allows concrete English opening and ending beats", () => {
    ...
    expect(findRule(detectEpisodeBoundaryQuality(immediateEvent, "en"), "Empty opening")).toBeUndefined();
    expect(findRule(detectEpisodeBoundaryQuality(concreteEnding, "en"), "Summary ending")).toBeUndefined();
    expect(findRule(detectEpisodeBoundaryQuality(dialogueThreat, "en"), "Summary ending")).toBeUndefined();
  });
  ...
  it("detects paragraph density drift against recent episodes", () => {
    const recent = [
  ...
  it("prefers a concrete qualifier over weak function words like 预计", () => {
    const result = resolveDuplicateTitle(
      "你自由了",
      ["你自由了"],
      "zh",
      {
        content: "她预计明天启程，雨夜码头边，她把怀表放进抽屉锁上。",
      },
    );
    expect(result.title).toContain("：");
    expect(result.title).not.toContain("预计");
    expect(result.title).not.toBe("你自由了（2）");
  });

  it("avoids a 3-char fragment when a fresh 2-char noun exists elsewhere in the content", () => {
    // Regression: a blocked 2-char fragment ("计时") previously forced the
    // awkward 3-char fragment "计时长" from the same sentence. A fresh 2-char
    // image ("开城") elsewhere must be preferred, and the measure-word
    // fragment "时长" must be skipped as hollow.
    const result = resolveDuplicateTitle(
      "书生报国",
      ["书生报国", "书生报国：计时"],
      "zh",
      {
        content: "计时长，开城前他听见第三声钟响。",
      },
    );
    expect(result.title).toBe("书生报国：开城");
  });
