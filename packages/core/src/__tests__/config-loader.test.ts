...
describe("loadProjectConfig local provider auth", () => {
  ...
  it("surfaces an actionable hint when the resolved llm.baseUrl fails validation", async () => {
    // Paid-run regression: with configSource "studio" the CLI ignored
    // inkos.json's baseUrl/model and failed with a bare zod "Invalid url".
    root = await mkdtemp(join(tmpdir(), "inkos-config-loader-invalid-url-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
    // The key resolves from env (as in the paid rerun); the endpoint stays
    // broken so the failure reaches schema validation.
    process.env.INKOS_LLM_API_KEY = "sk-test";
    ...
    await expect(loadProjectConfig(root)).rejects.toThrow(/INKOS_LLM_BASE_URL/);
  });
