...
export class WriterAgent extends BaseAgent {
  constructor(ctx: AgentContext, private readonly settlerCtx?: AgentContext) {
    super(ctx);
  }
  ...
  async writeChapter(input: WriteChapterInput): Promise<WriteChapterOutput> {
    ...
    const creativeSystemPrompt = await this.withPromptPackGuidance(buildWriterSystemPrompt(
      ...
      chapterNumber, "creative", resolvedLanguage,
  ...
  private async settle(params: {
    ...
    const observerResponse = await this.settlerChat(
      [
  ...
  private isMeaningfulRuntimeSnapshot(snapshot: RuntimeStateSnapshot): boolean {
