...
description 中要明确引用 hook_id，并把状态列中 stale / blocked 的原文标记字面抄进去。本维度只审结构，不评价伏笔文笔。`;
    ...
    case 33:
      return language === "en"
        ? "Cross-check the supplied chapter_memo against the finished prose. Audit only fields the memo actually populates: current task, payoff / held-back boundary, transitional function, key choices, required end change, hook actions, volume-KR movement, and prohibitions. For each populated commitment, identify visible prose evidence or report the missing/contradicted commitment as critical. For numeric caps such as 'at most 3 scenes', count actual scenes first: an arrow/list naming the 3 allowed scenes is not violation evidence, and a critical issue requires identifying a 4th or later scene. A sparse memo is legitimate. Do not invent plot decisions, expand the memo, or rewrite prose; report evidence gaps for Planner/Writer to repair."
        : "对照随章提供的 chapter_memo 与成稿，只审 memo 实际填充的字段：当前任务、该兑现/暂不掀边界、过渡功能、关键抉择、章尾改变、hook 动作、卷级 KR 推进和不要做。每个已填承诺都要能指到正文中的可见证据；缺失或写反则标记 critical。遇到“场景最多3个/不要超过3个场景”之类数量上限时，必须先统计实际场景：箭头或清单列出的3个允许场景不是违禁证据，只有明确指出第4个及以后新增场景时才可判 critical。稀疏 memo 合法。Auditor 不发明剧情、不扩写 memo、不重写正文，只报告需要 Planner/Writer 修复的证据缺口。";
    default:
      return "";
  }
}
...
综合评分，不要因为单一小问题大幅拉低分数。`;
    ...
    let prevChapterBlock = previousChapter
      ? isEnglish
