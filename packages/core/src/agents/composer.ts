...
import {
  ...
  type ChapterClaimArtifactPaths,
...
export interface ComposeChapterOutput {
  ...
  readonly claimArtifacts?: ChapterClaimArtifactPaths;
}
...
export async function composeGovernedChapter(input: ComposeChapterInput): Promise<ComposeChapterOutput> {
  ...
  const baseSelectedContext = await collectSelectedContext(
    storyDir,
    input.plan,
...
async function buildChapterVolumeContext(
  bookDir: string,
  ...
  const contractFile = await loadVolumeContracts(bookDir, { chapterNumber });
  const contract = selectVolumeContract(contractFile.contracts, chapterNumber);
  if (!contract) {
    ...
    contextEntries: [
      {
        ...
        reason: "Binding current volume objective, key results, and irreversible event for this chapter.",
...
async function buildChapterClaimContext(
  bookDir: string,
  chapterNumber: number,
  plan: PlanChapterOutput,
  ...
  readonly composerInputs: string[];
  readonly notes: string[];
  readonly artifacts?: ChapterClaimArtifactPaths;
}> {
  ...
  const compiled = compileChapterClaims(claimsFile.claims, {
    chapterNumber,
    pov,
    memo: memoText,
  ...
  const artifacts = await saveChapterClaimArtifacts(bookDir, compiled, { chapterNumber, pov });
  ...
  return {
    ...
    composerInputs: [
      ...
      `story/runtime/chapter-${String(chapterNumber).padStart(4, "0")}.claims.json`,
      `story/runtime/chapter-${String(chapterNumber).padStart(4, "0")}.claim-brief.md`,
    ],
    contextEntries: [
      {
        source: "runtime/chapter_claim_brief",
        reason: "Binding chapter-level canon working set: usable claims, hidden truths, non-generalizable exceptions, costs, and conflict resolution.",
        excerpt: renderClaimBrief(compiled, { chapterNumber, pov }).trim(),
      },
      ...(validatorExcerpt
        ? [{
...
function inferPovFromMemo(text: string): string | undefined {
  const match = text.match(/(?:^|\n)\s*(?:[-*]\s*)?(?:POV|Point of view|视角|本章视角|视角人物)\s*[：:]\s*([^\n,，;；/|]+)/i);
  ...
  const chapterMemoEntry = [{
    ...
    const entries = await Promise.all([
      ...
      maybeContextSource(
        storyDir,
        "current_focus.md",
        skillReason("current-focus", "Current task focus for this chapter."),
      ),
      maybeContextSource(
        storyDir,
        "author_intent.md",
    ...
    const canonEntries = await Promise.all([
      maybeContextSource(
        storyDir,
        "parent_canon.md",
        "Preserve parent canon constraints for governed continuation writing.",
      ),
    ]);
...
async function maybeRuntimeCurrentArcSource(
  storyDir: string,
): Promise<ContextPackage["selectedContext"][number] | null> {
    const content = await readFileOrDefault(join(storyDir, "runtime", "tier2_current_arc.md"));
    ...
    return {
      ...
      reason: "Carry the structured current-arc pressure snapshot selected by the planner.",
      excerpt,
    };
}

function skillReason(needId: string, fallback: string): string {
  return fallback;
}
...
function deriveRetrievalHints(plan: PlanChapterOutput): string[] {
  return [
    ...
    ...(plan.memo.volumeKrRefs ?? []),
    plan.memo.volumeKrRationale,
