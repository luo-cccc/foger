...
//   === SECTION: pending_hooks ===  13-column 表；可含 startEpisode=0 种子行
//
...
export class ArchitectAgent extends BaseAgent {
  ...
  async generateFoundation(
    ...
    const userMessage = resolvedLanguage === "en"
      ? `Generate the complete 100-episode comic-drama foundation for the ${gp.name} series "${book.title}". Write everything in English.`
      : `请为标题为"${book.title}"的${gp.name}漫剧生成完整全剧基础设定。`;
  ...
  private buildRevisePrompt(reviseFrom: {
    ...
    return `\n\n## 既有架构稿修订模式
你在把一部已有漫剧的架构稿升级为当前的全剧总纲 + 篇章地图 + 一人一卡角色目录；如果它已经是 Episode v2 结构，则按用户反馈二次重写。
...
3. volume_map 使用段落式篇章方向，默认 10 个篇章、每篇 10 集，并把节奏原则放进末段
4. roles 必须按一人一卡输出，主要/次要角色判断沿用原内容，缺失才按主线重要性推断
5. pending_hooks 必须保留原有未回收伏笔，不要因为重写架构稿而清空
6. 不要改动已写剧集的运行时事实，不要重置 current_state / pending_hooks 之外的运行时日志
...
`;
  ...
  private buildChineseFoundationPrompt(
    ...
    const compactSeries = targetEpisodes <= FOUNDATION_COMPACT_MAX_EPISODES;
    const volumeMapBudget = compactSeries ? 8000 : 5000;
...
- 每集时长：约${book.episodeDurationSeconds ?? EPISODE_DURATION_TARGET_SECONDS}秒
...
## 去重铁律（必读）
禁止在多段里重复同一事实。主角弧线只写在 roles；世界铁律只写在 story_frame.世界观底色；节奏原则只写在 volume_map 最后一段；角色当前现状只写在 roles.当前现状；初始钩子只写在 pending_hooks（startEpisode=0 行）。需要真实年份、季节或重大历史事件锚点的题材，把环境/时代锚自然织进 story_frame.世界观底色；纯架空题材直接省略。如果一个段落写了另一段的内容，删掉。

## 预算（超预算必删）
- story_frame ≤ 3000 chars
- volume_map ≤ ${volumeMapBudget} chars${compactSeries ? "（紧凑篇逐集节拍合同优先，不能为压缩散文而删除）" : ""}
- roles 总 ≤ 8000 chars
...
- **前台故事**：读者每集看得到的表层冲突（查案、打怪、升级、谈恋爱、搞事业等），每个卷/arc 有独立的显性目标和完结点
...
这是故事篇章地图，**5 段主体 + 1 段节奏原则尾段**。默认把 100 集划为 10 个故事篇章，每篇 10 集；写清每篇的目标、情绪曲线、关系压力、计划反转、应回收 Hook 和进入下一篇时的状态。普通长项目的单集具体动作属于 Planner，不在这里逐镜编写；测试用短项目则按目标集数等比例缩放。${compactSeries ? `本剧只有 ${targetEpisodes} 集，属于紧凑完结项目：必须先原样输出上方“紧凑篇逐集节拍合同”，按第1-${targetEpisodes}集逐行或 Markdown 表格填写目标、阻碍、转折、交付、集末钩子；这是篇章级散文限制的唯一例外，后面的五段散文不得替代或省略它。` : ""}

### 段 1：各卷主题与情绪曲线
有几卷？每卷的主题一句话，每卷的情绪曲线一段（哪里压、哪里爽、哪里冷、哪里暖）。不要机械的"第一卷打小怪第二卷打大怪"，写情绪的流动。

### 段 2：卷间钩子与回收承诺（前台/后台双层都要覆盖）
第 1 卷埋什么钩子、在哪一卷回收；第 2 卷埋什么、在哪一卷回收。散文写，不要表格。**只写卷级**（如"第 1 卷埋的身世之谜在第 3 卷回收"），不要写具体集号。
...
### 段 4：卷尾必须发生的改变
每个篇章最后一集必须发生什么不可逆的事——权力结构改变、关系破裂、秘密暴露、主角身份重定位。写散文，一篇一段。**只写“必须发生什么”；紧凑项目的逐集合同必须明确第${targetEpisodes}集如何闭合核心冲突。**

### 段 5：节奏原则（具体化 + 通用）
**这是节奏原则的唯一归宿，不再有独立 rhythm_principles section。** 本段输出 6 条节奏原则。**至少 3 条必须具体化到本剧（也就是至少 3 条必须具体化到本书）**（例：“每两集至少完成一次可观察的局部兑现，兑现必须改变下一集选择”），其余可保留通用原则（例：“拒绝机械降神”“反转前必须有可见证据”）。具体化 + 通用混合是合法的。6 条各写 1-2 句，覆盖（顺序不强制、可替换同权重议题）：
1. 高潮间距——本剧大高潮之间最长多少集？（具体化优先）
2. 喘息频率——高压段多长必须插入短暂喘息？喘息集承担什么任务？
3. 钩子密度——每集集末留下什么明确观众问题，主钩最多允许悬多少集？
4. 信息释放节奏——主线信息在前 1/3、中段、后 1/3 分别释放多少比例？（可通用）
5. 爽点节奏——爽点间距多少集一个？什么类型为主？（具体化优先）
6. 情感节点递进——人物关系每多少集必须有一次实质推进？

如果外部指令给了内容比例（例如权谋线/感情线各半、事业线/恋爱线的权重），必须在本段写成全剧节奏承诺：哪些篇章偏哪条线、连续几集内哪条线必须可见、高潮后哪条线要承担后效。不要只写“保持平衡”。
...
## 当前现状（第 0 集初始状态）
（第 0 集前他在哪、做什么、处境如何、最近最烦心的事。**只写角色个人处境**——初始钩子写在 pending_hooks 的 startEpisode=0 行；环境/时代锚织进 story_frame.世界观底色。不再有独立的 current_state section。）
...
| hook_id | 起始剧集 | 类型 | 状态 | 最近推进 | 预期回收 | 回收节奏 | 上游依赖 | 回收篇章 | 核心 | 半衰期 | 备注 |

伏笔表规则：
- 第5列必须是纯数字剧集号，不能写自然语言描述
...
- 第9列「回收卷」：用自然语言写该伏笔计划在哪一卷哪一段回收（例："第2卷中段"、"终卷终集前"）。不强制解析为集号
- 第10列「核心」：是否主线承重伏笔 true / false。主线承重伏笔一本书最多 3-7 条（主谜团、身世、核心承诺），其余次要伏笔填 false
- 第11列「半衰期」：可选，整数集数。若不填自动按回收节奏推导（立即/近期 = 10、中程 = 30、慢烧/终局 = 80）
- 初始线索放备注列，不放第5列
- **初始世界状态 / 初始敌我关系** 如果有关键信息（例如"主角身上带着父亲的笔记本"、"体制已经开始监视码头"），可以作为 startEpisode=0 的种子行录入，备注列说明其"初始状态"属性。

## 最后强调
- 符合${book.platform}平台口味、${gp.name}题材特征
- 主角人设鲜明、行为边界清晰
- 伏笔前后呼应、配角有独立动机不是工具人
- **story_frame / volume_map / roles 必须保持信息密度；紧凑篇逐集节拍合同和 pending_hooks 表格是允许且必需的结构化例外**
- **book_rules 用普通 Markdown 规则卡，不要 YAML/JSON/代码块，也不要写成长篇散文**
- **不要输出 rhythm_principles 或 current_state 独立 section**——节奏原则合并进 volume_map 尾段；角色初始状态写在 roles.当前现状，初始钩子写在 pending_hooks（startEpisode=0 行），环境/时代锚（仅历史/年代/都市重生等需要年份的题材）织进 story_frame.世界观底色，不要硬凑
...
- Target episodes: ${book.targetEpisodes ?? 100}
- Episode duration: about ${book.episodeDurationSeconds ?? EPISODE_DURATION_TARGET_SECONDS} seconds
- North star: novel setting x familiar payoffs x high-pressure relationships x frequent causal reversals x emotional hooks
- Title: ${book.title}
...
- **Foreground story**: the surface conflict the reader sees every episode (cases, combat, leveling up, romance, business moves). Each volume / arc has its own visible goal and closure point.
...
Prose volume map, **5 sections + 1 closing rhythm paragraph**. **Critical requirement: stay at volume-level prose only** — specify each volume's theme, emotional curve, cross-volume hooks, character stage goals, and volume-end irreversible changes. **Do NOT prescribe episode-level tasks** (no "episode 17 sends him home"). Episode planning is the Phase 3 planner's job; the architect builds the skeleton, not the episode list. The sole exception is the Compact Episode Beat Contract required by the whole-book scale contract: for targets up to 12 episodes, emit it exactly, fill every field, and do not duplicate it in the prose that follows.
...
Volume 1 plants hook A, paid off in volume N; volume 2 plants hook B, paid off in volume M. Prose, not tables. **Stay at volume-level** (e.g., "the origin mystery planted in volume 1 pays off in volume 3"); do not specify episode numbers.
...
## Current_State (initial state at episode 0)
(Where they are at episode 0, what's on their mind, most recent worry. **Character-only**: initial hooks go in pending_hooks start_episode=0 rows; environment / era anchors (when the genre has a real year) are woven into story_frame's world-tonal-ground paragraph. No separate current_state section is produced.)
...
Rules:
- Column 5 is a pure episode number, not narrative description
- At book creation all planned hooks have last_advanced_episode = 0
...
- Column 9 (pays_off_in_arc): free-form prose on where this hook is scheduled to pay off (e.g. "mid of volume 2", "right before the finale"). NOT parsed into episode numbers
...
- Column 11 (half_life): optional integer episodes. If blank, derived from payoff_timing (immediate/near-term = 10, mid-arc = 30, slow-burn/endgame = 80)
- Put initial signal text in notes, not column 5
- **Initial world / alliance state**: any load-bearing initial condition ("protagonist carries the father's notebook", "the regime already watches the harbor") can be seeded as a start_episode=0 row with a note-column tag indicating its initial-state nature.
...
- **Do NOT emit rhythm_principles or current_state as separate sections** — rhythm principles live in the last paragraph of volume_map; character initial status goes in roles.Current_State; initial hooks go in pending_hooks (start_episode=0 rows); environment / era anchors (only when the genre has a real year) are woven into story_frame's world-tonal-ground paragraph
...
You MUST emit all **5 SECTION blocks in order**: story_frame → volume_map → roles → book_rules → pending_hooks. Do NOT stop after story_frame or volume_map just because they ran long. Even if roles lists only 3 characters, book_rules is a small Markdown block, and pending_hooks has only 3 rows, all five must appear. The output is only considered delivered after the last row of pending_hooks is written.`;
  ...
  private async parseSectionsWithRepair(
    ...
    try {
      return this.parseSections(content, language, targetEpisodes);
    } catch (error) {
      ...
      try {
        return this.parseSections(repaired, language, targetEpisodes);
      } catch (repairError) {
  ...
  private parseSections(
    ...
    // Legacy books (v12 / Phase 5 initial / pre-revert) and imported/spinoff books
    ...
    const effectiveVolumeMap = targetEpisodes === undefined
  ...
  async writeFoundationFiles(
    ...
    if (!isPhase5Output) {
      ...
      if (mode === "init") {
        const currentStateSeed = output.currentState?.trim()
          ? output.currentState
          : (language === "en"
              ? "# Current State\n\n> Seeded at book creation. Runtime state is appended by the consolidator after each episode.\n"
              : "# 当前状态\n\n> 建书时占位。运行时每集之后由 consolidator 追加最新状态。\n");
        ...
        writes.push(writeFile(
          ...
          language === "en"
            ? "# Emotional Arcs\n\n| Character | Episode | Emotional State | Trigger Event | Intensity (1-10) | Arc Direction |\n| --- | --- | --- | --- | --- | --- |\n"
            : "# 情感弧线\n\n| 角色 | 剧集 | 情绪状态 | 触发事件 | 强度(1-10) | 弧线方向 |\n|------|------|----------|----------|------------|----------|\n",
          "utf-8",
        ));
      }
    ...
    // section (only 3 genre families — historical derivatives / period fiction /
    // urban reincarnation — benefit from a
    ...
    // consolidator has a file to append each episode's state into.
    // Per-character state lives in roles/*.Current_State; initial hook rows
    // live in pending_hooks with start_episode=0. Legacy books / imports that
    // still produced the section keep their content as-is.
    if (mode === "init") {
      const currentStateSeed = output.currentState?.trim()
        ? output.currentState
        : (language === "en"
            ? "# Current State\n\n> Seeded at book creation. Runtime state is appended by the consolidator after each episode. Initial per-character state lives in roles/*.Current_State; load-bearing initial world facts live in pending_hooks rows with start_episode=0.\n"
            : "# 当前状态\n\n> 建书时占位。运行时每集之后由 consolidator 追加最新状态。每个角色的初始状态详见 roles/*.当前现状；承重的初始世界设定见 pending_hooks 里 startEpisode=0 的行。\n");
      writes.push(writeFile(join(storyDir, "current_state.md"), currentStateSeed, "utf-8"));
      writes.push(writeFile(join(storyDir, "pending_hooks.md"), output.pendingHooks, "utf-8"));
      writes.push(writeFile(
        join(storyDir, "emotional_arcs.md"),
    ...
    // episode_summaries.md are pure runtime projections derived from EpisodeScript.
    // The architect no longer seeds them here — mixing a
    ...
    // readers see the placeholder and the first episode state projection creates
  ...
  /**
   * Reverse-engineer foundation from existing episodes.
   */
...
The continuation portion must open up new narrative space — new conflict vector, new location, new time horizon. Ignite within 5 episodes; at least 50% fresh scenes.`
...
Naturally extend the existing arc. Advance existing conflicts, pay off planted hooks, introduce new complications organically.`)
      : (isSeries
          ? `## 续写方向要求
续写必须引入新叙事空间——新冲突、新地点、新时间。5集内引爆，50%以上场景新鲜。`
          : `## 续写方向
自然延续已有叙事弧线。推进现有冲突、兑现已埋伏笔、引入有机新变数。`);

    const systemPrompt = resolvedLanguage === "en"
      ? `You are a professional comic-drama series architect. Reverse-engineer an Episode v2 foundation from the source EpisodeScript artifacts and write the continuation path.${contextBlock}${reviewFeedbackBlock}
...
## Output contract
Follow the consolidated 5-section === SECTION: === layout: story_frame, volume_map, roles, book_rules, pending_hooks. Do NOT emit rhythm_principles or current_state — rhythm principles live in the last paragraph of volume_map; character initial status lives in roles.Current_State; initial hooks live in pending_hooks start_episode=0 rows; era / setting anchors (only when the genre pins to a real year) are woven into story_frame's world-tonal-ground paragraph.

All prose must be derived from the source package. Do not invent settings. If the package says it is compressed, treat episode catalog + excerpts as evidence for the foundation; the full episodes will be replayed later for detailed truth files. For volume_map, treat existing episodes as "review" (one paragraph) and continuation as prose episode-level planning. Hook extraction must be complete for the evidence provided.

All output MUST be written in English.`
      : `你是专业的漫剧总架构师。从已有 EpisodeScript 剧集中反向推导 Episode v2 基础设定，同时设计续写路径。${contextBlock}${reviewFeedbackBlock}

## 书籍元信息
- 标题：${book.title}
- 平台：${book.platform}
- 题材：${gp.name}（${book.genre}）
- 目标集数：${book.targetEpisodes ?? 100}集
- 每集时长：约${book.episodeDurationSeconds ?? EPISODE_DURATION_TARGET_SECONDS}秒
- 北极星：新颖设定 × 熟悉爽点 × 高压关系 × 高频反转 × 情绪钩子
...
## 输出契约
合并后的 5 段 === SECTION: === 结构：story_frame / volume_map / roles / book_rules / pending_hooks。**不要输出 rhythm_principles 或 current_state 两个 section**——节奏原则合并进 volume_map 尾段，角色初始状态合并进 roles.当前现状，初始钩子写在 pending_hooks startEpisode=0 行；环境/时代锚（只有年代文 / 历史派生长篇 / 都市重生等真实年份题材需要）织进 story_frame.世界观底色，其他题材直接省略。

所有 prose 必须从资料包中推导，不得臆造。若资料包声明为压缩包，把剧集目录与结构化剧本摘录当作基础设定证据；完整剧集会在后续通过确定性 reducer 顺序回放进入 truth files。volume_map 中，已有剧集作为回顾段，续写部分写到篇章级方向。伏笔识别以资料包提供的证据为准。`;

    const userMessage = resolvedLanguage === "en"
      ? `Generate the complete foundation for the imported ${gp.name} comic-drama series "${book.title}". Write everything in English.\n\n${episodesText}`
      : `以下是《${book.title}》的已有 EpisodeScript 剧集资料包，请从中反向推导完整基础设定：\n\n${episodesText}`;
...
${trimmed}\n`;
  ...
  private normalizePendingHooksSection(section: string, volumeMapRaw: string): string {
    ...
    const normalizedHooks = dataRows.map((row, index) => {
      ...
      const normalizedProgress = this.parseHookEpisodeNumber(rawProgress);
      ...
      const base: Record<string, unknown> = {
        ...
        startEpisode: this.parseHookEpisodeNumber(row[1]),
        type: row[2] ?? "",
        status: row[3] ?? "open",
        lastAdvancedEpisode: normalizedProgress,
    ...
    const allSeedStartEpisodes = new Map<string, number>(
      normalizedHooks.map((hook) => [hook.hookId, hook.startEpisode]),
    );
    const promotionContext: PromotionContext = {
      volumeBoundaries,
      currentEpisode: 0,
      advancedCounts: new Map(),
      allSeedStartEpisodes,
    };
    const promotedHooks = normalizedHooks.map((hook) => {
      const decision = shouldPromoteHook(hook, promotionContext);
      const status = !decision.promote && hook.lastAdvancedEpisode <= 0
  ...
  /**
   * Parse `第N卷 (A-B章)` / `Volume N (episodes A-B)` headers from the
  ...
  private parseHookEpisodeNumber(value: string | undefined): number {
    if (!value) return 0;
  ...
  /**
   * Rewrite only the volume_map section so the outline covers a new target
   * episode count while story_frame / roles / book_rules stay untouched.
   * Single explicit LLM call, invoked by `inkos foundation extend`.
   */
  async generateVolumeMapExtension(params: {
    readonly book: BookConfig;
    readonly genreProfile: GenreProfile;
    readonly genreBody: string;
    readonly storyFrame: string;
    readonly currentVolumeMap: string;
    ...
    const userPrompt = [
      isEn ? "## Book metadata" : "## 书籍元信息",
      `- ${isEn ? "Title" : "标题"}：${params.book.title}`,
      `- ${isEn ? "Platform" : "平台"}：${params.book.platform}`,
      `- ${isEn ? "Genre" : "题材"}：${params.genreProfile.name}（${params.book.genre}）`,
      `- ${isEn ? "Target episodes" : "目标集数"}：${targetEpisodes} 集`,
      `- ${isEn ? "Episode duration" : "每集时长"}：约 ${params.book.episodeDurationSeconds ?? EPISODE_DURATION_TARGET_SECONDS} 秒`,
      "",
      "## 题材底色 / Genre body",
      params.genreBody || "(none)",
      "",
      "## 既有故事框架 / Existing story_frame（只读，不修改）",
      params.storyFrame || "(none)",
      "",
      "## 当前 volume_map / Current volume_map（只读参考，需重写以覆盖新集数）",
      params.currentVolumeMap || "(none)",
      "",
      scaleGuidance,
      "",
      isEn
        ? "Requirements: keep every already-established episode/arc direction consistent; split the series into coherent volumes; each volume defines 3 observable Key Results and the irreversible volume-end change; episodes must be attributable to KR progress; the finale episode must close the core conflict."
        : "要求：已发生的剧集与篇章方向必须保持连续；把全剧分成连贯的篇章；每个篇章定义 3 条可观察的 Key Result 与卷尾不可逆改变；每集必须可归因到 KR 进度；终局集必须闭合核心冲突。",
      compact
        ? (isEn
            ? `This is a compact ${targetEpisodes}-episode series: output the compact per-episode beat contract covering episodes 1-${targetEpisodes} (Goal / Obstacle / Turn / Delivery / End Hook per row), then the prose sections.`
            : `这是紧凑完结作品（${targetEpisodes} 集）：先输出覆盖第1-${targetEpisodes}集的紧凑篇逐集节拍合同（每集一行：目标/阻碍/转折/交付/集末钩子），再写散文段。`)
        : "",
    ].filter(Boolean).join("\n");
    ...
    const response = await this.chat(
      ...
      { temperature: 0.5, stream: false, callPhase: "architect-extend" },
    );
    const volumeMap = this.extractVolumeMapSection(response.content).trim();
    const issues = validateFoundationVolumeScale(volumeMap, targetEpisodes);
    return {
      volumeMap,
      warnings: issues.map((issue) => (language === "en" ? issue.en : issue.zh)),
    };
  }

  private extractVolumeMapSection(content: string): string {
    const match = content.match(/===\s*SECTION:\s*volume_map\s*===\s*([\s\S]*?)(?====\s*SECTION:|$)/i);
