...
// Legacy English novel-era prompt fragments kept for public API compatibility.
// The Episode v2 writer pipeline does not consume these; the live English
// screenplay prompt is assembled in writer-prompts.ts.
...
8. **Episode hooks**: Every episode ending needs a hook—question, reveal, threat, promise.
...
### Logic / Consistency
13. **World rules are law**: Once established, physics/magic/social rules cannot bend for plot convenience.
14. **Cost matters**: Every power, ability, or advantage must have a cost or limitation that creates real trade-offs.
15. **Consequences stick**: Actions have consequences. Characters can't escape repercussions through luck or author fiat.
16. **No reset buttons**: The world must change permanently in response to major events.

### Reader Psychology
17. **Promise and payoff**: Every planted hook must be resolved. Every mystery must have an answer.
18. **Escalation**: Each conflict should feel higher-stakes than the last—either externally or emotionally.
19. **Reader proxy**: One character should react with surprise/excitement/fear when remarkable things happen, giving readers permission to feel the same.
20. **Pacing breathing room**: After a high-intensity sequence, give 0.5-1 episode of lower intensity before the next escalation.

### Prose Execution Rhythm
- Every scene must advance the episode goal, relationship, evidence, or risk. Avoid explanatory filler.
- Density comes from action, information, and choice, not from inventing hooks outside the memo.
- Vary paragraph length and avoid stacking three or more one-line action/reaction paragraphs. Deterministic post-write checks own the numeric thresholds.

### Episode Cut (80/20 cliffhanger, hard ruler)
- End on the memo's required change or hook; do not defer a payoff that the planner explicitly assigned to this episode.
- Stay inside the supplied hard length range. Trim transition and explanation first; never pad to reach length or keep redundant prose beyond the hard maximum.`;
}
...
**[IRON LAW 5]** Planning terms never appear in episode text.
"Current situation," "core motivation," "information boundary" are PRE_WRITE_CHECK tools only.

**[IRON LAW 6]** Ban the "Not X; Y" construction. Max once per episode.
...
This method is for YOUR planning. The terms never appear in the episode text.`;
}

// English pre-write checklist
export function buildEnglishPreWriteChecklist(book: BookConfig, gp: GenreProfile): string {
  const items = [
    "Outline anchor: Which volume_outline plot point does this episode advance?",
    "POV: Whose perspective? Consistent throughout?",
    "Hook planted: What question/promise/threat carries reader to next episode?",
    ...
    `Pacing: Episode targets about ${book.episodeDurationSeconds} seconds. ${gp.pacingRule}`,
    "Show don't tell: Are emotions shown through action, not labeled?",
    "AI-tell check: No banned analytical language in prose?",
    "Conflict: What is the core tension driving this episode?",
  ];
...
Planned series length: ${book.targetEpisodes} episodes. The current episode's user prompt is the only authority for its word range.
