...
export class PlannerAgent extends BaseAgent {
  ...
  async planChapter(input: PlanChapterInput): Promise<PlanChapterOutput> {
    ...
    const volumeContract = selectVolumeContract(volumeContractFile.contracts, input.chapterNumber);
      ...
      volumeContractBrief: volumeContract
        ? [
            ...
            renderVolumeProgressBrief(volumeProgress!, volumeContract, {
              beforeChapter: input.chapterNumber,
              windowSize: 5,
            }),
          ].join("\n")
        : "",
    ...
    if (input.book.format === "screenplay") {
      await writeFile(
        join(runtimeDir, `episode-${String(input.chapterNumber).padStart(4, "0")}.intent.md`),
        intentMarkdown,
        "utf-8",
      );
    }
  ...
  async planChapterMemo(input: {
    ...
    const [characterMatrix, subplotBoard, emotionalArcs, pendingHooks, bookRulesRaw] = await Promise.all([
      ...
      renderBookRulesMarkdown(input.storyDir),
    ]);
    ...
    const currentArcProse = composeCurrentArcProse(subplotBoard, emotionalArcs, input.chapterNumber);
    await this.saveCurrentArcRuntimeArtifact(input.storyDir, input.chapterNumber, currentArcProse);
  ...
  private async saveCurrentArcRuntimeArtifact(
    ...
    await writeFile(
      ...
      [
        "# Tier2 Current Arc",
        "",
        `- updated_for_chapter: ${chapterNumber}`,
  ...
  private buildFallbackMemoMarkdown(input: {
    ...
    if (input.language === "en") {
      return [
        `# Episode ${input.chapterNumber} memo`,
        "",
        "## Episode goal",
        input.fallbackGoal || `Continue episode ${input.chapterNumber} according to the current outline`,
        "",
        "## Thread refs",
        "none",
        "",
        "## Current task",
        `Use the current episode goal and authoritative series context to continue episode ${input.chapterNumber} without inventing a new direction.`,
        "",
        "## Episode payoff",
    ...
    return [
      `# 第 ${input.chapterNumber} 集 memo`,
      "",
      "## 本集目标",
      input.fallbackGoal || `按当前大纲继续推进第 ${input.chapterNumber} 集`,
      "",
      "## 关联线索",
      "无",
      "",
      "## 当前任务",
      `沿用当前单集目标和权威设定推进第 ${input.chapterNumber} 集，不临时改方向，也不把本集写成泛泛过渡。`,
      "",
      "## 本集爽点",
