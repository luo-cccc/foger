  ...
  private buildFallbackMemoMarkdown(input: {
    ...
    if (input.language === "en") {
      return [
        ...
        "",
        "## Volume KR binding",
        "Bind: buffer. Advancement: this fallback memo could not inspect the current volume contract, so the chapter must visibly support the nearest active objective or explain the transition on page.",
        "",
        "## Do not",
    ...
    return [
      ...
      "",
      "## 卷级 KR 绑定",
      "绑定：缓冲。推进方式：fallback memo 无法可靠读取当前卷合同，本章必须可见地服务最近的卷级目标，或在正文推进中说明过渡承接。",
      "",
      "## 不要做",
