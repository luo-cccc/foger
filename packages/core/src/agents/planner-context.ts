...
/**
 * Grab the last N row(s) from episode_summaries.md formatted as markdown
...
export function formatRecentSummaries(
  episodeSummariesRaw: string,
  episodeNumber: number,
  limit: number,
): string {
  const rows = parseMarkdownTableRows(episodeSummariesRaw)
    .filter((row) => /^\d+$/.test(row[0] ?? ""))
    .filter((row) => parseInt(row[0]!, 10) < episodeNumber)
  ...
  const header = "| 剧集 | 标题 | 出场人物 | 关键事件 | 状态变化 | Hook 动态 | 情绪基调 | 剧集类型 |";
  const divider = "| --- | --- | --- | --- | --- | --- | --- | --- |";
...
export interface CurrentArcSubplot {
  ...
  readonly lastTouchEpisode?: number;
  readonly silenceEpisodes?: number;
 ...
 * pressure, and next-episode focus instead of dumping raw recent rows.
 */
export function buildCurrentArcSnapshot(
  ...
  const activeSubplots = extractActiveSubplots(subplotBoardRaw, episodeNumber)
...
export function composeCurrentArcProse(
  ...
  const snapshot = buildCurrentArcSnapshot(subplotBoardRaw, emotionalArcsRaw, episodeNumber);
...
function extractActiveSubplots(raw: string, episodeNumber: number): CurrentArcSubplot[] {
...
function extractRecentEmotionalArcEntries(raw: string, episodeNumber: number, limit: number): CurrentArcEmotion[] {
  const rows = parseMarkdownTableRows(raw);
  if (rows.length === 0) {
    return raw
      .split("\n")
  ...
  // emotional_arcs.md column layout: 角色 | 剧集 | 情绪状态 | 触发事件 | 强度 | 弧线方向
  // Episode number lives in column index 1 (row[1]), not column 0.
  return rows
    .filter((row) => !isEmotionalArcHeaderRow(row))
    .map(parseEmotionalArcRow)
    .filter((entry) => entry.episode > 0 && entry.episode < episodeNumber)
    .sort((a, b) => a.episode - b.episode)
    .slice(-limit)
    .map((entry) => entry);
}

function parseActiveSubplotRow(row: ReadonlyArray<string>, episodeNumber: number): CurrentArcSubplot {
  const lastTouchEpisode = parseEpisodeCell(row[4]) ?? latestEpisodeRef(row);
  const explicitSilence = parsePlainInteger(row[5]);
  const silenceEpisodes = explicitSilence ?? (
    lastTouchEpisode === undefined ? undefined : Math.max(0, episodeNumber - lastTouchEpisode)
  );
  const status = subplotStatusCell(row);
...
function parseEmotionalArcRow(row: ReadonlyArray<string>): CurrentArcEmotion {
  const episode = parsePlainInteger(row[1]) ?? 0;
  return {
...
function summarizeCurrentArcPressure(
  ...
  if (activeSubplots.length > 0) {
    ...
    lines.push(`支线压力集中在 ${labels}，本集应选择其中 1 条做可见推进或明确暂压理由`);
  }

  const highestEmotion = [...recentEmotions]
    .filter((entry) => entry.intensity !== undefined)
    .sort((a, b) => (b.intensity ?? 0) - (a.intensity ?? 0) || b.episode - a.episode)[0];
  if (highestEmotion) {
    const episodeLabel = highestEmotion.episode > 0 ? `ep${highestEmotion.episode} ` : "";
    lines.push(
      `情绪压力最高点：${highestEmotion.character} ${episodeLabel}${highestEmotion.state}`
    ...
    const episodeLabel = latest.episode > 0 ? `ep${latest.episode} ` : "";
    lines.push(`最近情绪落点：${latest.character} ${episodeLabel}${latest.state}`);
  }
...
function deriveNextPlanningFocus(
  ...
  const staleSubplot = activeSubplots
    .filter((subplot) => (subplot.silenceEpisodes ?? 0) >= 3)
    .sort((a, b) => (b.silenceEpisodes ?? 0) - (a.silenceEpisodes ?? 0))[0];
  if (staleSubplot) {
    focus.push(
      `优先处理 ${formatSubplotLabel(staleSubplot)}，它已沉默 ${staleSubplot.silenceEpisodes} 集，避免活跃支线悬置`,
    );
  ...
  const chargedEmotion = [...recentEmotions]
    ...
    .sort((a, b) => b.episode - a.episode || (b.intensity ?? 0) - (a.intensity ?? 0))[0];
  if (chargedEmotion) {
    focus.push(
...
function renderCurrentArcSubplot(subplot: CurrentArcSubplot): string {
  ...
  if (subplot.lastTouchEpisode !== undefined) meta.push(`最近触达=ep${subplot.lastTouchEpisode}`);
  if (subplot.silenceEpisodes !== undefined) meta.push(`沉默=${subplot.silenceEpisodes}集`);
...
function renderCurrentArcTrajectory(trajectory: CurrentArcTrajectory): string {
  const entries = trajectory.entries.map((entry) => {
    const episode = entry.episode > 0 ? `ep${entry.episode} ` : "";
    const details = [
      entry.trigger,
      entry.intensity !== undefined ? `强度${entry.intensity}` : "",
      entry.direction,
    ].filter(Boolean).join("，");
    return `${episode}${entry.state}${details ? `（${details}）` : ""}`;
  });
...
function isEmotionalArcHeaderRow(row: ReadonlyArray<string>): boolean {
  return row.some((cell) => /^(角色|character|剧集|episode|情绪状态|state|触发事件|trigger|强度|intensity|弧线方向|direction)$/i.test(cell.trim()));
}
...
function parseEpisodeCell(value: string | undefined): number | undefined {
...
function latestEpisodeRef(row: ReadonlyArray<string>): number | undefined {
  const refs = row
    .map(parseEpisodeCell)
...
export function extractRelevantThreads(
  ...
  const hookStartIndex = hookHeader?.findIndex((cell) => /^(起始剧集|start(?:_episode)?)$/i.test(cell.trim())) ?? -1;
  const selectedHookRows = parsedHookRows
    ...
    .filter((row) => {
      ...
      const startEpisode = parsePlainInteger(row[hookStartIndex >= 0 ? hookStartIndex : 1]);
      const deferred = DEFERRED_STATUS_PATTERN.test(status);
      const resolved = RESOLVED_STATUS_PATTERN.test(status);
      return deferred && !resolved && startEpisode !== undefined && startEpisode <= episodeNumber;
    })
    .map((row) => {
      const status = row[hookStatusIndex >= 0 ? hookStatusIndex : 1] ?? "";
      const scheduled = episodeNumber !== undefined
  ...
  const futureHookRegistry = episodeNumber === undefined
    ? []
    : parsedHookRows
      .filter((row) => !/^(hook_id)$/i.test(row[0] ?? ""))
      ...
      .map((row) => {
        ...
        return `- ${row[0]}: 已存在（起始剧集 ${start}，状态 ${status}），仅作 ID 注册，不得作为 [new] 重开`;
      });
 ...
 * episode's memo ("## 本集 hook 账"). These are already filtered by
...
export function formatRecyclableHooks(
  ...
  const lines = topSlice.map((hook) => {
    ...
    const silence = lastTouch <= 0 ? episodeNumber : Math.max(0, episodeNumber - lastTouch);
    const payoff = hook.expectedPayoff?.trim() || hook.notes?.trim() || "";
    const core = hook.coreHook === true ? (language === "en" ? " [core]" : " [核心]") : "";
    return language === "en"
      ? `- ${hook.hookId} "${payoff}" — status=${hook.status}, silent ${silence} episodes${core}`
      : `- ${hook.hookId} "${payoff}" — 状态=${hook.status}，已沉默 ${silence} 集${core}`;
  });
