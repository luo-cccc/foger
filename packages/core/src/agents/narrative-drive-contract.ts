...
把整本书设计成“新颖设定 x 熟悉爽点 x 高压关系 x 因果反转 x 情绪钩子”的组合。乘法意味着任何一项长期为零都会拖垮追读，但不是要求每集机械集齐五项。
...
- 情绪钩子让读者带着一个具体的选择、代价、亏欠或关系余震进入下一集，不把“突然出现更强敌人”当成唯一集末方式。

结构归位：差异点及其代价写进 story_frame，熟悉回报与反转节奏写进 volume_map，高压关系写进核心冲突与 roles，情绪钩子和兑现承诺进入 volume_map 尾段的节奏原则与 pending_hooks。`;
  }
...
Design the book as Novel Premise x Familiar Payoff x High-Pressure Relationship x Causal Reversal x Emotional Hook. Multiplication means that leaving any factor at zero for too long collapses reader momentum; it does not mean mechanically checking all five in every episode.
...
Place premise and cost in story_frame; payoff and reversal cadence in volume_map; pressure relationships in core conflict and roles; emotional hooks and delivery promises in the volume_map rhythm principles and pending_hooks.`;
  }
...
Render the memo's selected drivers as one causal chain. Never narrate this framework, and do not invent an extra reversal or hook. Make the premise difference visible through choice and loss; use a familiar desire to carry its learning cost; embody relationship pressure through exchange, refusal, testing, betrayal, or forced cooperation. A reversal must point back to evidence and change subsequent action. End on a concrete choice, cost, debt, or relationship aftershock rather than merely raising the volume of danger. In breather or aftermath episodes, render the last turn's loss and new normal instead of turning the board again.`;
  }

  return `## Narrative Drive Audit

Audit only the drivers actually promised by the episode memo and existing setup; never require all five in every episode. Check whether the premise difference changes choice or cost, familiar desire receives the planned observable feedback, relationship pressure grows from both parties' interests, a reversal points back to evidence and changes later action, and the ending leaves a concrete choice, cost, debt, or aftershock. Report a structural failure only when an explicit memo promise is missing. Do not penalize a breather or aftermath episode that renders the prior reversal's consequences without adding a new turn. Coincidence turns, newly invented rules, convenient stupidity, withheld viewpoint knowledge, and unrelated-enemy cliffhangers are structural defects.`;
}
