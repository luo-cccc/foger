...
import type { CompiledEpisodeClaims } from "./episode-claim-compiler.js";

export type ClaimGateIssue = AuditIssue;

export interface ClaimGateTextInput {
  readonly text: string;
  readonly compiled: CompiledEpisodeClaims;
...
/**
 * Shared deterministic matcher used by the post-write claim gate and the
 * canon evolution pass: does the given text carry enough salient terms from
 * the claim content to count as the fact being present?
 */
export function claimTextMatches(text: string, claimContent: string): boolean {
  return hasCoreTermOverlap(normalizeText(text), claimContent);
}
    ...
    if (textRevealsHiddenClaim(input.text, claim)) {
      ...
      issues.push(issue({
        ...
        description: characterKnowledgeLeak
          ...
          : input.phase === "pre"
            ? `Episode memo references hidden canon claim "${claim.id}" before it is available.`
            : `Episode prose appears to reveal hidden canon claim "${claim.id}" before it is available.`,
        suggestion: characterKnowledgeLeak
          ? "Keep the character's knowledge within the declared visibility boundary, or make this episode an explicit reveal."
          : "Remove the hidden fact from this episode or move it into an explicit reveal plan.",
      }));
    }
  }
  ...
  if (input.phase === "pre") {
    for (const claim of input.compiled.revealNow) {
      issues.push(issue({
        ...
        claim,
        phase: input.phase,
        description: `Episode memo plans to reveal canon claim "${claim.id}" this episode.`,
  ...
  if (input.phase === "post") {
    for (const claim of input.compiled.revealNow) {
      ...
      issues.push(issue({
        ...
        claim,
        phase: input.phase,
        description: `Episode memo planned to reveal canon claim "${claim.id}", but the draft does not make that reveal observable on-page.`,
        suggestion: "Either stage the reveal explicitly in prose, or revise the episode memo so this canon fact is not promised for this episode.",
      }));
    }
  }
  ...
  for (const claim of input.compiled.usable) {
    if (claim.claimType === "prohibition" && detectsProhibitedContent(input.text, claim)) {
      issues.push(issue({
        ...
        suggestion: "Delete or rewrite the prohibited element; hard prohibitions cannot be bypassed by episode prose.",
      }));
    }
