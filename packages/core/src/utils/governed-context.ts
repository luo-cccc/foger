...
export function buildGovernedMemoryEvidenceBlocks(
  ...
  const summaryEntries = contextPackage.selectedContext.filter((entry) =>
    entry.source.startsWith("story/episode_summaries.md#")
      && entry.source !== "story/episode_summaries.md#recent_titles"
      && entry.source !== "story/episode_summaries.md#recent_mood_type_trail",
  );
  ...
  return {
    ...
    summariesBlock: summaryEntries.length > 0
      ? renderEvidenceBlock(
          resolvedLanguage === "en" ? "Selected Episode Summary Evidence" : "已选剧集摘要证据",
          summaryEntries,
        )
      : undefined,
