...
import { DEFAULT_EPISODE_CADENCE_WINDOW } from "./episode-cadence.js";
 ...
 * files every episode while still making the omission visible to the model.
 */
...
/** Filter episode_summaries: keep only the most recent N episodes. */
export function filterSummaries(
  summaries: string,
  currentEpisode: number,
  keepRecent = DEFAULT_EPISODE_CADENCE_WINDOW,
): string {
  if (!summaries || summaries === "(文件尚未创建)") return summaries;
  return filterTableRows(summaries, (row) => {
    const match = row.match(/\|\s*(\d+)\s*\|/);
    if (!match) return true;
    return parseInt(match[1]!, 10) > currentEpisode - keepRecent;
  });
}
...
/** Filter emotional_arcs: keep only the most recent N episodes. */
...
function isHeaderRow(line: string): boolean {
  ...
  return /^\|\s*(章节|角色|支线|hook_id|Episode|Character|Subplot)/i.test(line);
}
