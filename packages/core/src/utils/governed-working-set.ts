...
export function buildGovernedHookWorkingSet(params: {
  ...
  const agendaIds = collectHookAgendaIds(params.episodeIntent);
...
function collectHookAgendaIds(episodeIntent?: string): Set<string> {
  if (!episodeIntent || episodeIntent.trim().length === 0) {
    return new Set();
  }

  const ids = new Set<string>();
  const lines = episodeIntent.split("\n");
