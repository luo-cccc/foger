...
export async function resolveEffectiveLLMConfig(
  ...
  try {
    parsed = ProjectConfigSchema.parse(config);
  } catch (error) {
    // A raw ZodError like `[{validation: "url", path: ["llm","baseUrl"]}]`
    // gives no hint about WHERE the bad value came from (observed in paid
    // runs: `configSource: "studio"` silently ignores inkos.json's
    // baseUrl/model and the CLI failed with an opaque "Invalid url").
    const issues = (error as { readonly issues?: ReadonlyArray<{ readonly path?: ReadonlyArray<string | number>; readonly message?: string }> }).issues;
    const llmIssue = issues?.find((issue) => issue.path?.[0] === "llm");
    if (llmIssue) {
      const field = String(llmIssue.path?.[1] ?? "");
      const current = field ? (llm as Record<string, unknown>)[field] : undefined;
      throw new Error(
        `LLM 配置无效：llm.${field || "?"} ${llmIssue.message ?? "校验失败"}（当前解析值：${JSON.stringify(current ?? "")}）。`
        + `配置来源为 ${configMode}——当 configSource 为 "studio" 时，inkos.json 里的 baseUrl/model 会被 Studio 服务配置接管；`
        + `请用 INKOS_LLM_BASE_URL / INKOS_LLM_MODEL / INKOS_LLM_API_KEY 环境变量，或运行 'inkos config set-global' 重新写入端点配置。`,
      );
    }
    throw error;
  }
