 ...
 * ledger into noise within a few episodes.
 *
 * A seed is *promoted* into the ledger when any of these four conditions
 * holds. The consolidator (maintains pending_hooks.md post-episode) calls
 ...
 *      more episodes since planting, proving readers already track it.
...
export interface PromotionContext {
  ...
  /** Current episode number (0 at book creation). */
  readonly currentEpisode: number;
  /** Hook id -> advanced count (how many episodes advanced this hook since planting). */
  readonly advancedCounts: ReadonlyMap<string, number>;
  /**
   * Hook id -> start episode lookup for every seed (including unpromoted ones).
   * Used to tell whether a `depends_on` target is declared in a later volume.
   */
  readonly allSeedStartEpisodes: ReadonlyMap<string, number>;
}
...
function isCrossVolume(hook: StoredHook, context: PromotionContext): boolean {
  const { volumeBoundaries, allSeedStartEpisodes } = context;
  if (volumeBoundaries.length < 2) return false;

  const seedVolumeIndex = findVolumeIndex(volumeBoundaries, hook.startEpisode);
  ...
  for (const upstreamId of hook.dependsOn ?? []) {
    const upstreamStart = allSeedStartEpisodes.get(upstreamId);
...
function findVolumeIndex(
  ...
  for (let i = 0; i < boundaries.length; i++) {
    ...
    if (episode >= vol.startCh && episode <= vol.endCh) return i;
  }
  // Episode 0 (seed time) counts as volume 0 if volumes start at 1.
  if (episode <= 0 && boundaries.length > 0) return 0;
  return -1;
}
...
/**
 * Derive the default half-life (in episodes) for a hook that did not set it
 ...
 */
export function defaultHalfLifeEpisodes(
...
export function resolveHalfLifeEpisodes(hook: StoredHook): number {
  return hook.halfLifeEpisodes ?? defaultHalfLifeEpisodes(hook.payoffTiming);
}
...
/**
 * Derive hook advancement counts from episode_summaries.md content.
 *
 * Counts how many episode-summary data rows mention each hook id in the
...
export function deriveAdvancedCountsFromSummaries(
  ...
  for (const hookId of hookIds) {
    ...
    for (const line of lines) {
      ...
      if (line.includes("---") || /\|\s*(章节|Episode)\s*\|/i.test(line)) continue;
...
/**
 * Lightweight promotion pass: read hooks + episode_summaries, check
