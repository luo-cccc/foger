import type { EpisodeIntent, EpisodeMemo, ContextPackage } from "../models/input-governance.js";
import {
...
const EPISODE_REF_PATTERNS: ReadonlyArray<RegExp> = [
  /\bch(?:apter)?\s*\d+\b/gi,
  /第\s*\d+\s*章/g,
];
...
const EN_REPLACEMENTS: ReadonlyArray<[RegExp, string]> = [
  [/\bprevious episodes\b/gi, "earlier scenes"],
  [/\bthis episode needs to\b/gi, "the current move is to"],
];
...
export function sanitizeNarrativeControlText(
  ...
  for (const pattern of EPISODE_REF_PATTERNS) {
...
/**
 * Render a EpisodeMemo + optional EpisodeIntent into a sanitized narrative
...
export function renderMemoAsNarrativeBlock(
  ...
  const contract = compileEpisodeExecutionContract(memo);
  const sections: string[] = [renderEpisodeExecutionContract(contract, language)];
  ...
  sections.push(isEn
    ? [
        ...
        "- The final scene must visibly deliver every item in Required end-of-episode change.",
...
export function buildNarrativeIntentBrief(
  ...
  const rendered = sections
    .map(({ heading, label }) => {
      const section = extractMarkdownSection(episodeIntent, heading);
