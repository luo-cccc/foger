...
import { analyzeEpisodeCadence } from "./episode-cadence.js";
import {
...
export async function buildEnglishVarianceBrief(params: {
  ...
  const episodeBodies = await loadPreviousEpisodeBodies(
    params.bookDir,
    params.episodeNumber,
    CADENCE_WINDOW_DEFAULTS.englishVarianceLookback,
  );
  if (episodeBodies.length < 2) {
    return null;
  }

  const summaryRows = await loadSummaryRows(join(params.bookDir, "story", "episode_summaries.md"));
  ...
  const highFrequencyPhrases = collectRepeatedEnglishPhrases(episodeBodies);
  const repeatedOpeningPatterns = collectRepeatedBoundaryPatterns(episodeBodies, "opening");
  const repeatedEndingShapes = collectRepeatedBoundaryPatterns(episodeBodies, "ending");
...
export async function analyzeLongSpanFatigue(
  ...
  const summaryRows = await loadSummaryRows(join(input.bookDir, "story", "episode_summaries.md"));
  const mergedRows = mergeCurrentSummary(summaryRows, input.episodeSummary);
  const recentRows = mergedRows
    .filter((row) => row.episode <= input.episodeNumber)
    .sort((left, right) => left.episode - right.episode)
    .slice(-CADENCE_WINDOW_DEFAULTS.summaryLookback);
  const cadence = analyzeEpisodeCadence({
    rows: recentRows,
  ...
  const episodeTypeIssue = buildEpisodeTypeIssue(cadence, language);
  if (episodeTypeIssue) {
    issues.push(episodeTypeIssue);
  }
  ...
  const recentEpisodeBodies = await loadRecentEpisodeBodies(
    input.bookDir,
    input.episodeNumber,
    input.episodeContent,
  );

  const openingIssue = buildSentencePatternIssue(recentEpisodeBodies, "opening", language);
  if (openingIssue) {
    issues.push(openingIssue);
  }

  const endingIssue = buildSentencePatternIssue(recentEpisodeBodies, "ending", language);
  if (endingIssue) {
...
async function loadPreviousEpisodeBodies(
  bookDir: string,
  ...
  try {
    ...
    const previousFiles = files
      ...
      .filter((entry) => Number.isFinite(entry.episode) && entry.episode < currentEpisode && entry.file.endsWith(".md"))
      .sort((left, right) => left.episode - right.episode)
      .slice(-limit);

    return Promise.all(
      previousFiles.map((entry) => readFile(join(episodesDir, entry.file), "utf-8")),
    );
  } catch {
    return [];
  }
...
function mergeCurrentSummary(rows: ReadonlyArray<SummaryRow>, currentSummary?: string): SummaryRow[] {
  ...
  const nextRows = rows.filter((row) => row.episode !== parsedCurrent.episode);
...
function parseSummaryRow(line: string): SummaryRow | null {
  ...
  if (!trimmed.startsWith("|") || trimmed.includes("章节 |") || trimmed.includes("Episode |") || trimmed.includes("---")) {
    return null;
  }
  ...
  const episode = Number.parseInt(cells[0] ?? "", 10);
...
function buildEpisodeTypeIssue(
  cadence: ReturnType<typeof analyzeEpisodeCadence>,
  language: "zh" | "en",
  ...
  if (language === "en") {
    return {
      ...
      description: `The last ${streak} episode types have stayed on ${repeatedType}, which suggests macro pacing monotony.`,
      suggestion: "Switch the next episode's function instead of extending the same beat again. Rotate setup, payoff, reversal, and fallout more deliberately.",
    };
  }
...
function buildMoodIssue(
  ...
  if (language === "en") {
    return {
      ...
      description: `High-tension mood has locked in for ${highTensionStreak} episodes (${recentMoods.join(" -> ")}), with no visible emotional release.`,
...
function buildTitleIssue(
  ...
  if (language === "en") {
    return {
      ...
      description: `Recent titles keep collapsing around "${repeatedToken}" (${count} hits in the current window), which makes episode naming feel formulaic.`,
  ...
  return {
    ...
    suggestion: "下一集标题换一个新的意象、动作、后果或人物焦点，不要继续套同一个关键词壳。",
  };
}

async function loadRecentEpisodeBodies(
  bookDir: string,
  currentEpisode: number,
...
function buildSentencePatternIssue(
  episodeBodies: ReadonlyArray<string>,
  boundary: "opening" | "ending",
  language: "zh" | "en",
): LongSpanFatigueIssue | null {
  if (episodeBodies.length < LONG_SPAN_FATIGUE_THRESHOLDS.boundaryPatternMinBodies) return null;

  const sentences = episodeBodies.map((body) => extractBoundarySentence(body, boundary));
  ...
  if (language === "en") {
    ...
    return {
      severity: "warning",
      category,
      description: `The last 3 episode ${position} are highly similar (adjacent similarity ${pairText}), which risks a formulaic rhythm. Current ${boundary} signature: "${sample}".`,
      suggestion: boundary === "opening"
        ? "Change the next episode opening vector. Start from action, consequence, or surprise instead of repeating the same camera move."
        : "Change the next episode landing pattern. End on consequence, decision, or a new variable instead of repeating the same explanatory cadence.",
    };
  }
...
function collectRepeatedEnglishPhrases(episodeBodies: ReadonlyArray<string>): string[] {
  const counts = new Map<string, number>();

  for (const body of episodeBodies) {
    const tokens = body
      .toLowerCase()
      .replace(/[^a-z0-9\s]+/gi, " ")
      .split(/\s+/)
