...
export function analyzeHookHealth(params: {
  ...
  readonly delta?: Pick<EpisodeRuntimeStateDelta, "episode" | "hookOps">;
  ...
  /** Hook ids explicitly named by the current memo/contract. */
  readonly relevantHookIds?: ReadonlyArray<string>;
}): AuditIssue[] {
  const maxActiveHooks = params.maxActiveHooks ?? HOOK_HEALTH_DEFAULTS.maxActiveHooks;
  const staleAfterEpisodes = params.staleAfterEpisodes ?? HOOK_HEALTH_DEFAULTS.staleAfterEpisodes;
  ...
  const relevantHookIds = params.relevantHookIds ? new Set(params.relevantHookIds) : undefined;
  const pressuredHooks = lifecycleEntries.filter(({ hook, lifecycle }) =>
    ...
    && (!relevantHookIds || relevantHookIds.has(hook.hookId)),
  );
  ...
  if (unresolvedPressure.length > 0) {
    issues.push(warning(
      ...
      buildPressureDescription({
        ...
        mentionsCurrentEpisode: Boolean(params.delta),
      }),
      params.language === "en"
    ...
    const latestRealAdvance = activeHooks.reduce(
      (max, hook) => Math.max(max, hook.lastAdvancedEpisode),
      0,
    );
    if (
      params.noAdvanceWindow !== undefined
      && activeHooks.length > 0
      && params.episodeNumber - latestRealAdvance >= noAdvanceWindow
    ) {
      issues.push(warning(
        params.language,
        params.language === "en"
          ? `No real hook advancement has landed for ${params.episodeNumber - latestRealAdvance} episodes.`
          : `已经连续 ${params.episodeNumber - latestRealAdvance} 章没有真实伏笔推进。`,
        params.language === "en"
          ? "Schedule one old hook for real movement instead of opening parallel restatements."
          : "下一集优先让一个旧伏笔发生真实推进，而不是继续平行重述。",
      ));
    }
  }
  ...
  if (params.delta) {
    ...
    const hasOlderActiveDebt = params.hooks.some((hook) =>
      ...
      && hook.startEpisode < params.episodeNumber
    ...
    if (
      ...
      issues.push(warning(
        ...
        params.language === "en"
          ...
          : `本集新开了 ${newHookIds.length} 个伏笔，但没有回收任何旧债。`,
        params.language === "en"
...
function buildPressureDescription(params: {
  ...
  readonly mentionsCurrentEpisode: boolean;
}): string {
  ...
  if (params.language === "en") {
    return params.mentionsCurrentEpisode
      ? `Hooks are already under payoff pressure but this episode left them untouched: ${summarized.join(", ")}${suffix}.`
      : `Hooks are already under payoff pressure without recent movement: ${summarized.join(", ")}${suffix}.`;
  }

  return params.mentionsCurrentEpisode
    ? `这些伏笔已经进入回收/推进压力，但本集没有真正处理：${summarized.join("、")}${suffix}。`
