...
export async function loadCurrentVolumeContract(
  ...
  const file = await loadVolumeContracts(bookDir, { episodeNumber });
  return selectVolumeContract(file.contracts, episodeNumber);
}
...
export async function recordVolumeProgressEntry(
  ...
  // Re-planning the same episode must not wipe evidence recorded by the
  // post-write gate. Preserve previously detected visible/attempted KR refs
  // and merge the incoming set.
  const existing = progress.entries.find((candidate) => candidate.episode === entry.episode);
  ...
  const entries = [
    ...progress.entries.filter((existing) => existing.episode !== entry.episode),
    nextEntry,
...
export async function recordVisibleVolumeProgress(
  ...
  const existing = progress.entries.find((entry) => entry.episode === params.episode);
  const nextEntry: VolumeProgressEntry = {
  ...
  const next = VolumeProgressFileSchema.parse({
    ...
    generatedAt: recordedAt,
    entries: [
      ...progress.entries.filter((entry) => entry.episode !== params.episode),
      nextEntry,
    ].sort((left, right) => left.episode - right.episode),
  });
  await saveVolumeProgress(bookDir, next);
...
export function renderVolumeProgressBrief(
  ...
  options: {
    readonly beforeEpisode?: number;
    readonly windowSize?: number;
  } = {},
): string {
  const beforeEpisode = options.beforeEpisode ?? Number.POSITIVE_INFINITY;
  const windowSize = options.windowSize ?? 5;
  const entries = recentVolumeProgressEntries(progress, contract, beforeEpisode, windowSize);
  if (entries.length === 0) {
  ...
  return [
    ...
    "",
    ...entries.map((entry) => [
      `- ch${entry.episode}: planned=${entry.krRefs.length > 0 ? entry.krRefs.join(" / ") : "(buffer)"}`,
...
function renderVolumeDashboardSection(
  ...
  const krRows = contract.keyResults.map((kr) => {
    const plannedEpisodes = episodesForKr(volumeEntries, kr.id, "planned");
    const visibleEpisodes = episodesForKr(volumeEntries, kr.id, "visible");
    const attemptedEpisodes = episodesForKr(volumeEntries, kr.id, "attempted");
    const status = resolveKrStatus(contract, kr.id, visibleEpisodes, attemptedEpisodes);
    return `| ${kr.id} | ${status} | ${plannedEpisodes.length > 0 ? plannedEpisodes.map((ch) => `ch${ch}`).join(", ") : "-"} | ${visibleEpisodes.length > 0 ? visibleEpisodes.map((ch) => `ch${ch}`).join(", ") : "-"} | ${attemptedEpisodes.length > 0 ? attemptedEpisodes.map((ch) => `ch${ch}`).join(", ") : "-"} | ${kr.text} |`;
  });
  const recent = volumeEntries.slice(-5).map((entry) =>
    `- ch${entry.episode}: planned=${entry.krRefs.length > 0 ? entry.krRefs.join(" / ") : "(buffer)"} visible=${entry.visibleKrRefs.length > 0 ? entry.visibleKrRefs.join(" / ") : "-"} attempted=${(entry.attemptedKrRefs ?? []).length > 0 ? (entry.attemptedKrRefs ?? []).join(" / ") : "-"}${entry.memoGoal ? ` | ${entry.memoGoal}` : ""}${entry.rationale ? ` | ${entry.rationale}` : ""}`,
  );
  return [
    `${heading} ${contract.volumeId} ${contract.title}`,
    "",
    `- episodes: ${contract.episodeStart && contract.episodeEnd ? `${contract.episodeStart}-${contract.episodeEnd}` : "(not declared)"}`,
    ...
    "",
    "| KR | status | plannedEpisodes | visibleEpisodes | attemptedEpisodes | text |",
    "| --- | --- | --- | --- | --- | --- |",
    ...krRows,
    "",
    "### Recent entries",
...
export function selectVolumeContract(
  ...
  const ranged = contracts.find((contract) =>
    contract.episodeStart !== undefined
    && contract.episodeEnd !== undefined
    && episodeNumber >= contract.episodeStart
    && episodeNumber <= contract.episodeEnd,
  );
  if (ranged) return ranged;
  const starts = contracts
    .filter((contract) => contract.episodeStart !== undefined && episodeNumber >= contract.episodeStart)
    .sort((left, right) => (right.episodeStart ?? 0) - (left.episodeStart ?? 0));
...
export function renderVolumeContractBrief(
  ...
  return [
    ...
    contract.episodeStart && contract.episodeEnd
      ? `- episodes: ${contract.episodeStart}-${contract.episodeEnd}`
      : undefined,
    `- objective: ${contract.objective}`,
    `- irreversibleEvent: ${contract.irreversibleEvent}`,
    contract.protagonistStageGoal ? `- protagonistStageGoal: ${contract.protagonistStageGoal}` : undefined,
    ...
    ...contract.keyResults.map((kr) => {
      const episodes = progress ? episodesForKr(
        progress.entries.filter((entry) => entry.volumeId === contract.volumeId),
        kr.id,
        "visible",
      ) : [];
      const attemptedEpisodes = progress ? episodesForKr(
        progress.entries.filter((entry) => entry.volumeId === contract.volumeId),
        kr.id,
        "attempted",
      ) : [];
      const status = progress ? resolveKrStatus(contract, kr.id, episodes, attemptedEpisodes) : kr.status;
      return [
        `- ${kr.id}: ${kr.text} [${status}]`,
        episodes.length > 0 ? `visible=${episodes.join(",")}` : undefined,
        attemptedEpisodes.length > 0 ? `attempted=${attemptedEpisodes.join(",")}` : undefined,
      ].filter(Boolean).join(" ");
    }),
  ].filter((line): line is string => line !== undefined).join("\n");
}
...
export function runVolumeGate(input: {
  ...
  const episodeNumber = input.episodeNumber ?? input.memo.episode;
  ...
  if (unknownRefs.length > 0) {
    issues.push({
      ...
      description: `[${input.phase}-write volume gate] Episode memo references unknown volume KR: ${[...new Set(unknownRefs)].join(", ")}.`,
      suggestion: "Bind the episode to an existing KR from the current VolumeContract or explain why the episode is a buffer/transition.",
      repairScope: "local",
    });
  }
  ...
  if (boundRefs.length === 0 && !hasRationale) {
    issues.push({
      ...
      description: `[${input.phase}-write volume gate] Episode memo neither binds an existing volume KR nor explains a buffer/transition exception.`,
      suggestion: "Add at least one volume KR binding (for example KR1) or a concrete rationale for not advancing a KR this episode.",
      repairScope: "local",
    });
  }

  if (input.phase === "pre" && input.progress && episodeNumber > 1) {
    ...
    if (boundRefs.length === 0 && recentEntries.length >= Math.max(2, miniCycleWindow - 1) && !recentAdvanced) {
      issues.push({
        ...
        description: `[pre-write volume gate] Recent ${recentEntries.length + 1}-episode mini-cycle has no visible volume KR advancement, including this episode's memo.`,
        suggestion: "Advance at least one current-volume KR this episode, or revise the recent buffer run so the volume objective remains observable.",
        repairScope: "local",
      });
    }
  }
  ...
  if (input.phase === "post" && input.text && isVolumeEndEpisode(input.contract, episodeNumber)) {
    const text = normalizeText(input.text);
    const progressedRefs = new Set<string>(detectVisibleKrRefs(input.contract, text));
    for (const entry of input.progress?.entries ?? []) {
      if (entry.volumeId !== input.contract.volumeId || entry.episode > episodeNumber) continue;
    ...
    if (missingKrs.length > 0) {
      issues.push({
        ...
        description: `[post-write volume gate] Volume end reached but these KRs have no recorded episode binding: ${missingKrs.map((kr) => kr.id).join(", ")}.`,
...
export function recentVolumeProgressEntries(
  ...
  return progress.entries
    .filter((entry) =>
      ...
      && entry.episode < beforeEpisode
      && isEpisodeInContract(entry.episode, contract),
    )
...
function resolveKrStatus(
  ...
  attemptedEpisodes: ReadonlyArray<number> = [],
): "pending" | "attempted" | "advanced" | "done" {
  if (episodes.length === 0) return attemptedEpisodes.length > 0 ? "attempted" : "pending";
  if (contract.episodeEnd !== undefined && episodes.some((episode) => episode >= contract.episodeEnd!)) {
    return "done";
  }
  return "advanced";
}

function isEpisodeInContract(episode: number, contract: VolumeContract): boolean {
  if (contract.episodeStart !== undefined && episode < contract.episodeStart) return false;
  if (contract.episodeEnd !== undefined && episode > contract.episodeEnd) return false;
  return true;
}

function isVolumeEndEpisode(contract: VolumeContract, episodeNumber: number): boolean {
  // Fire the volume-end gate only on the planned last episode. Internal episode
  // written sequentially, so `=== episodeEnd` hits exactly once per volume.
  // Using `>=` here re-fired the full volume-end critical suite on every episode
  // past the planned end (e.g. when writing overruns the outline and
  // selectVolumeContract falls back to the last volume for out-of-range episodes).
  return contract.episodeEnd !== undefined && episodeNumber === contract.episodeEnd;
}
...
function contractFromBlock(
  ...
  const range = parseEpisodeRange(`${block.heading}\n${block.body}`);
  return VolumeContractSchema.parse({
    ...
    ...(range ? { episodeStart: range.start, episodeEnd: range.end } : {}),
    objective,
    keyResults,
    irreversibleEvent,
...
function parseEpisodeRange(text: string): { start: number; end: number } | null {
  const patterns = [
    /(?:(?:Episodes?|Episodes?)|Ch\.?)\s*(\d+)\s*[-~–—]\s*(\d+)/i,
