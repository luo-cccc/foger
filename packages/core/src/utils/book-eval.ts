...
export interface BookEval {
  ...
  readonly episodes: ReadonlyArray<EpisodeEval>;
  readonly qualityTrend: ReadonlyArray<{ readonly episode: number; readonly score: number }>;
}
...
export function computeEpisodeEvalScore(ch: EpisodeEval): number {
  let score = 100;
...
function parseEpisodeRange(range?: string): { readonly start: number; readonly end: number } {
...
export async function evaluateBookQuality(options: EvaluateBookQualityOptions): Promise<BookEval> {
  ...
  const filteredIndex = index.filter((ch) => ch.episodeNumber >= start && ch.episodeNumber <= end);
  const episodeFiles = await readdir(episodesDir).catch(() => [] as string[]);
  const episodeEvals: EpisodeEval[] = [];

  for (const ch of filteredIndex) {
    const paddedNum = String(ch.episodeNumber).padStart(4, "0");
    const file = episodeFiles.find((f) => f.startsWith(paddedNum) && f.endsWith(".md"));
    const content = file ? await readFile(join(episodesDir, file), "utf-8") : "";
  ...
  const avgAiTellDensity = episodeEvals.length > 0
    ? episodeEvals.reduce((s, c) => s + c.aiTellDensity, 0) / episodeEvals.length
    : 0;
  const avgParagraphWarnings = episodeEvals.length > 0
    ? episodeEvals.reduce((s, c) => s + c.paragraphWarnings, 0) / episodeEvals.length
    : 0;
  ...
  return {
    bookId,
    totalEpisodes: filteredIndex.length,
    totalDurationSeconds: filteredIndex.reduce((s, c) => s + c.episodeDurationSeconds, 0),
    ...
    duplicateTitles,
    qualityScore,
    episodes: episodeEvals,
    qualityTrend: episodeEvals.map((ch) => ({
      episode: ch.episodeNumber,
      score: computeEpisodeEvalScore(ch),
    })),
  };
}
