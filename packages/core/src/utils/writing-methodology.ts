 ...
 * writer on every episode as part of the style_guide context.
 */
...
去AI味不只是"禁什么"，更要"多写什么"。以下四类是正向倾向，优先采用：

1. **无意义小动作**：给角色一些不推进剧情但有真实感的小动作——挠头、点烟、抠桌角、把玩打火机、无意识转笔。人真实起来靠这些"废动作"，不是靠对白效率。
2. **现实落差**：让期待和现实错位一下——满怀信心推开门却发现没人、鼓足勇气开口对方却已经走了、以为的大人物只是个跑腿的。落差自带人味和黑色幽默。
3. **嘴硬补偿**：角色输了场面、丢了面子，不写"他很沮丧"，写他嘴硬找补——"切，谁稀罕""我本来也没想赢"。嘴硬比认输更像活人。
4. **生活噪音**：场景里留一点无关的背景噪音——隔壁的电视声、楼下吵架、没关严的水龙头、一只不停叫的狗。这些噪音不服务主线，但让世界不像真空。`;
}
...
1. Which outline node does this episode correspond to?
...
6. Does the episode end with a hook?
7. Any flowchart passages? If so, add causality or strong emotion.
8. Does this episode advance the main plotline?

## 7. Encouraged Positive Techniques (the pro-active side of de-AI-ing)

De-AI-ing is not only about what to ban — it is even more about what to write more of. These four are positive leanings; prefer them:

1. **Meaningless small actions**: give characters little gestures that carry no plot weight but read as real — scratching a head, lighting a cigarette, picking at the table edge, fiddling with a lighter, absently spinning a pen. People feel real through these "wasted" actions, not through efficient dialogue.
2. **Reality gaps**: let expectation and reality miss each other — pushing the door open with confidence to find no one there, working up the nerve to speak only to realize they already left, the "big shot" turning out to be an errand runner. The gap carries humanity and dark humor.
3. **Stubborn-mouth compensation**: when a character loses face or loses the exchange, don't write "he was dejected" — write them saving face out loud: "Tch, who wants it anyway," "I wasn't trying to win." Bravado reads more alive than open defeat.
4. **Life noise**: leave some irrelevant background noise in the scene — a TV next door, a couple arguing downstairs, a faucet not quite shut off, a dog that won't stop barking. This noise serves no plotline, but it keeps the world from feeling like a vacuum.`;
}
