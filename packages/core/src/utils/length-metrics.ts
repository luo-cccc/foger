...
// Per-episode length default in the book's native unit: Chinese counts characters (3000字),
// English counts words (~2000 ≈ a 3000-char episode). One cross-language number would mis-scale —
// 3000 read as English words runs ~50% long, and the hard-range guard then force-expands correct episodes.
export const DEFAULT_EPISODE_LENGTH_ZH = 3000;
export const DEFAULT_EPISODE_LENGTH_EN = 2000;

export function defaultEpisodeLength(language: LengthLanguage = "zh"): number {
  return language === "en" ? DEFAULT_EPISODE_LENGTH_EN : DEFAULT_EPISODE_LENGTH_ZH;
}

export function countEpisodeLength(
  content: string,
