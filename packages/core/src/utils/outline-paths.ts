...
// the consolidator has appended output from episode 1 onward. Readers that
// previously relied on architect-provided initial state (writer phase-1
// creative prompt, continuity, deterministic replay, reviser, composer) should
...
/**
 * Read current_state.md; when the file is only a seed placeholder (episode 0,
 * before consolidator has appended anything), derive an initial-state block
 * from roles/*.Current_State + pending_hooks startEpisode=0 rows so callers
 * still have substantive content to feed into writer and review prompts.
 */
