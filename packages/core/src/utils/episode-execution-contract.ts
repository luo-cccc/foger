...
export type EpisodeExecutionDirectiveKind =
  | "current-task"
  | "payoff"
  | "end-change"
  | "do-not"
...
export interface EpisodeExecutionDirective {
  readonly id: string;
  readonly kind: EpisodeExecutionDirectiveKind;
  readonly text: string;
  readonly anchors: ReadonlyArray<string>;
}

export interface EpisodeExecutionHookAction {
...
export interface EpisodeExecutionContract {
  readonly version: "1.0";
  readonly episode: number;
  readonly goal: string;
  readonly referencedHooks: ReadonlyArray<string>;
  readonly volumeMovement: ReadonlyArray<string>;
  readonly mustLand: ReadonlyArray<EpisodeExecutionDirective>;
  readonly mustAvoid: ReadonlyArray<EpisodeExecutionDirective>;
  readonly hookActions: ReadonlyArray<EpisodeExecutionHookAction>;
  readonly deferredHooks: ReadonlyArray<EpisodeDeferredHook>;
...
const SECTION_HEADINGS = {
  ...
  endChange: ["## 章尾必须发生的改变", "## Required end-of-episode change"],
  doNot: ["## 不要做", "## Do not"],
...
export function compileEpisodeExecutionContract(memo: EpisodeMemo): EpisodeExecutionContract {
  ...
  const mustLand: EpisodeExecutionDirective[] = [];
  ...
  const mustAvoid: EpisodeExecutionDirective[] = [];
  for (const text of sectionItems(payoff).flatMap(splitPayoffClauses)) {
...
export function renderEpisodeExecutionContract(
  contract: EpisodeExecutionContract,
  language: "zh" | "en" = "zh",
): string {
  const isEnglish = language === "en";
  const lines = [
    `## ${isEnglish ? "Episode execution contract" : "单集执行合同"} (${contract.fingerprint})`,
...
export function extractDirectiveAnchors(
  ...
  if (language === "en") {
    const generic = new Set([
      "episode", "current", "must", "should", "keep", "buried", "payoff", "advance",
...
function directive(
  prefix: string,
  kind: EpisodeExecutionDirectiveKind,
  text: string,
): EpisodeExecutionDirective {
...
function hookAction(
  action: EpisodeExecutionHookAction["action"],
  entry: HookLedgerEntry,
): EpisodeExecutionHookAction {
  return {
    hookId: entry.id,
    action,
...
function deferredHook(entry: HookLedgerEntry): EpisodeDeferredHook {
  return {
    hookId: entry.id,
    description: truncate(entry.descriptor),
    anchors: extractDirectiveAnchors(entry.descriptor),
...
function compileVolumeMovement(memo: EpisodeMemo, section: string): string[] {
...
function appendDirectiveSection(
  lines: string[],
  heading: string,
  directives: ReadonlyArray<EpisodeExecutionDirective>,
): void {
...
function dedupeDirectives(items: ReadonlyArray<EpisodeExecutionDirective>): EpisodeExecutionDirective[] {
...
function dedupeHookActions(items: ReadonlyArray<EpisodeExecutionHookAction>): EpisodeExecutionHookAction[] {
  const seen = new Set<string>();
  return items.filter((item) => {
...
function dedupeDeferredHooks(items: ReadonlyArray<EpisodeDeferredHook>): EpisodeDeferredHook[] {
  const seen = new Set<string>();
  return items.filter((item) => {
