...
export function parseEpisodeSummariesMarkdown(markdown: string): StoredSummary[] {
  const rows = parseMarkdownTableRows(markdown)
    .filter((row) => /^\d+$/.test(row[0] ?? ""));

  return rows.map((row) => ({
    episode: parseInt(row[0]!, 10),
    title: row[1] ?? "",
    characters: row[2] ?? "",
    events: row[3] ?? "",
    stateChanges: row[4] ?? "",
...
export function parsePendingHooksMarkdown(markdown: string): StoredHook[] {
  ...
  if (tableRows.length > 0) {
    return tableRows
      .filter((row) => isHookIdShape(normalizeHookId(row[0])))
...
export function parseCurrentStateFacts(
  ...
  if (fieldValueRows.length > 0) {
    ...
    const stateEpisode = parseInteger(episodeFromTable?.[1]) || fallbackEpisode;
  ...
  return bulletFacts.map((line, index) => ({
    ...
    object: line,
    validFromEpisode: fallbackEpisode,
    validUntilEpisode: null,
    sourceEpisode: fallbackEpisode,
  }));
}
...
export function isCurrentEpisodeLabel(label: string): boolean {
  return /^(当前剧集|current episode)$/i.test(label.trim());
}
 ...
 * mistaken for episode/progress values.
 */
function parseStrictEpisodeInteger(value: string | undefined): number {
  if (!value) return 0;
...
function parsePendingHookRow(row: ReadonlyArray<string | undefined>): StoredHook {
  ...
  const base = {
    ...
    startEpisode: parseStrictEpisodeInteger(row[1]),
    type: row[2] ?? "",
    status: row[3] ?? "open",
    lastAdvancedEpisode: parseStrictEpisodeInteger(row[4]),
...
function parseDependsOn(cell: string): ReadonlyArray<string> {
  ...
  return stripped
    ...
    .filter((item) => isHookIdShape(item));
}

/**
 * Hook ids are id-shaped tokens: an ASCII letter prefix (H001, D001, hook-7,
 * mystery-废弃十三号信号塔) or a numeric id. Chinese prose like "这条线索"
 * must never become a durable hook identity — the ledger validator and the
 * deterministic reducer both treat it as a real id and pollute state.
 */
function isHookIdShape(value: string): boolean {
