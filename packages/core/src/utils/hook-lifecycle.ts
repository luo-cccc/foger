...
export const DEFAULT_HOOK_LOOKAHEAD_EPISODES = 3;
...
export function isFuturePlannedHook(
  ...
  lookahead: number = DEFAULT_HOOK_LOOKAHEAD_EPISODES,
): boolean {
  return hook.lastAdvancedEpisode <= 0 && hook.startEpisode > episodeNumber + lookahead;
}

export function isHookWithinEpisodeWindow(
  hook: StoredHook,
  episodeNumber: number,
  recentWindow: number = 5,
  lookahead: number = DEFAULT_HOOK_LOOKAHEAD_EPISODES,
): boolean {
  const recentCutoff = Math.max(0, episodeNumber - recentWindow);

  if (hook.lastAdvancedEpisode > 0 && hook.lastAdvancedEpisode >= recentCutoff) {
    return true;
  }

  if (hook.lastAdvancedEpisode > 0) {
    return false;
  }

  if (hook.startEpisode <= 0) {
    return true;
  }

  if (hook.startEpisode >= recentCutoff && hook.startEpisode <= episodeNumber) {
    return true;
  }

  return hook.startEpisode > episodeNumber && hook.startEpisode <= episodeNumber + lookahead;
}
...
const TIMING_ALIASES: Array<[HookPayoffTiming, RegExp]> = [
  ["immediate", /^(?:立即|马上|当章|本章|本集|下一章|下一集|immediate|instant|next(?:\s+episode|\s+beat)?|right\s+away)$/i],
...
const SIGNAL_PATTERNS: Array<[HookPayoffTiming, RegExp]> = [
  ...
  ["immediate", /(当章|本章|本集|下一章|下一集|马上|立刻|即刻|immediate|next episode|right away|at once)/i],
...
export function describeHookLifecycle(params: {
  ...
  const phase = resolveHookPhase(params.episodeNumber, params.targetEpisodes);
  const age = Math.max(0, params.episodeNumber - Math.max(1, params.startEpisode));
  const lastTouchEpisode = Math.max(params.startEpisode, params.lastAdvancedEpisode);
  const dormancy = Math.max(0, params.episodeNumber - Math.max(1, lastTouchEpisode));
...
function resolveHookPhase(episodeNumber: number, targetEpisodes?: number): HookPhase {
  if (targetEpisodes && targetEpisodes > 0) {
    const progress = episodeNumber / targetEpisodes;
    if (progress >= HOOK_PHASE_THRESHOLDS.lateProgress) return "late";
    if (progress >= HOOK_PHASE_THRESHOLDS.middleProgress) return "middle";
    return "opening";
  }

  if (episodeNumber >= HOOK_PHASE_THRESHOLDS.lateEpisode) return "late";
  if (episodeNumber >= HOOK_PHASE_THRESHOLDS.middleEpisode) return "middle";
  return "opening";
}
