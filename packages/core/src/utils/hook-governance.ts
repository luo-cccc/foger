import type { HookRecord, HookStatus, EpisodeRuntimeStateDelta } from "../models/runtime-state.js";
...
export function collectStaleHookDebt(params: {
  ...
  return params.hooks
    ...
    .filter((hook) => hook.startEpisode <= params.episodeNumber)
    .filter((hook) => {
      ...
      if (params.staleAfterEpisodes !== undefined) {
        return hook.lastAdvancedEpisode <= params.episodeNumber - params.staleAfterEpisodes;
      }
...
export function classifyHookDisposition(params: {
  ...
  if (delta.hookOps.upsert.some((hook) => hook.hookId === hookId && hook.lastAdvancedEpisode === delta.episode)) {
    return "advance";
  }
