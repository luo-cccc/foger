import { EpisodeMemoSchema, type EpisodeMemo } from "../models/input-governance.js";
...
// for episodes with no extra prohibitions.
//
// Threshold rationale:
...
//   breath/transition episodes (Phase 6 sparse-memo principle).
...
//   "—" are all legitimate for a episode with no extra prohibitions; we
...
const REQUIRED_SECTIONS: ReadonlyArray<RequiredSection> = [
  ...
  { zh: "## 结尾状态", en: "## End state", minContentChars: 10 },
  {
    ...
    aliases: ["## 本章 hook 账", "## Hook ledger for this episode"],
    minContentChars: 20,
  },
...
// Internal episode aliases remain readable for persisted plans created before
...
const LEGACY_REQUIRED_SECTIONS: ReadonlyArray<RequiredSection> = [
  ...
  { zh: "## 章尾必须发生的改变", en: "## Required end-of-episode change", minContentChars: 20 },
  { zh: "## 本章 hook 账", en: "## Hook ledger for this episode", minContentChars: 20 },
  { zh: "## 卷级 KR 绑定", en: "## Volume KR binding", minContentChars: 8 },
  { zh: "## 不要做", en: "## Do not", minContentChars: 1 },
];
...
const GOAL_HEADINGS = ["## 本集目标", "## Episode goal", "## 本章目标", "## Episode goal"] as const;
...
function prependFullGoalIfNeeded(markdown: string, body: string, fullGoal: string, displayGoal: string): string {
  ...
  const heading = markdown.includes("## Episode goal") || markdown.includes("## Episode goal")
    ? "## Episode goal"
    : "## 本集目标";
  return `${heading}\n${fullGoal}\n\n${body}`;
}
 ...
 *
 * Format: plain Markdown containing a `## 本章目标` / `## Episode goal`
 * section, an optional thread-ref section, and the required memo section
 * headings.
 *
 * Strict on the LLM-owned memo sections. Caller-owned fields (episode /
 * golden-opening) come from the host, not from the model. A long episode goal
...
export function parseMemo(
  ...
  return EpisodeMemoSchema.parse({
