...
export function auditEpisodeToolDiagnostics(script: EpisodeScript, previous?: EpisodeScript): AuditIssue[] {
  ...
  if (previous && script.episode === previous.episode + 1) {
    const previousTail = previous.contract.handoffState.physical.join(" ");
    const incoming = script.contract.incomingState.physical.join(" ");
    if (previousTail && incoming && previousTail !== incoming) {
      findings.push(issue(
        "timeline-drift",
        "The incoming physical state differs from the previous handoff without an explicit intervening event.",
