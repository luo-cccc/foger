...
export const UnattendedEpisodeMetricsSchema = z.object({
...
export type UnattendedEpisodeMetrics = z.infer<typeof UnattendedEpisodeMetricsSchema>;

export const UnattendedTotalsSchema = z.object({
  episodes: z.number().int().min(0),
  hardRangeEpisodes: z.number().int().min(0),
  calls: z.number().int().min(0),
  retries: z.number().int().min(0),
  timeouts: z.number().int().min(0),
  errors: z.number().int().min(0),
...
export const UnattendedBookStateSchema = z.object({
  ...
  lastEpisodeNumber: z.number().int().min(0).optional(),
  lastFailureKind: UnattendedFailureKindSchema.optional(),
  lastError: z.string().optional(),
  nextAttemptAt: z.string().datetime().optional(),
  lastSuccessAt: z.string().datetime().optional(),
  currentMetrics: UnattendedEpisodeMetricsSchema.optional(),
  lastMetrics: UnattendedEpisodeMetricsSchema.optional(),
...
export const UnattendedSchedulerStateSchema = z.object({
  ...
  dailyEpisodeCount: z.record(z.number().int().min(0)),
});
