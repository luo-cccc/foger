...
export type EpisodeQualityStatus = "ready-for-review" | "audit-failed" | "state-degraded";
...
export interface EpisodeQualityGateDecision extends AuditIssueCounts {
  readonly status: EpisodeQualityStatus;
  readonly hardLengthPassed: boolean;
  readonly stateDegraded: boolean;
 ...
 */
export function resolveEpisodeReviewStatus(
  input: EpisodeQualityGateInput,
): EpisodeQualityGateDecision {
  ...
  const status: EpisodeQualityStatus = stateDegraded
    ? "state-degraded"
