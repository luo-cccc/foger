...
const CHAPTER_RUNTIME_LABELS: Record<string, { readonly zh: string; readonly en: string; readonly order: number }> = {
  "intent.md": { zh: "写作意图", en: "Intent", order: 10 },
  "plan.md": { zh: "章节计划", en: "Plan", order: 20 },
...
export function runtimeDiagnosticFileLabel(name: string): string | undefined {
  const chapter = name.match(/^runtime\/chapter-(\d{4})\.(.+)$/);
  if (chapter) {
    const meta = CHAPTER_RUNTIME_LABELS[chapter[2]];
    if (!meta) return undefined;
    const chapterNumber = Number.parseInt(chapter[1], 10);
    return getAppLanguage() === "en"
      ? `Chapter ${chapterNumber} ${meta.en}`
      : `第${chapterNumber}章${meta.zh}`;
  }
...
function runtimeDiagnosticSortKey(name: string): string | null {
  ...
  if (chapter) {
    ...
    return `2-${chapter[1]}-${meta.order.toString().padStart(4, "0")}`;
  }
