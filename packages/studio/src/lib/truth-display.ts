...
const EPISODE_RUNTIME_LABELS: Record<string, { readonly zh: string; readonly en: string; readonly order: number }> = {
  "intent.md": { zh: "写作意图", en: "Intent", order: 10 },
  "plan.md": { zh: "剧集计划", en: "Plan", order: 20 },
  "context.json": { zh: "上下文快照", en: "Context Snapshot", order: 30 },
  "rule-stack.yaml": { zh: "规则栈", en: "Rule Stack", order: 40 },
  "trace.json": { zh: "上下文追踪", en: "Context Trace", order: 50 },
  "claims.json": { zh: "设定工作集数据", en: "Claim Set Data", order: 60 },
  "claim-brief.md": { zh: "设定工作集摘要", en: "Claim Set Brief", order: 70 },
};

const VOLUME_RUNTIME_LABELS: Record<string, { readonly zh: string; readonly en: string; readonly order: number }> = {
  "tier2_current_arc.md": { zh: "当前叙事弧", en: "Current Arc", order: 5 },
  "volume-contracts.json": { zh: "卷级合同汇总", en: "Volume Contracts", order: 10 },
  "volume-progress.json": { zh: "卷级进度记录", en: "Volume Progress", order: 20 },
  "volume-dashboard.md": { zh: "卷级进度看板", en: "Volume Dashboard", order: 30 },
};

// Runtime diagnostics are not editable truth, but they are now intentionally
// visible in Studio for governance debugging. Give them author-facing labels so
// the truth browser does not read like a raw artifact directory.
export function runtimeDiagnosticFileLabel(name: string): string | undefined {
  const episode = name.match(/^runtime\/episode-(\d{4})\.(.+)$/);
  if (episode) {
    const meta = EPISODE_RUNTIME_LABELS[episode[2]];
    if (!meta) return undefined;
    const episodeNumber = Number.parseInt(episode[1], 10);
    return getAppLanguage() === "en"
      ? `Episode ${episodeNumber} ${meta.en}`
      : `第${episodeNumber}集${meta.zh}`;
  }

  const volumeFile = name.match(/^runtime\/(.+)$/);
  if (!volumeFile) return undefined;

  const aggregate = VOLUME_RUNTIME_LABELS[volumeFile[1]];
  if (aggregate) return getAppLanguage() === "en" ? aggregate.en : aggregate.zh;

  const volume = volumeFile[1].match(/^volume-(\d{3})\.(contract\.json|dashboard\.md)$/);
  if (!volume) return undefined;
  const volumeNumber = Number.parseInt(volume[1], 10);
  const label = volume[2] === "contract.json"
    ? { zh: "合同", en: "Contract" }
    : { zh: "看板", en: "Dashboard" };
  return getAppLanguage() === "en"
    ? `Volume ${volumeNumber} ${label.en}`
    : `第${volumeNumber}卷${label.zh}`;
}

export function truthFileDisplayLabel(name: string): string {
  return foundationFileLabel(name) ?? roleFromPath(name)?.name ?? runtimeDiagnosticFileLabel(name) ?? name;
}

function runtimeDiagnosticSortKey(name: string): string | null {
  const volumeFile = name.match(/^runtime\/(.+)$/);
  if (!volumeFile) return null;

  const aggregate = VOLUME_RUNTIME_LABELS[volumeFile[1]];
  if (aggregate) return `0-${aggregate.order.toString().padStart(4, "0")}`;

  const volume = volumeFile[1].match(/^volume-(\d{3})\.(contract\.json|dashboard\.md)$/);
  if (volume) {
    const order = volume[2] === "contract.json" ? 10 : 20;
    return `1-${volume[1]}-${order.toString().padStart(4, "0")}`;
  }

  const episode = name.match(/^runtime\/episode-(\d{4})\.(.+)$/);
  if (episode) {
    const meta = EPISODE_RUNTIME_LABELS[episode[2]];
    if (!meta) return `9-${name}`;
    return `2-${episode[1]}-${meta.order.toString().padStart(4, "0")}`;
  }

  return `9-${name}`;
}

function truthFileSortKey(name: string): string {
  const foundationIndex = FOUNDATION_FILE_ORDER.indexOf(name);
  if (foundationIndex >= 0) return `0-${foundationIndex.toString().padStart(4, "0")}-${name}`;

  const role = roleFromPath(name);
  if (role) return `1-${role.tier === "major" ? "0" : "1"}-${role.name}`;

  const runtimeKey = runtimeDiagnosticSortKey(name);
  if (runtimeKey) return `2-${runtimeKey}`;

  return `9-${name}`;
}

export function sortTruthFiles<T extends { readonly name: string }>(files: ReadonlyArray<T>): T[] {
  return [...files].sort((a, b) => truthFileSortKey(a.name).localeCompare(truthFileSortKey(b.name)));
}

// --- current_state.md ---------------------------------------------------

// At book creation current_state.md holds only an engineering seed note
// (referencing the consolidator / roles/*.当前现状 / pending_hooks startEpisode=0)
// that means nothing to a reader. The consolidator later APPENDS real state
// after each episode, so we strip the seed note and report whether any real
...
// per-episode during writing, so we detect the header-only state to show a
