...
const KNOWN_RUNTIME_REPLACEMENTS: ReadonlyArray<{
  ...
  {
    pattern: /Latest episode (\d+) is audit-failed\. Revise or rewrite that episode before continuing\./g,
    replacement: "最新第 $1 集审稿未通过（audit-failed）。继续写下一集前，请先修订或重写这一集。",
  },
  {
    pattern: /Latest episode (\d+) is state-degraded\. Repair state or rewrite that episode before continuing\./g,
    replacement: "最新第 $1 集处于状态降级（state-degraded）。继续写下一集前，请先修复状态，或重写这一集。",
  },
  {
    pattern: /Episode (\d+) is not state-degraded\./g,
    replacement: "第 $1 集并不是状态降级（state-degraded），无需按状态修复。",
  },
  {
    pattern: /Only the latest state-degraded episode can be repaired safely \(latest is (\d+)\)\./g,
    replacement: "只能安全修复最新的状态降级剧集；当前最新剧集是第 $1 集。",
  },
  {
    pattern: /State repair still failed for episode (\d+)\./g,
    replacement: "第 $1 集状态修复仍然失败。",
  },
  {
