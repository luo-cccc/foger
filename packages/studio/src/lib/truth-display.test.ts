...
describe("runtimeDiagnosticFileLabel", () => {
  it("labels canon and volume runtime files for Chinese users", () => {
    ...
    expect(runtimeDiagnosticFileLabel("runtime/episode-0003.claim-brief.md")).toBe("第3集设定工作集摘要");
  ...
  it("labels canon and volume runtime files for English users", () => {
    ...
    expect(runtimeDiagnosticFileLabel("runtime/episode-0003.claims.json")).toBe("Episode 3 Claim Set Data");
  ...
  it("returns undefined for non-runtime files or unknown runtime files", () => {
    ...
    expect(runtimeDiagnosticFileLabel("runtime/episode-0003.unknown.txt")).toBeUndefined();
  });
});
...
describe("sortTruthFiles", () => {
  it("orders foundation files, role files, and runtime diagnostics consistently", () => {
    const sorted = sortTruthFiles([
      { name: "runtime/episode-0002.trace.json" },
    ...
    expect(sorted).toEqual([
      ...
      "runtime/episode-0002.context.json",
      "runtime/episode-0002.trace.json",
    ]);
  });
});
