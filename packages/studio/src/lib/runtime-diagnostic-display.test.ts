...
describe("buildRuntimeDiagnosticViewModel", () => {
  it("summarizes episode claim working sets", () => {
    setAppLanguage("zh");

    const model = buildRuntimeDiagnosticViewModel(
      "runtime/episode-0007.claims.json",
      JSON.stringify({
        episodeNumber: 7,
    ...
    expect(model?.facts).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ label: "本集揭示", value: "1" }),
      expect.objectContaining({ label: "必须隐藏", value: "1" }),
    ]));
    expect(model?.sections).toEqual(expect.arrayContaining([
      expect.objectContaining({ title: "本集计划揭示", items: ["[C2] 师父真实身份暴露"] }),
      expect.objectContaining({ title: "使用需付代价", items: ["[C5] 强开灵脉 · 代价: 寿元折损"] }),
    ]));
  });

  it("summarizes episode claim brief markdown", () => {
    const model = buildRuntimeDiagnosticViewModel(
      "runtime/episode-0007.claim-brief.md",
      [
        "# 本集设定工作集 — 第 7 集",
        "",
        "视角：林丙",
  ...
  it("summarizes episode intent markdown", () => {
    const model = buildRuntimeDiagnosticViewModel(
      "runtime/episode-0007.intent.md",
      [
        "# Episode Intent",
        "",
        "## Goal",
    ...
    expect(model?.sections).toEqual(expect.arrayContaining([
      expect.objectContaining({ title: "剧集目标", items: ["让师徒债务公开上桌"] }),
  ...
  it("summarizes persisted plan markdown", () => {
    const model = buildRuntimeDiagnosticViewModel(
      ...
      [
        ...
        "",
        "## Planner Inputs",
        "- story/runtime/episode-0007.intent.md",
    ...
    expect(model?.sections).toEqual(expect.arrayContaining([
      expect.objectContaining({ title: "剧集目标", items: ["让债务进入明面"] }),
      expect.objectContaining({ title: "规划输入", items: ["runtime/episode-0007.intent.md", "runtime/volume-progress.json"] }),
    ]));
  });
  ...
  it("summarizes volume progress logs", () => {
    ...
    expect(model?.facts).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ label: "最新剧集", value: "5" }),
    ]));
    expect(model?.sections[0]).toEqual(expect.objectContaining({
      title: "最近进度",
      items: expect.arrayContaining([
        expect.stringContaining("第5集"),
        expect.stringContaining("第4集"),
      ]),
    }));
  });
  ...
  it("summarizes governed context package json", () => {
    const model = buildRuntimeDiagnosticViewModel(
      "runtime/episode-0007.context.json",
      JSON.stringify({
        episode: 7,
        selectedContext: [
          {
    ...
    expect(model?.sections).toEqual(expect.arrayContaining([
      expect.objectContaining({ title: "运行时上下文", items: expect.arrayContaining([expect.stringContaining("runtime/episode_memo")]) }),
    ]));
  });

  it("summarizes episode rule stack yaml", () => {
    const model = buildRuntimeDiagnosticViewModel(
      "runtime/episode-0007.rule-stack.yaml",
      [
        "layers:",
        "  - id: L1",
        "    name: hard_facts",
    ...
    expect(model?.sections).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ title: "当前激活覆盖", items: expect.arrayContaining([expect.stringContaining("episode:7/mustAvoid")]) }),
    ]));
  });

  it("summarizes episode trace json", () => {
    setAppLanguage("en");

    const model = buildRuntimeDiagnosticViewModel(
      "runtime/episode-0007.trace.json",
      JSON.stringify({
        episode: 7,
        plannerInputs: ["story/runtime/episode-0007.intent.md"],
        composerInputs: ["story/runtime/volume-progress.json", "story/runtime/episode-0007.claim-brief.md"],
        selectedSources: ["runtime/episode_memo", "runtime/volume_contract", "story/current_focus.md"],
        contextTiers: {
          protectedSources: ["runtime/episode_memo", "runtime/volume_contract"],
        ...
        compression: {
          ...
          protectedSources: ["runtime/episode_memo"],
    ...
    expect(model?.facts).toEqual(expect.arrayContaining([
      expect.objectContaining({ label: "Episode", value: "7" }),
      expect.objectContaining({ label: "Selected Tokens", value: "420" }),
      expect.objectContaining({ label: "Protected Sources", value: "2" }),
    ]));
    expect(model?.sections).toEqual(expect.arrayContaining([
      expect.objectContaining({ title: "Composer Inputs", items: ["runtime/volume-progress.json", "runtime/episode-0007.claim-brief.md"] }),
  ...
  it("parses current arc markdown into sections", () => {
    ...
    expect(model?.facts).toEqual(expect.arrayContaining([
      expect.objectContaining({ label: "更新到剧集", value: "9" }),
      expect.objectContaining({ label: "活跃支线", value: "1" }),
      expect.objectContaining({ label: "下集焦点", value: "1" }),
    ]));
    expect(model?.sections).toEqual(expect.arrayContaining([
      expect.objectContaining({ title: "当前叙事压力", items: ["支线压力集中在 S1「债务」"] }),
      expect.objectContaining({ title: "下一集规划焦点", items: ["让债务变成公开选择"] }),
    ]));
  });
