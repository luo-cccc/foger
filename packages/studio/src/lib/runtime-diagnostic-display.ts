...
export function buildRuntimeDiagnosticViewModel(
  ...
  if (/^runtime\/episode-\d{4}\.claims\.json$/.test(fileName)) {
    return buildEpisodeClaimsViewModel(trimmed);
  }
  if (/^runtime\/episode-\d{4}\.intent\.md$/.test(fileName)) {
    return buildIntentViewModel(trimmed);
  }
  if (/^runtime\/episode-\d{4}\.plan\.md$/.test(fileName)) {
    return buildPersistedPlanViewModel(trimmed);
  }
  if (/^runtime\/episode-\d{4}\.claim-brief\.md$/.test(fileName)) {
    return buildClaimBriefViewModel(trimmed);
  }
  if (/^runtime\/episode-\d{4}\.context\.json$/.test(fileName)) {
    return buildContextPackageViewModel(trimmed);
  }
  if (/^runtime\/episode-\d{4}\.rule-stack\.yaml$/.test(fileName)) {
    return buildRuleStackViewModel(trimmed);
  }
  if (/^runtime\/episode-\d{4}\.trace\.json$/.test(fileName)) {
...
function buildEpisodeClaimsViewModel(content: string): RuntimeDiagnosticViewModel | null {
  const parsed = safeJsonParse<EpisodeClaimsLike>(content);
  ...
  if (typeof parsed.episodeNumber === "number") {
    facts.push({ label: tr("剧集", "Episode"), value: String(parsed.episodeNumber) });
  }
  facts.push(
    { label: tr("可用设定", "Usable"), value: String((parsed.usable ?? []).length) },
    { label: tr("本集揭示", "Reveal Now"), value: String(revealNow.length) },
  ...
  pushSection(sections, tr("本集计划揭示", "Planned Reveal"), revealNow.map(formatClaimLine));
...
function buildIntentViewModel(content: string): RuntimeDiagnosticViewModel | null {
  ...
  const memoSectionLines = sectionLines(sections, "Episode Memo");
  ...
  const memoBody = sectionBody(sections, "Episode Memo");
  ...
  if (goal) pushSection(viewSections, tr("剧集目标", "Episode Goal"), [goal]);
  ...
  if (memoBody) pushSection(viewSections, tr("剧集备忘", "Episode Memo"), collectListItems(memoBody.split(/\r?\n/)));
...
function buildPersistedPlanViewModel(content: string): RuntimeDiagnosticViewModel | null {
  ...
  const facts: RuntimeDiagnosticFact[] = [
    { label: tr("剧集", "Episode"), value: metadata.get("Episode") ?? "-" },
  ...
  if (intentGoal) pushSection(viewSections, tr("剧集目标", "Intent Goal"), [intentGoal]);
  if (outlineNode) pushSection(viewSections, tr("大纲节点", "Outline Node"), [outlineNode]);
  if (arcContext) pushSection(viewSections, tr("叙事弧上下文", "Arc Context"), [arcContext]);
...
function buildClaimBriefViewModel(content: string): RuntimeDiagnosticViewModel | null {
  ...
  const episodeNumber = lines
    .map((line) => line.match(/第\s*(\d+)\s*集|Episode\s+(\d+)/i))
    .find(Boolean);
  const pov = lines
  ...
  const usable = findMarkdownSectionItems(sections, "本集可用设定", "writer 可渲染", "usable");
  const revealNow = findMarkdownSectionItems(sections, "本集计划揭示", "reveal");
  const mustHide = findMarkdownSectionItems(sections, "本集必须隐藏", "hide");
  ...
  const facts: RuntimeDiagnosticFact[] = [];
  const parsedEpisode = episodeNumber?.[1] ?? episodeNumber?.[2];
  if (parsedEpisode) facts.push({ label: tr("剧集", "Episode"), value: parsedEpisode });
  ...
  const viewSections: RuntimeDiagnosticSection[] = [];
  pushSection(viewSections, tr("本集计划揭示", "Planned Reveal"), revealNow);
...
function buildSingleVolumeContractViewModel(content: string): RuntimeDiagnosticViewModel | null {
  ...
  const facts: RuntimeDiagnosticFact[] = [
    ...
    {
      label: tr("剧集范围", "Episode Range"),
      value: contract.episodeStart && contract.episodeEnd
...
function buildVolumeProgressViewModel(content: string): RuntimeDiagnosticViewModel | null {
  ...
  const entries = (parsed?.entries ?? [])
    .filter((entry) => typeof entry.episode === "number")
  ...
  const facts: RuntimeDiagnosticFact[] = [
    ...
    { label: tr("最新剧集", "Latest Episode"), value: latest?.episode ? String(latest.episode) : "-" },
  ];
  ...
  const groupedByVolume = volumeIds
    .map((volumeId) => {
      ...
      return {
        ...
        items: [
          `${tr("记录剧集", "Tracked episodes")} ${volumeEntries.map((entry) => `ep${entry.episode}`).join(", ")}`,
...
function buildCurrentArcViewModel(content: string): RuntimeDiagnosticViewModel | null {
  const lines = content.split(/\r?\n/);
  const updatedEpisode = lines
    .map((line) => line.match(/^-+\s*updated_for_episode:\s*(\d+)/i)?.[1])
    .find(Boolean);
  const sections = parseLabeledBulletSections(content);

  const facts: RuntimeDiagnosticFact[] = [];
  if (updatedEpisode) {
    facts.push({ label: tr("更新到剧集", "Updated For"), value: updatedEpisode });
  }
  facts.push(
    ...
    { label: tr("下集焦点", "Next Focus"), value: String(sections.get("下一集规划焦点")?.length ?? 0) },
  );
  ...
  pushSection(viewSections, tr("下一集规划焦点", "Next Planning Focus"), sections.get("下一集规划焦点") ?? []);
...
function buildContextPackageViewModel(content: string): RuntimeDiagnosticViewModel | null {
  ...
  const facts: RuntimeDiagnosticFact[] = [
    { label: tr("剧集", "Episode"), value: parsed?.episode ? String(parsed.episode) : "-" },
...
function buildTraceViewModel(content: string): RuntimeDiagnosticViewModel | null {
  const parsed = safeJsonParse<EpisodeTraceLike>(content);
  if (!parsed) return null;
...
function formatVolumeProgressLine(entry: NonNullable<VolumeProgressLike["entries"]>[number]): string {
  const parts = [
    `${tr("第", "Episode ")}${entry.episode}${tr("集", "")}`,
...
function summarizeVolumeDashboardSection(section: MarkdownSection): { title: string; items: string[]; krRows: Array<Record<string, string>> } {
  ...
  for (const entry of recentEntries.slice(0, 3)) {
    items.push(`${tr("最近剧集", "Recent")}: ${entry}`);
  }
...
function summarizeCompression(compression: NonNullable<EpisodeTraceLike["compression"]>): string[] {
  const items = [
