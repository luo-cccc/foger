...
export type HashRoute =
  ...
  | { page: "episode"; bookId: string; episodeNumber: number }
  | { page: "analytics"; bookId: string }
  | { page: "truth"; bookId: string }
  | { page: "daemon" }
  | { page: "logs" }
  | { page: "genres" }
  | { page: "import"; tab?: "episodes" | "canon" }
  | { page: "doctor"; operationId?: string };
...
function parseHash(hash: string): HashRoute {
  ...
  const importMatch = path.match(/^import\/(episodes|canon)$/);
  if (importMatch) return { page: "import", tab: importMatch[1] as "episodes" | "canon" };
  if (path === "book/new") return { page: "book-create" };

  const doctorMatch = path.match(/^doctor(?:\?(.+))?$/);
  if (doctorMatch) {
    const operationId = new URLSearchParams(doctorMatch[1] ?? "").get("operationId")?.trim();
    return { page: "doctor", ...(operationId ? { operationId } : {}) };
  }
...
function routeToHash(route: HashRoute): string {
  switch (route.page) {
    ...
    case "doctor": return route.operationId
      ? `#/doctor?operationId=${encodeURIComponent(route.operationId)}`
      : "#/doctor";
    default: return "";
  }
}
...
const HASH_PAGES = new Set(["dashboard", "chat", "book", "book-settings", "book-create", "services", "project-settings", "service-detail", "import", "doctor"]);
