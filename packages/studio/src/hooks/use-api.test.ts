  ...
  it("localizes known runtime errors before throwing", async () => {
    const fetchImpl = vi.fn(async () =>
      new Response(JSON.stringify({
        error: "Latest episode 1 is state-degraded. Repair state or rewrite that episode before continuing.",
      }), {
        status: 409,
        headers: { "Content-Type": "application/json" },
      }),
...
describe("deriveInvalidationPaths", () => {
  ...
  it("refreshes both collections and the current book after book mutations", () => {
    ...
    expect(deriveInvalidationPaths("/books/demo/episodes/3/approve")).toEqual([
      "/api/v1/books",
      "/api/v1/books/demo",
    ]);
  });
