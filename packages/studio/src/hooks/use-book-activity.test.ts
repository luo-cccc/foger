...
describe("deriveBookActivity", () => {
  ...
  it("clears writing state after completion or error", () => {
    const completed: ReadonlyArray<SSEMessage> = [
      msg("write:start", { bookId: "alpha" }, 1),
      msg("write:complete", { bookId: "alpha", episodeNumber: 2 }, 2),
    ];
  ...
  it("tracks asynchronous revision operations until their terminal event", () => {
    ...
    const completed: ReadonlyArray<SSEMessage> = [
      ...active,
      msg("revise:complete", { bookId: "alpha", requestId: "revise-1", episode: 3 }, 2),
    ];
...
describe("deriveActiveBookIds", () => {
  it("returns only books with in-flight background work", () => {
    const messages: ReadonlyArray<SSEMessage> = [
      ...
      msg("write:complete", { bookId: "alpha", episodeNumber: 2 }, 3),
...
describe("shouldRefetchBookView", () => {
  it("refreshes the book detail view after terminal background jobs for that book", () => {
    ...
    expect(shouldRefetchBookView(msg("rewrite:complete", { bookId: "alpha", episodeNumber: 3 }, 1), "alpha")).toBe(true);
    expect(shouldRefetchBookView(msg("write:cancelled", { bookId: "alpha", requestId: "req-1" }, 1), "alpha")).toBe(true);
    expect(shouldRefetchBookView(msg("resync:complete", { bookId: "alpha", episode: 3 }, 1), "alpha")).toBe(true);
    expect(shouldRefetchBookView(msg("revise:error", { bookId: "alpha", error: "bad" }, 1), "alpha")).toBe(true);
    expect(shouldRefetchBookView(msg("audit:complete", { bookId: "alpha", episode: 3, passed: true }, 1), "alpha")).toBe(true);
    expect(shouldRefetchBookView(msg("audit:start", { bookId: "alpha", episode: 3 }, 1), "alpha")).toBe(false);
    expect(shouldRefetchBookView(msg("rewrite:complete", { bookId: "beta" }, 1), "alpha")).toBe(false);
  });
});

describe("shouldRefetchBookCollections", () => {
  it("refreshes book lists for create/delete and episode-changing terminal events", () => {
...
describe("shouldRefetchDaemonStatus", () => {
  it("refreshes daemon status for daemon terminal events", () => {
    ...
    expect(shouldRefetchDaemonStatus(msg("daemon:episode", {}, 1))).toBe(false);
  });
});
...
describe("applyBookCollectionEvent", () => {
  it("upserts a created book from the event payload without requiring a refetch", () => {
    const books: SidebarBookSummary[] = [
      { id: "alpha", title: "Alpha", genre: "urban", status: "active", episodesWritten: 3 },
    ];

    expect(applyBookCollectionEvent(books, msg("book:created", {
      bookId: "beta",
      book: { id: "beta", title: "Beta", genre: "xuanhuan", status: "outlining", episodesWritten: 0 },
    }, 1))).toEqual([
      { id: "alpha", title: "Alpha", genre: "urban", status: "active", episodesWritten: 3 },
      { id: "beta", title: "Beta", genre: "xuanhuan", status: "outlining", episodesWritten: 0 },
    ]);
  });
