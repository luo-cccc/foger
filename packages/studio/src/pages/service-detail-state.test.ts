...
describe("rehydrateServiceConnectionStatus", () => {
  it("detects a saved key without returning it to the page", async () => {
...
describe("saveServiceConfig", () => {
  ...
  it("reuses a stored API key without exposing it to the page", async () => {
    const calls: string[] = [];
    const bodies: unknown[] = [];
    const fetchJsonImpl = vi.fn(async (path: string, init?: { body?: string }) => {
      calls.push(path);
  ...
  it("passes an explicit custom model through validation and saves it as default", async () => {
    const calls: string[] = [];
    const bodies: unknown[] = [];
    const fetchJsonImpl = vi.fn(async (path: string, init?: { body?: string }) => {
      calls.push(path);
      if (init?.body) bodies.push(JSON.parse(init.body));
      if (path === "/services/custom%3AArkPlan/test") {
        return {
          ok: true,
      ...
      if (path === "/services/custom%3AArkPlan/secret") return { ok: true };
      if (path === "/services/config") return { ok: true };
      throw new Error(`unexpected path: ${path}`);
    });

    const result = await saveServiceConfig({
      effectiveServiceId: "custom:ArkPlan",
      serviceId: "custom",
      isCustom: true,
      resolvedCustomName: "ArkPlan",
      apiKey: "ark-live",
      baseUrl: "https://ark.cn-beijing.volces.com/api/plan/v3",
      apiFormat: "chat",
      stream: false,
    ...
    expect(calls).toEqual([
      "/services/custom%3AArkPlan/test",
      "/services/custom%3AArkPlan/secret",
      "/services/config",
    ]);
    expect(bodies).toEqual([
      {
    ...
    expect(result).toMatchObject({
      detectedModel: "deepseek-v4-pro",
      status: { state: "connected", models: [{ id: "deepseek-v4-pro" }] },
    });
  });
});
