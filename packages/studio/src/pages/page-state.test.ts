...
describe("defaultEpisodeDurationForLanguage", () => {
  it("uses a 150-second target for chinese and english projects", () => {
    expect(defaultEpisodeDurationForLanguage("zh")).toBe("150");
    expect(defaultEpisodeDurationForLanguage("en")).toBe("150");
  });
});
  ...
  it("groups the draft by creation stages so users do not create from a mixed blob", () => {
    ...
    expect(stages.map((stage) => ({
    ...
    }))).toEqual([
      {
        ...
        label: "基础信息",
        status: "complete",
        rows: ["title", "genre", "platform", "targetEpisodes", "episodeDurationSeconds"],
        missing: [],
      },
      {
        key: "world",
