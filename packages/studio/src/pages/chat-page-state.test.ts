...
describe("getBookContinuationBlock", () => {
  it("blocks continuation when the latest episode failed audit", () => {
    expect(getBookContinuationBlock([
      { episodeNumber: 1, status: "ready-for-review" },
      { episodeNumber: 2, status: "audit-failed" },
    ])).toEqual({ episodeNumber: 2, status: "audit-failed" });
  });

  it("allows continuation when the latest episode is reviewable", () => {
    expect(getBookContinuationBlock([
      { episodeNumber: 2, status: "audit-failed" },
      { episodeNumber: 3, status: "ready-for-review" },
    ])).toBeNull();
  });
});
...
describe("pickProjectChatSessionId", () => {
  it("prefers the newest project chat session that already has messages", () => {
    expect(pickProjectChatSessionId([
      ...
      { sessionId: "active-project-session", messageCount: 3 },
      { sessionId: "older-session", messageCount: 1 },
    ])).toBe("active-project-session");
  });

  it("ignores non-chat session kinds when restoring the project chat session", () => {
    expect(pickProjectChatSessionId([
      { sessionId: "book-session", sessionKind: "book", messageCount: 4 },
