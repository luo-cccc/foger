...
export interface ChatBookEpisodeStatus {
...
export function getBookContinuationBlock(
  episodes: ReadonlyArray<ChatBookEpisodeStatus>,
): ChatBookContinuationBlock | null {
  const latest = episodes.reduce<ChatBookEpisodeStatus | null>(
    (current, episode) => !current || episode.episodeNumber > current.episodeNumber ? episode : current,
    null,
  );
  if (latest?.status !== "audit-failed" && latest?.status !== "state-degraded") {
    return null;
  }
  return { episodeNumber: latest.episodeNumber, status: latest.status };
}
