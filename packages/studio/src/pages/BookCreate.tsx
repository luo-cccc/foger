...
const PAGE_COPY: Record<"zh" | "en", PlatformCopy> = {
  zh: {
    ...
    briefPlaceholder: "写清世界观、主角、目标、核心冲突和第一阶段方向。例如：架空都市，落魄调音师被卷入旧案，想自证清白却越陷越深。",
    createBook: "创建书籍",
    creatingBook: "创建中…",
    ...
    promptPlaceholderFollowup: "例如：世界观改成架空仙侠；女主不要太早出场；卷一先查旧案再揭开真相。",
    submit: "更新草案",
    submitting: "处理中…",
    create: "按当前草案建书",
    creating: "创建中…",
  ...
  en: {
    ...
    titleLabel: "Title",
    titlePlaceholder: "Example: Echoes of the Star Sea",
    genreLabel: "Genre",
...
const DRAFT_STAGE_LABELS: Record<"zh" | "en", Record<string, string>> = {
  ...
  en: {
    ...
    episodeDurationSeconds: "Duration per Episode (seconds)",
    worldPremise: "World",
...
const DRAFT_STAGE_FIELDS: ReadonlyArray<{
  ...
  { key: "basic", fields: ["title", "genre", "platform", "targetEpisodes", "episodeDurationSeconds", "language"] },
...
export function BookCreate({ nav, theme, t }: { nav: Nav; theme: Theme; t: TFunction }) {
  ...
  const applyDraftToForm = () => {
    ...
    setForm((current) => ({
      ...
      targetEpisodes: draft.targetEpisodes ? String(draft.targetEpisodes) : current.targetEpisodes,
      episodeDurationSeconds: draft.episodeDurationSeconds ? String(draft.episodeDurationSeconds) : current.episodeDurationSeconds,
