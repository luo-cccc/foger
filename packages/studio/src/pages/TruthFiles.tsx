...
export function TruthFiles({ bookId, nav, theme, t }: { bookId: string; nav: Nav; theme: Theme; t: TFunction }) {
  ...
  const latestEpisode = latestRuntimeEpisode(truthFiles);
  ...
  const { data: episodeIntentOverviewData } = useApi<{ file: string; content: string | null }>(
    targetName("episode_intent") ? `/books/${bookId}/truth/${targetName("episode_intent")}` : "",
  );
  const { data: episodeClaimBriefOverviewData } = useApi<{ file: string; content: string | null }>(
    targetName("episode_claim_brief") ? `/books/${bookId}/truth/${targetName("episode_claim_brief")}` : "",
  );
  const { data: episodeRuleStackOverviewData } = useApi<{ file: string; content: string | null }>(
    targetName("episode_rule_stack") ? `/books/${bookId}/truth/${targetName("episode_rule_stack")}` : "",
  );
  const { data: episodeTraceOverviewData } = useApi<{ file: string; content: string | null }>(
    targetName("episode_trace") ? `/books/${bookId}/truth/${targetName("episode_trace")}` : "",
  );
  ...
  const overviewCards = [
    ...
    buildOverviewCard(targetName("episode_intent"), episodeIntentOverviewData?.content),
    buildOverviewCard(targetName("episode_claim_brief"), episodeClaimBriefOverviewData?.content),
    buildOverviewCard(targetName("episode_rule_stack"), episodeRuleStackOverviewData?.content),
    buildOverviewCard(targetName("episode_trace"), episodeTraceOverviewData?.content),
    ...
    return (
      <div className="space-y-5">
        <div className="grid gap-px overflow-hidden rounded-md border border-border/50 bg-border/40 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="bg-background px-3 py-2.5">
            <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("truth.latestEpisode")}</div>
            <div className="mt-1 text-sm font-medium text-foreground">{latestEpisode ? Number.parseInt(latestEpisode, 10) : "-"}</div>
          </div>
          <div className="bg-background px-3 py-2.5">
            ...
            <>
              ...
              {isRuntimeDiagnostic && (
                ...
                >
                  ...
                  <div className="mt-1">
                    这里展示写作时生成的上下文、设定工作集、篇章合同或进度看板。它只用于追溯系统如何治理剧集，不作为可编辑设定。
                  </div>
                </div>
              )}
...
function overviewSectionTitle(section: GovernanceOverviewSection, t: TFunction): string {
  return section.id === "volume" ? t("truth.volumeGroup") : t("truth.episodeGroup");
}
