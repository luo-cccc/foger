...
export function EpisodeReader({ bookId, episodeNumber, nav, theme, t }: {
  bookId: string;
  ...
  const { data, loading, error, refetch } = useApi<EpisodeData>(
    `/books/${bookId}/episodes/${episodeNumber}`,
  );
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await fetchJson(`/books/${bookId}/episodes/${episodeNumber}`, {
        method: "PUT",
  ...
  const title = titleLine?.replace(/^#\s*/, "") ?? `Episode ${episodeNumber}`;
  const body = lines
  ...
  const handleApprove = async () => {
    try {
      await postApi(`/books/${bookId}/episodes/${episodeNumber}/approve`);
      nav.toBook(bookId);
    } catch (e) {
  ...
  const handleReject = async () => {
    try {
      await postApi(`/books/${bookId}/episodes/${episodeNumber}/reject`);
      nav.toBook(bookId);
    } catch (e) {
      ...
      <div className="paper-sheet rounded-2xl p-8 md:p-16 lg:p-24 shadow-2xl shadow-primary/5 min-h-[80vh] relative overflow-hidden">
        ...
        <header className="mb-16 text-center">
          ...
          <div className="mt-8 flex items-center justify-center gap-4 text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60">
            ...
            <span className="text-border">·</span>
            <span>{episodeNumber.toString().padStart(2, '0')}</span>
          </div>
        </header>
        ...
        <footer className="mt-24 pt-12 border-t border-border/20 flex flex-col items-center gap-6 text-center">
          ...
          <p className="text-[10px] uppercase tracking-widest text-muted-foreground/40 font-bold">{t("reader.endOfEpisode")}</p>
        </footer>
      </div>
