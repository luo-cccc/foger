...
interface GenreDetail {
  readonly profile: {
    ...
    readonly episodeTypes: ReadonlyArray<string>;
    ...
    <div className="space-y-4">
      ...
      <div>
        <label className="text-xs text-muted-foreground uppercase tracking-wide">
          {t("genre.episodeTypes")} ({t("genre.commaSeparated")})
        </label>
        <input
          type="text"
          value={form.episodeTypes}
          onChange={(e) => set("episodeTypes", e.target.value)}
          className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
        />
      </div>
...
export function GenreManager({ nav, theme, t }: { nav: Nav; theme: Theme; t: TFunction }) {
  ...
  const openEditForm = () => {
    ...
    setForm({
      ...
      episodeTypes: detail.profile.episodeTypes.join(", "),
  ...
  const handleCreate = async () => {
    try {
      await postApi("/genres/create", {
        ...
        episodeTypes: parseCommaSeparated(form.episodeTypes),
        ...
        <div className={`border ${c.cardStatic} rounded-lg p-6 min-h-[400px]`}>
          {validSelected && detail ? (
            <div className="space-y-6">
              ...
              <div>
                <div className="text-xs text-muted-foreground uppercase tracking-wide mb-2">{t("genre.episodeTypes")}</div>
                <div className="flex gap-2 flex-wrap">
                  {detail.profile.episodeTypes.map((ct) => (
