...
interface Nav {
  ...
  toServices: () => void;
  toImport: (tab?: "episodes" | "canon") => void;
}
...
interface BookContinuationPayload {
  readonly episodes?: ReadonlyArray<{ readonly episodeNumber: number; readonly status: string }>;
}
...
export function ChatPage({ activeBookId, mode = activeBookId ? "book" : "book-create", nav, theme, t, sse }: ChatPageProps) {
  ...
  useEffect(() => {
    ...
    void fetchJson<BookContinuationPayload>(`/books/${encodeURIComponent(activeBookId)}`)
      .then((payload) => {
        if (!cancelled) {
          setContinuationBlock(getBookContinuationBlock(payload.episodes ?? []));
        }
      })
      .catch(() => {
  ...
  const handleProposedAction = async (details: ProposedActionDetails) => {
    ...
    if (details.targetRoute) {
      if (details.targetRoute === "import:episodes") nav.toImport("episodes");
            ...
            <QuickActions
              ...
              writeNextBlockedTitle={continuationBlock
                ? continuationBlock.status === "audit-failed"
                  ? (isZh
                      ? `第 ${continuationBlock.episodeNumber} 集审稿未通过，请先修订或重写。`
                      : `Episode ${continuationBlock.episodeNumber} failed audit. Revise or rewrite it first.`)
                  : (isZh
                      ? `第 ${continuationBlock.episodeNumber} 集状态结算失败，请先修复状态或重写。`
                      : `Episode ${continuationBlock.episodeNumber} has degraded state. Repair or rewrite it first.`)
                : undefined}
            />
          </div>
        </div>
