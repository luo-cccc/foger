...
export function ImportManager({ nav, theme, t, initialTab }: { nav: Nav; theme: Theme; t: TFunction; initialTab?: Tab }) {
  ...
  const [tab, setTab] = useState<Tab>(initialTab ?? "episodes");
  ...
  const handleImportEpisodes = async () => {
    ...
    setStatus("");
    try {
      const data = await fetchJson<{ importedCount?: number }>(`/books/${chBookId}/import/episodes`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: chText, splitRegex: chSplitRegex || undefined }),
      });
      setStatus(`Imported ${data.importedCount} episodes`);
    } catch (e) {
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
    { id: "episodes", label: t("import.episodes"), icon: <FileInput size={14} /> },
