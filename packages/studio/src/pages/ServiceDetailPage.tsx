...
export function ServiceDetailPage({ serviceId, nav }: { serviceId: string; nav: Nav }) {
  ...
  useEffect(() => {
    ...
    void rehydrateServiceConnectionStatus({
      ...
      .then((result) => {
        ...
        setHasStoredApiKey(result.hasStoredApiKey);
  ...
  const handleTest = async () => {
    ...
    if (!trimmedKey && !hasStoredApiKey && !isCustom) {
    ...
    try {
      ...
      if (result.ok) {
        ...
        if (result.selectedModel) setModel(result.selectedModel);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.status.state === "connected") {
        ...
        setDetectedModel(result.detectedModel);
        if (result.detectedModel) setModel(result.detectedModel);
        setDetectedConfig(result.detectedConfig);
        setStoreModels(effectiveServiceId, result.status.models);
        setStatus(result.status);
      } else {
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
            <Field label={tr("服务名称", "Service name")}>
              <input data-testid="service-custom-name" type="text" value={customName} onChange={(e) => setCustomName(e.target.value)}
            ...
            <Field label="Base URL">
              <input data-testid="service-base-url" type="text" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)}
        ...
        <Field label="API Key">
          <div className="relative">
            <input
              data-testid="service-api-key"
              type={showKey ? "text" : "password"} value={apiKey}
              onChange={(e) => {
                apiKeyDirtyRef.current = true;
                setApiKey(e.target.value);
              }} placeholder={hasStoredApiKey
                ? tr("已配置；留空可继续使用，输入新值以替换", "Configured; leave blank to keep it or enter a replacement")
                : "sk-..."}
        ...
        <Field label={tr("默认/测试模型", "Default/test model")}>
          <input
            type="text"
            value={model}
            onChange={(e) => {
            ...
            placeholder={tr("例如：deepseek-v4-pro", "e.g. deepseek-v4-pro")}
        ...
        <div className="flex items-center gap-2">
          <button data-testid="service-test-connection" onClick={handleTest} disabled={isBusy}
          ...
          <button data-testid="service-save" onClick={handleSave} disabled={isBusy}
          ...
          {status.state === "connected" && (
            <span data-testid="service-connection-success" className="text-xs text-emerald-500">
          ...
          {status.state === "error" && (
            <span data-testid="service-connection-error" className="text-xs text-destructive">{status.message}</span>
          )}
          {status.state === "saved" && (
