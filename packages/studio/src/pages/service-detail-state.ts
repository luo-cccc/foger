...
export async function rehydrateServiceConnectionStatus(args: {
  ...
  const secret = await fetchJsonImpl<{ apiKey?: string; configured?: boolean }>(
  ...
  return {
    apiKey,
    hasStoredApiKey: secret.configured === true,
    status: { state: "idle" },
    detectedModel: "",
    detectedConfig: null,
  };
...
export async function saveServiceConfig(args: {
  ...
  if (!trimmedKey && !args.hasStoredApiKey && !args.isCustom) {
    return {
  ...
  const detectedModel = (probe.selectedModel ?? trimmedModel) || args.detectedModel;
