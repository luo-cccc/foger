...
const PIPELINE_STAGES: Record<string, ReadonlyArray<BilingualLabel>> = {
  writer: [
    { zh: "准备剧集输入", en: "Prepare episode input" },
    { zh: "撰写剧集草稿", en: "Write episode draft" },
    { zh: "落盘最终剧集", en: "Save final episode" },
    { zh: "生成最终真相文件", en: "Generate final truth files" },
    { zh: "校验真相文件变更", en: "Validate truth file changes" },
    { zh: "同步记忆索引", en: "Sync memory index" },
    { zh: "更新剧集索引与快照", en: "Update episode index and snapshot" },
  ],
  architect: [
...
function isTextAttachment(filename: string, mimeType: string): boolean {
  ...
  return mimeType.startsWith("text/")
    || [".json", ".md", ".markdown"].some((suffix) => lower.endsWith(suffix));
}
...
function shouldRunDirectWriteNext(args: {
  ...
  if (args.actionSource === "free-text") return isExplicitWriteEpisodeCommand(args.instruction);
...
function validateAgentActionExecution(args: {
  ...
  if (
    ...
    return pick(
      lang,
      "模型声称已完成下一集，但没有实际调用写作工具。请重试；如果仍失败，请检查模型是否支持工具调用。",
      "The model claimed the next episode is done, but it never called the writing tool. Retry; if it keeps failing, check whether the model supports tool calls.",
    );
  }
...
async function executeConfirmedProductionAction(args: {
  ...
  const params = {
    ...
    ...(payload?.targetEpisodes ? { targetEpisodes: payload.targetEpisodes } : {}),
    ...(payload?.episodeDurationSeconds ? { episodeDurationSeconds: payload.episodeDurationSeconds } : {}),
  };
...
async function loadStudioBookListSummary(
  ...
  const nextEpisode = await state.getNextEpisodeNumber(bookId);
  return { ...book, episodesWritten: nextEpisode - 1 };
}
...
function rethrowCoreMutationApiError(error: unknown): void {
  ...
  if (error instanceof EpisodeMutationEpisodeNotFoundError) {
    throw new ApiError(404, error.code, error.message);
  }
...
type EpisodeReviewMode = "auto" | "manual";

function normalizeEpisodeReviewMode(mode: unknown): EpisodeReviewMode {
  return mode === "manual" ? "manual" : "auto";
}

function readProjectEpisodeReviewMode(config: Record<string, unknown>): EpisodeReviewMode {
  const writing = config.writing && typeof config.writing === "object" && !Array.isArray(config.writing)
    ...
    : {};
  return normalizeEpisodeReviewMode(writing.reviewMode);
}

function readBookEpisodeReviewMode(rawBook: Record<string, unknown>): EpisodeReviewMode | undefined {
...
async function resolveBookEpisodeReviewMode(root: string, bookId: string | undefined, projectMode: EpisodeReviewMode): Promise<EpisodeReviewMode> {
  if (!bookId || !isSafeBookId(bookId)) return projectMode;
  try {
    const rawBook = await loadRawBookConfig(root, bookId);
    return readBookEpisodeReviewMode(rawBook) ?? projectMode;
  } catch {
    return projectMode;
  }
}
...
export function createStudioServer(
  ...
  async function buildPipelineConfig(
    ...
    const projectReviewMode = readProjectEpisodeReviewMode(currentConfig as unknown as Record<string, unknown>);
    const episodeReviewMode = await resolveBookEpisodeReviewMode(root, overrides?.bookIdForSettings, projectReviewMode);
  ...
  app.get("/api/v1/books/:id", async (c) => {
    ...
    try {
      ...
      const [episodeIndex, nextEpisode, performance] = await Promise.all([
        state.loadEpisodeIndex(id),
        state.getNextEpisodeNumber(id),
        loadStudioEpisodePerformance(state.bookDir(id)),
      ]);
      const episodes = episodeIndex.map((episode) => ({
        ...episode,
        ...(performance.byEpisode.get(episode.episodeNumber ?? episode.episodeNumber)
          ? { performanceReport: performance.byEpisode.get(episode.episodeNumber ?? episode.episodeNumber) }
          : {}),
      }));
      return c.json({ book, episodes, nextEpisode, ...(performance.aggregate ? { seriesPerformance: performance.aggregate } : {}) });
    } catch {
  ...
  app.post("/api/v1/books/create", async (c) => {
    ...
    processProjectInteractionRequest({
      ...
      request: {
        ...
        episodeDurationSeconds: body.episodeDurationSeconds ?? 150,
        blurb: body.blurb,
      },
      tools,
    }).then(
  ...
  const TRUTH_FLAT_FILES = [
    ...
    "particle_ledger.md", "pending_hooks.md", "episode_summaries.md",
  ...
  const RUNTIME_DIAGNOSTIC_FILE_RE = /^runtime\/(?:episode-\d{4}\.(?:intent\.md|plan\.md|context\.json|rule-stack\.yaml|trace\.json|claims\.json|claim-brief\.md)|recovery\.json|tier2_current_arc\.md|volume-contracts\.json|volume-progress\.json|volume-dashboard\.md|volume-\d{3}\.(?:contract\.json|dashboard\.md))$/;
  ...
  app.get("/api/v1/books/:id/analytics", async (c) => {
    const id = c.req.param("id");
    try {
      const episodes = await state.loadEpisodeIndex(id);
      return c.json(computeAnalytics(id, episodes));
    } catch {
      return c.json({ error: `Book "${id}" not found` }, 404);
    }
  });
  ...
  app.post("/api/v1/books/:id/write-next", async (c) => {
    const id = c.req.param("id");
    const body = await c.req.json<{ episodeDurationSeconds?: number }>().catch(() => ({ episodeDurationSeconds: undefined }));
    ...
    launchStudioOperation({
      ...
      run: async () => {
        ...
        return pipeline.writeNextEpisode(id, body.episodeDurationSeconds);
      },
      completeData: (result) => ({
        episodeNumber: result.episodeNumber,
        status: result.status,
  ...
  app.post("/api/v1/books/:id/draft", async (c) => {
    const id = c.req.param("id");
    const body = await c.req.json<{ episodeDurationSeconds?: number; context?: string }>().catch(() => ({ episodeDurationSeconds: undefined, context: undefined }));
    ...
    launchStudioOperation({
      ...
      run: async () => {
        ...
        return pipeline.writeDraft(id, body.context, body.episodeDurationSeconds);
      },
      completeData: (result) => ({
        episodeNumber: result.episodeNumber,
        title: result.title,
  ...
  app.get("/api/v1/books/:id/eval", async (c) => {
    const id = c.req.param("id");
    const episodes = c.req.query("episodes");
    try {
      return c.json(await evaluateBookQuality({ state, bookId: id, episodes }));
    } catch (e) {
  ...
  app.post("/api/v1/books/:id/repair-state/:episode", async (c) => {
    const id = c.req.param("id");
    const episodeNum = normalizePositiveIntegerParam(c.req.param("episode"), "episode number");
    ...
    launchStudioOperation({
      ...
      run: async () => {
        ...
        return pipeline.repairEpisodeState(id, episodeNum);
      },
      completeData: (result) => ({ episode: episodeNum, status: result.status, resultStatus: result.status }),
    });
    return c.json({ status: "repairing", bookId: id, episode: episodeNum, requestId: operation.requestId }, 202);
  });
  ...
  app.post("/api/v1/books/:id/episodes/:num/approve", async (c) => {
    ...
    try {
      ...
      return c.json({ ok: true, episodeNumber: result.episodeNumber, status: result.status });
    } catch (error) {
      rethrowCoreMutationApiError(error);
      return c.json({ error: String(error) }, 500);
    }
  ...
  app.post("/api/v1/daemon/start", async (c) => {
    ...
    try {
      ...
      const scheduler = new Scheduler({
        ...
        episodesPerCycle: currentConfig.daemon.episodesPerCycle,
        retryDelayMs: currentConfig.daemon.retryDelayMs,
        cooldownAfterEpisodeMs: currentConfig.daemon.cooldownAfterEpisodeMs,
        maxEpisodesPerDay: currentConfig.daemon.maxEpisodesPerDay,
        onEpisodeComplete: (bookId, episode, status) => {
          broadcast("daemon:episode", { bookId, episode, status });
        },
  ...
  app.post("/api/v1/books/:id/audit/:episode", async (c) => {
    const id = c.req.param("id");
    const episodeNum = normalizePositiveIntegerParam(c.req.param("episode"), "episode number");

    broadcast("audit:start", { bookId: id, episode: episodeNum });
    try {
      const pipeline = new PipelineRunner(await buildPipelineConfig({ bookIdForTelemetry: id }));
      ...
      broadcast("audit:complete", { bookId: id, episode: episodeNum, passed: result.passed });
      return c.json(result);
    } catch (e) {
      rethrowCoreMutationApiError(e);
  ...
  app.post("/api/v1/books/:id/revise/:episode", async (c) => {
    const id = c.req.param("id");
    const episodeNum = normalizePositiveIntegerParam(c.req.param("episode"), "episode number");
    const bookDir = state.bookDir(id);
    ...
    const episodesDir = join(bookDir, "episodes");
    const files = await readdir(episodesDir);
    const paddedNum = String(episodeNum).padStart(4, "0");
    ...
    launchStudioOperation({
      ...
      run: async () => {
        ...
        return pipeline.reviseDraft(id, episodeNum, normalizedMode.data);
      },
      completeData: (result) => ({
    ...
    return c.json({ status: "revising", bookId: id, episode: episodeNum, requestId: operation.requestId }, 202);
  });
  ...
  app.get("/api/v1/books/:id/export", async (c) => {
    const id = c.req.param("id");
    const format = (c.req.query("format") ?? "screenplay-md") as string;
    const approvedOnly = c.req.query("approvedOnly") === "true";

    try {
      const artifact = await buildExportArtifact(state, id, {
        format: format as "screenplay-md" | "screenplay-json" | "dialogue",
        approvedOnly,
      });
  ...
  app.post("/api/v1/books/:id/export-save", async (c) => {
    const id = c.req.param("id");
    const { format, approvedOnly } = await c.req.json<{ format?: string; approvedOnly?: boolean }>().catch(() => ({ format: "screenplay-md", approvedOnly: false }));
    const fmt = format ?? "screenplay-md";

    try {
      const pipeline = new PipelineRunner(await buildPipelineConfig());
      const tools = createInteractionToolsFromDeps(pipeline, state);
      const bookDir = state.bookDir(id);
      const extension = fmt === "screenplay-json" ? "json" : fmt === "dialogue" ? "txt" : "md";
      const outputPath = join(bookDir, `${id}.${extension}`);
      const result = await processProjectInteractionRequest({
        projectRoot: root,
        request: {
          intent: "export_book",
          bookId: id,
          format: fmt as "screenplay-md" | "screenplay-json" | "dialogue",
          approvedOnly,
          outputPath,
        },
        tools,
      ...
      return c.json({
        ...
        format: fmt,
        episodes: (result.details?.episodesExported as number | undefined) ?? 0,
      });
    } catch (e) {
      rethrowApiError(e);
      return c.json({ error: String(e) }, 500);
  ...
  // --- Episode review mode (C4a: auto pipeline vs manual checkpoint) ---

  app.get("/api/v1/project/episode-review-mode", async (c) => {
    const raw = await loadRawConfig(root);
    return c.json({ mode: readProjectEpisodeReviewMode(raw) });
  });

  app.put("/api/v1/project/episode-review-mode", async (c) => {
    const { mode } = await c.req.json<{ mode?: string }>();
    const next = normalizeEpisodeReviewMode(mode);
    await mutateRawConfig(root, (raw) => {
  ...
  app.get("/api/v1/books/:id/episode-review-mode", async (c) => {
    const bookId = c.req.param("id");
    ...
    try {
      ...
      const projectMode = readProjectEpisodeReviewMode(projectConfig);
      const bookMode = readBookEpisodeReviewMode(rawBook);
      return c.json({
  ...
  app.put("/api/v1/books/:id/episode-review-mode", async (c) => {
    const bookId = c.req.param("id");
    if (!isSafeBookId(bookId)) return c.json({ error: "Invalid book id" }, 400);
    const { mode } = await c.req.json<{ mode?: string }>();
    try {
      if (mode !== "auto" && mode !== "manual" && mode !== "inherit") {
        throw new CoreMutationValidationError("INVALID_MUTATION", `Invalid episode review mode: ${String(mode)}`);
      }
      const projectMode = readProjectEpisodeReviewMode(await loadRawConfig(root));
      const result = await executeCoreMutation({ state }, {
  ...
  app.post("/api/v1/books/:id/detect/:episode", async (c) => {
    const id = c.req.param("id");
    const episodeNum = normalizePositiveIntegerParam(c.req.param("episode"), "episode number");
    const bookDir = state.bookDir(id);
    ...
    try {
      ...
      const content = await readFile(join(episodesDir, match), "utf-8");
      const { analyzeAITells } = await import("@actalk/inkos-core");
      const result = analyzeAITells(content);
      return c.json({ episodeNumber: episodeNum, ...result });
    } catch (e) {
      rethrowApiError(e);
      return c.json({ error: String(e) }, 500);
    }
  ...
  // --- Write Rewrite (specific episode) ---

  app.post("/api/v1/books/:id/rewrite/:episode", async (c) => {
    const id = c.req.param("id");
    const episodeNum = normalizePositiveIntegerParam(c.req.param("episode"), "episode number");
    ...
    return c.json({ status: "rewriting", bookId: id, episode: episodeNum, requestId: operation.requestId }, 202);
  });

  app.post("/api/v1/books/:id/resync/:episode", async (c) => {
    const id = c.req.param("id");
    const episodeNum = normalizePositiveIntegerParam(c.req.param("episode"), "episode number");
    const body: { brief?: string } = await c.req
      .json<{ brief?: string }>()
    ...
    launchStudioOperation({
      ...
      run: async () => {
        ...
        return pipeline.resyncEpisodeArtifacts(id, episodeNum);
      },
      completeData: (result) => ({ episode: episodeNum, status: result.status, resultStatus: result.status }),
    });
    return c.json({ status: "resyncing", bookId: id, episode: episodeNum, requestId: operation.requestId }, 202);
  });
    ...
    try {
      ...
      const results = await Promise.all(
        mdFiles.map(async (f) => {
          ...
          const content = await readFile(join(episodesDir, f), "utf-8");
          const result = analyzeAITells(content);
          return { episodeNumber: num, filename: f, ...result };
        }),
      );
  ...
  app.post("/api/v1/genres/create", async (c) => {
    const body = await c.req.json<{
      ...
      episodeTypes?: string[]; fatigueWords?: string[];
    ...
    const frontmatter = [
      ...
      `episodeTypes: ${JSON.stringify(body.episodeTypes ?? [])}`,
  ...
  app.put("/api/v1/genres/:id", async (c) => {
    ...
    const frontmatter = [
      ...
      `episodeTypes: ${JSON.stringify(p.episodeTypes ?? [])}`,
  ...
  app.post("/api/v1/books/:id/import/episodes", async (c) => {
    const id = c.req.param("id");
    const { text, splitRegex } = await c.req.json<{ text: string; splitRegex?: string }>();
    if (!text?.trim()) return c.json({ error: "text is required" }, 400);

    broadcast("import:start", { bookId: id, type: "episodes" });
    try {
      const { splitEpisodes } = await import("@actalk/inkos-core");
      const episodes = [...splitEpisodes(text, splitRegex)];

      const pipeline = new PipelineRunner(await buildPipelineConfig({ bookIdForTelemetry: id }));
      const result = await pipeline.importEpisodes({ bookId: id, episodes });
      broadcast("import:complete", { bookId: id, type: "episodes", count: result.importedCount });
      return c.json(result);
    } catch (e) {
