...
vi.mock("@actalk/inkos-core", async (importOriginal) => {
  ...
  class MockStateManager {
    ...
    async loadEpisodeIndex(): Promise<[]> { return []; }
    async getNextEpisodeNumber(): Promise<number> { return 1; }
...
describe("Issue 1 — Studio: new book (has outline/story_frame.md)", () => {
  ...
  it("PUT story_bible.md returns 400", async () => {
    ...
    expect(body.error).toMatchObject({ code: "LEGACY_TRUTH_SHIM" });
    expect(body.error.message).toMatch(/Legacy compat shim/);
  });
