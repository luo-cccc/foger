...
import type { ChatStore, MessageActions, MessagePart, PipelineStage, ToolExecution, ToolLLMCall } from "../../types";
...
interface LLMTelemetryEventPayload {
  readonly sessionId?: string;
...
function appendToolLLMCall(
  existing: ToolExecution["llmCalls"],
  incoming: ToolLLMCall,
): NonNullable<ToolExecution["llmCalls"]> {
  return [
    ...(existing ?? []),
    incoming,
  ].slice(-6);
...
function parseLLMTelemetryPayload(data: LLMTelemetryEventPayload | null): ToolLLMCall | null {
  if (!data) return null;
  if (
  ...
  return {
    ...
    model: data.model,
    durationMs: numberOrZero(data.durationMs),
...
function shouldCreateStandaloneLLMCall(call: ToolLLMCall): boolean {
  return call.status !== "success" || call.durationMs >= 15_000;
}
...
export function attachSessionStreamListeners({
  ...
  streamEs.addEventListener("llm:telemetry", (event: MessageEvent) => {
    try {
      const data = event.data ? JSON.parse(event.data) as LLMTelemetryEventPayload : null;
      if (!sessionMatchesEvent(sessionId, data)) return;
      const call = parseLLMTelemetryPayload(data);
      if (!call) return;
      flushTextDeltas();
      set((state) => ({
        sessions: updateSession(state.sessions, sessionId, (runtime) => {
          ...
          if (runningTool) {
            const parts = (stream.parts ?? []).map((part) => {
              ...
              return {
                ...
                execution: {
                  ...part.execution,
                  llmCalls: appendToolLLMCall(part.execution.llmCalls, call),
                },
              };
            });
            const flat = deriveFlat(parts);
          ...
          if (!shouldCreateStandaloneLLMCall(call)) return {};

          const now = Date.now();
          const status: ToolExecution["status"] =
            call.status === "error" || call.status === "timeout" ? "error" : "completed";
          const parts = [
            ...(stream.parts ?? []),
            {
              type: "tool" as const,
              execution: {
                ...
                status,
                llmCalls: appendToolLLMCall(undefined, call),
                ...(call.errorMessage ? { error: call.errorMessage } : {}),
                startedAt: now - Math.max(0, call.durationMs),
                completedAt: now,
              },
            },
          ];
