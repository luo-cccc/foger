...
describe("chat runtime error copy", () => {
  it("localizes known assistant errors", () => {
    expect(extractErrorMessage({
      message: "Latest episode 1 is state-degraded. Repair state or rewrite that episode before continuing.",
  ...
  it("localizes known tool errors", () => {
    expect(extractToolError({
      content: [
        {
          type: "text",
          text: "Latest episode 2 is state-degraded. Repair state or rewrite that episode before continuing.",
        },
      ],
    })).toBe("最新第 2 集处于状态降级（state-degraded）。继续写下一集前，请先修复状态，或重写这一集。");
  });
});
...
describe("deserializeMessages", () => {
  it("restores tool executions from legacyDisplay for tool-only assistant messages", () => {
    const messages = deserializeMessages([
      {
        ...
        legacyDisplay: {
          toolExecutions: [
            exec({
              id: "import-1",
              tool: "import_episodes",
              details: { kind: "import_episodes_completed", bookId: "night-harbor" },
            }),
          ],
        },
      } as any,
    ...
    expect(messages[0]?.toolExecutions?.[0]?.tool).toBe("import_episodes");
