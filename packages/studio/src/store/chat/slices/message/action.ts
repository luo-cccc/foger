...
function stripSkillDirectives(text: string): string {
  return text.replace(SKILL_DIRECTIVE_RE, (_match, prefix: string) => prefix).replace(/\s+/g, " ").trim() || text.trim();
}
...
export const createMessageSlice: StateCreator<ChatStore, [], [], MessageActions> = (set, get) => ({
  ...
  sendMessage: async (sessionId, text, options?: SendMessageOptions) => {
    ...
    const instruction = stripSkillDirectives(userInstruction);
    ...
    try {
      ...
      if (responseBookId || responseSessionKind || data.session?.title) {
        set((state) => {
