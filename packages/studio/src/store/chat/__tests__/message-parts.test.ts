...
describe("buildPartsFromEvents", () => {
  ...
  it("moves pre-tool text to thinking when tool starts", () => {
    const parts = buildPartsFromEvents([
      ...
      { type: "tool:end", id: "t1" },
      { type: "draft:delta", text: "Episode written." },
    ]);
    ...
    expect(parts[2]).toEqual({ type: "text", content: "Episode written." });
  });
  ...
  it("attaches llm telemetry to the running tool execution", () => {
    const parts = buildPartsFromEvents([
      { type: "tool:start", id: "t1", tool: "sub_agent", agent: "writer", stages: ["step-1"] },
      {
        type: "llm:telemetry",
        agent: "writer",
        phase: "compose",
    ...
    expect(parts[0].type).toBe("tool");
    if (parts[0].type === "tool") {
      expect(parts[0].execution.llmCalls).toEqual([
  ...
  it("creates a standalone llm telemetry card when no tool is active and the call is slow", () => {
    const parts = buildPartsFromEvents([
      {
        type: "llm:telemetry",
        agent: "chat",
    ...
    expect(parts[0].type).toBe("tool");
    if (parts[0].type === "tool") {
      expect(parts[0].execution.tool).toBe("llm_call");
      expect(parts[0].execution.status).toBe("completed");
      expect(parts[0].execution.llmCalls?.[0]).toMatchObject({
        phase: "response",
        status: "success",
        totalTokens: 300,
      });
  ...
  it("shows context compression token budget and source trace while running", () => {
    ...
    if (parts[0].type === "tool") {
      ...
      expect(compressionStage?.progress?.status).toContain("来源 4: story/episode_summaries.md#recent_titles");
  ...
  it("localizes known tool errors", () => {
    const parts = buildPartsFromEvents([
      { type: "tool:start", id: "t1", tool: "sub_agent", agent: "writer" },
      {
        ...
        id: "t1",
        isError: true,
        result: "Latest episode 1 is state-degraded. Repair state or rewrite that episode before continuing.",
      },
    ]);
    ...
    if (parts[0].type === "tool") {
      expect(parts[0].execution.error).toBe(
        "最新第 1 集处于状态降级（state-degraded）。继续写下一集前，请先修复状态，或重写这一集。",
      );
    }
  });
});
