...
import { LLMTelemetrySection } from "../sidebar/LLMTelemetrySection";
import { FoundationSection } from "../sidebar/FoundationSection";
import { SummarySection } from "../sidebar/SummarySection";
import { EpisodesSection } from "../sidebar/EpisodesSection";
...
function artifactLabel(file: string): string {
  return truthFileDisplayLabel(file);
}
...
function ArtifactView({ bookId }: { readonly bookId: string }) {
  ...
  const artifactEpisode = useChatStore((s) => s.artifactEpisode);
  ...
  const isEpisode = artifactEpisode !== null;
  ...
  useEffect(() => {
    ...
    setBody(null);
    if (isEpisode) {
      fetchJson<{ content: string }>(`/books/${bookId}/episodes/${artifactEpisode}`)
  ...
  }, [bookId, artifactFile, artifactEpisode, isEpisode]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      if (isEpisode) {
        await fetchJson(`/books/${bookId}/episodes/${artifactEpisode}`, {
          method: "PUT",
  ...
  }, [bookId, artifactFile, artifactEpisode, isEpisode, editContent]);
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-4 py-3 text-[15px] leading-7">
            {renderTruthBody(isEpisode ? null : artifactFile, content, frontmatter, body)}
          </div>
        )}
      </div>
    </div>
...
function PanelView({ bookId, theme: _theme, t, sse }: BookSidebarProps) {
  ...
  return (
    <div className="flex flex-col gap-2 p-3">
      ...
      <LLMTelemetrySection bookId={bookId} sse={sse} />
      <EpisodesSection bookId={bookId} isZh={isZh} />
