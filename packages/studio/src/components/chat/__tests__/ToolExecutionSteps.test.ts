...
describe("groupChronologically", () => {
  ...
  it("extracts proposed route actions for existing Studio workflows", () => {
    const cases = [
      { action: "import_episodes", route: "import:episodes", title: "导入章节" },
  ...
  it("renders pipeline status, result summary, and file-operation group in English", () => {
    const execs: ToolExecution[] = [
      makeExec({
        ...
        agent: "writer",
        label: "Write",
        result: "Episode 1 finished.",
      }),
      makeExec({ id: "read-en-1", tool: "read", label: "Read file", args: { path: "books/demo/episode-1.md" } }),
    ];
