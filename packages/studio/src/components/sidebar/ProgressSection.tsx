...
const WRITE_EPISODE_STEPS: ReadonlyArray<ProgressStep> = [
  { zh: "准备剧集输入", en: "Prepare episode input" },
  { zh: "撰写剧集草稿", en: "Draft the episode" },
...
export function ProgressSection({ sse }: ProgressSectionProps) {
  ...
  useEffect(() => {
    ...
    if (last.event === "book:creating") {
      ...
      const steps = operation === "init" ? INIT_BOOK_STEPS : WRITE_EPISODE_STEPS;
  ...
  const steps = operation === "init" ? INIT_BOOK_STEPS
    : operation === "write" ? WRITE_EPISODE_STEPS
    : null;
