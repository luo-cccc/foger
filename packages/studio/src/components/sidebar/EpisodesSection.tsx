...
export function EpisodesSection({ bookId, isZh }: EpisodesSectionProps) {
  const [episodes, setEpisodes] = useState<ReadonlyArray<EpisodeMeta>>([]);
  const bookDataVersion = useChatStore((s) => s.bookDataVersion);

  useEffect(() => {
    fetchJson<{ episodes: EpisodeMeta[] }>(`/books/${bookId}`)
      .then((data) => setEpisodes(data.episodes))
      .catch(() => setEpisodes([]));
  }, [bookId, bookDataVersion]);

  return (
    <SidebarCard title={isZh ? "剧集" : "Episodes"}>
      {episodes.length === 0 ? (
        <p className="text-[15px] leading-6 text-muted-foreground/50 italic">
          {isZh ? "暂无剧集" : "No episodes"}
        </p>
      ) : (
        <ul className="space-y-1 max-h-52 overflow-y-auto overflow-x-hidden">
          {episodes.map((ch) => {
            ...
            return (
              <li
                key={`${ch.episodeNumber}-${ch.title ?? ""}`}
                onClick={() => useChatStore.getState().openEpisodeArtifact(ch.episodeNumber)}
                ...
                <span className="truncate flex-1">
                  {String(ch.episodeNumber).padStart(2, "0")} {ch.title || (isZh ? `第${ch.episodeNumber}集` : `Episode ${ch.episodeNumber}`)}
                </span>
                <span className="tabular-nums text-[13px] text-muted-foreground/50 shrink-0">
                  {(ch.episodeDurationSeconds ?? 0).toLocaleString()}
                </span>
              </li>
            );
          })}
