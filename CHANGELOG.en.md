...
- Chat gains an `import_chapters` tool: existing novels in local files/directories (including attachments uploaded in chat) can be imported as real book chapters, with settings reverse-engineered automatically and chapter state replayed (#324)
...
- English parity batch: added bilingual copy for the long-form pipeline and dynamic areas of the Studio UI, plus CLI fixes for environment-language fallback and hardcoded Chinese output; added the full English changelog `CHANGELOG.en.md` and filled in missing English README sections

### Bug Fixes

- Canon governance: PostWriteClaimGate's planned-reveal-but-not-landed check (claim-reveal-missing) previously only fired when the memo echoed a claim id or its first 4 characters verbatim. A new memoCommitsToReveal reveal-intent bridge treats any memo that uses a reveal cue and names the claim subject as a this-chapter reveal commitment; deferral cues suppress it. This closes the memo-says-reveal-prose-does-not gap in the claim gate.
- Canon governance: revising a book foundation (`reviseFoundation`) and creating a side-story (`initSpinoffBook`) now re-extract structured canon, so `story/canon/*.json` no longer lags behind the new prose foundation. Previously only initial book creation extracted canon, leaving revised books' claim working sets built on stale canon and side-story claim gates idle for lack of canon (extraction failure remains a non-fatal degradation).
- Volume closure: the current-arc active-subplot decision now keys off the status column (column 7 of the standard `subplot_board.md` layout), falling back to a whole-row scan only for legacy/non-standard layouts where the status column is empty. This fixes finished subplots being misclassified as active — and polluting the planner's focus — whenever an activity keyword happened to appear in the progress/pressure columns.
- Volume closure: the volume-end gate (KR completeness / irreversible event / supply-landing criticals) now fires only once, on the volume's planned last chapter. It previously used `>=`, so every chapter written past the planned end (falling back to the last volume's contract) re-fired the same volume-end criticals as noise.
- Canon governance: PostWriteClaimGate's world-rule / institution-rule "bypass without cost" checks tightened their bypass signals to strong circumvention verbs only (无视 / 绕过 / 越过 / 破例 / 失效 / no-cost / bypass / ignore, etc.), no longer treating generic narrative adverbs (直接 / 照样 / 仍然能 / 轻易, "freely", "still can") as bypass signals. This fixes ordinary prose containing those adverbs from triggering false hard-rule-bypass / institution-rule-bypass criticals (true positives still require an explicit circumvention verb on page).
- Canon governance: `runtime/canon_validator` (deterministic canon schema / governance issues that must be honored before drafting) was missing from the protected context-source list, inconsistent with its same-tier sibling `runtime/pre_write_claim_gate`, so it could be compressed away under context-budget pressure — hiding required canon constraints from the writer. It is now a protected source, treated the same as the pre-write claim gate.
...
Chat collaboration and long-form revision updates: this release adds user file uploads, image attachments, long-task interruption, and one-off revision brief passing for old-chapter edits. The core goal is to make Chat feel like a real long-form creation workbench: it can read the materials the user provides, stop long tasks, and pass the current conversation requirements into the revision chain accurately.
...
Long-form creation stability release: this version focuses on Studio Chat, the long-form chapter pipeline, and collaborative user editing. It fixes several stability issues affecting long-task continuation and chapter-state safety.

### Major Features

- Studio Chat and the long-form action surface are aligned: heavy actions still go through confirmation cards to avoid accidental writes
...
Major long-form creation workbench release: pushes InkOS from a "write the next chapter automatically" pipeline tool toward a more complete long-form creation agent. Book creation, continuation, chapter rewriting, chapter import, and project-file editing now share the same Studio Chat / CLI / TUI interaction core, with systematic work on instruction following, context management, and the visual experience.

### Major Features

- Studio creation entries were reorganized around long-form work: book creation, existing chapter import, continuation, chapter rewriting, and project chat can all be launched from the workbench
- Added / improved parent-canon import and continuation chains, so existing long-form settings can keep spawning new chapters
- Studio Chat, TUI, and CLI unified onto the action surface: plain discussion, confirmed book creation, long-form chapter writing, chapter import, and rewrite/continuation no longer race on scattered keywords
...
- The Studio left navigation, long-form conversation area, artifact previews, and font sizes were reorganized as a whole
- Model settings, aggregator API entries, and error messages are further separated: InkOS execution errors and model provider errors are no longer mixed together
- The README was updated to the long-form creation agent positioning
...
Major Studio Chat collaboration release: adds persistent plain-chat sessions and artifact previews, and fixes long-form length normalization possibly being truncated by the output limit.
...
- Chat can directly edit generated long-form text artifacts inside the project, which is handy for adjusting chapters, settings, and control documents before continuing with the InkOS writing chain
...
- **README stats**: added the Skills Download History chart, synced across the Chinese and English READMEs
...
Unified interaction core — the TUI, Studio, and `inkos interact` share the same natural-language understanding and execution runtime.
...
- **`inkos interact --json`**: the shared interaction JSON entry, returning request / response / session / events, callable directly by external agents and scripts
...
Full writing-pipeline upgrade. Driven by multiple long-form writing experiments, book creation, continuation, hook governance, auditing, and revision quality improved significantly.

### New Features

- **Foundation Reviewer**: an independent review agent added at book creation, scoring core conflict, opening pacing, and long-form sustainability; below threshold it rejects automatically and feeds the review comments back to the Architect for regeneration
...
- **Governed path completion**: parent_canon.md for continuation and parent-canon import is injected into the Writer through the governed path
...
- approve no longer overwrites snapshots, Studio hot-reloads LLM config, theme persistence
...
- README updated with the Studio release announcement and roadmap
...
- **#111** — CLI adds missing commands (eval, consolidate, write rewrite, book update/delete, plan/compose, studio, genre create/copy)
...
- README description updated with the structured interaction entry
...
- All commands support `--json` structured output, directly parseable by external agents and scripts
