...
- Chat 新增 `import_chapters` 工具：可把本地文件/目录（含对话上传的附件）里的已有小说导入为书籍真实章节，自动逆向生成设定并重放章节状态（#324）
...
- 英文能力对齐批量改动：长篇创作管线和 Studio 动态区补充中英双语文案，CLI 修复环境语言回退与硬编码中文输出；新增全量英文更新日志 `CHANGELOG.en.md` 并补齐英文 README 缺失章节

### Bug Fixes

- canon 治理：PostWriteClaimGate 的「计划揭示却未落台面」检查（claim-reveal-missing）此前只在 memo 逐字回显 claim id 或前 4 字时才触发。新增 memoCommitsToReveal 揭示意图桥接：当 memo 出现揭示类线索词（揭示/揭晓/揭露/reveal/unveil 等）且点名 claim 主题时即视为本章揭示承诺；推迟类线索词（暂不揭示/留到/defer 等）显式抑制。这样“memo 说这章要揭、正文没揭”不再漏过 claim gate
- canon 治理：修订基础设定（`reviseFoundation`）和番外建书（`initSpinoffBook`）现在会重新抽取结构化 canon，`story/canon/*.json` 不再落后于新 prose 设定；此前只有初次建书抽取 canon，导致修订后的章节 claim 工作集基于过时设定、番外书 claim gate 因缺 canon 而空转（抽取失败仍非致命降级）
- 卷级闭环：当前叙事弧的活跃支线判定改为以状态列（`subplot_board.md` 标准布局第 7 列）为准，仅在状态列为空的旧/非标准布局才回退整行扫描；修复了“进度概述/压力”列里出现活跃词就把已完结支线误判为活跃、污染 planner 规划焦点的问题
- 卷级闭环：卷尾门禁（KR 完整性 / 卷尾不可逆事件 / 供给项落地等 critical 检查）改为只在本卷规划的最后一章触发一次；此前用 `>=` 判定，导致写作溢出卷范围后的每一章（回退到最后一卷 contract）都反复报同一批卷尾 critical 噪音
- canon 治理：PostWriteClaimGate 的世界规则/制度规则「无代价绕过」检查收紧绕过信号词，只认强环节词（无视 / 绕过 / 越过 / 破例 / 失效 / 无需代价 / bypass / ignore 等），不再把「直接 / 照样 / 仍然能 / 轻易」这类常见叙事副词当作绕过信号；修复了正常正文里出现这些词就误报 hard-rule-bypass / institution-rule-bypass critical 的问题（真阳性仍需正文出现强绕过词）
- canon 治理：`runtime/canon_validator`（确定性 canon schema / 治理问题，起草前必须遵守）此前不在受保护上下文源列表，与同级的 `runtime/pre_write_claim_gate` 不一致，上下文预算紧张时可能被压缩丢弃，导致写手看不到必须遵守的 canon 治理约束；现已加入受保护源，与 pre-write claim gate 同等对待
...
Chat 协作与长篇修订体验更新：补齐用户上传文件、图片附件、长任务中断和 Chat 旧章修订 brief 传递。核心目标是让 Chat 更像真实长篇创作工作台：能看用户给的材料，能停下长任务，也能把当前对话要求准确传入修订链路。
...
长篇创作稳定性大版本：围绕 Studio Chat、长篇章节管线和用户协作编辑补齐稳定性能力，修复几类影响长任务继续推进和章节状态安全的问题。

### Major Features

- Studio Chat 与长篇 action surface 对齐，重动作继续走确认卡，避免误触发写入
...
长篇创作工作台大版本：把 InkOS 从“自动写下一章”的管线工具推进到更完整的长篇创作 Agent。建书、续写、章节重写、章节导入和项目文件编辑开始共享同一套 Studio Chat / CLI / TUI 交互内核，并围绕指令遵循、上下文管理和可视化体验做了系统性整理。

### Major Features

- Studio 创作入口重组为长篇优先入口：长篇建书、已有章节导入、续写、章节重写和项目对话都可从工作台启动
- 新增 / 完善父书正典导入和续写链路，让已有长篇设定可以继续派生新章节
- Studio Chat、TUI 和 CLI 统一到 action surface：普通讨论、确认建书、长篇写章、章节导入和重写续写不再依赖散落关键词抢跑
...
- Studio 左侧导航、长篇对话区、生成物预览和字体尺寸做了整体整理
- 模型配置、聚合 API 入口和错误提示进一步区分：InkOS 执行错误和模型供应商错误不再混在一起
- README 同步更新为长篇创作 Agent 定位
...
Studio Chat 协作大版本：新增普通聊天持久化会话和生成物预览，并修复长篇长度归一化可能被输出上限截断的问题。

### Improvements

- Studio 普通聊天支持项目级持久化 session，刷新或重启后可继续查看、切换、改名和删除会话
- Chat 可直接编辑项目内长篇文本产物，适合调整章节、设定和控制文档后再继续使用 InkOS 写作链路
...
- **README 统计**：补充 Skills Download History 图表，并同步中文、英文 README
...
统一交互内核——TUI、Studio、`inkos interact` 共享同一套自然语言理解和执行运行时。
...
- **`inkos interact --json`**：共享交互 JSON 入口，返回 request / response / session / events，供外部 Agent 和脚本直接调用
...
写作管线全面升级。通过多轮长篇写作实验，建书、续写、伏笔治理和审稿修订质量显著提升。

### 新功能

- **Foundation Reviewer**：建书时新增独立审核 Agent，围绕核心冲突、开篇节奏、长篇可持续性等维度打分，不达标会驳回并将审核意见反馈给 Architect 重新生成
...
- **Governed 路径补全**：续写和父书正典导入的 parent_canon.md 通过 Governed 路径注入 Writer
...
- approve 不覆盖快照、Studio 热加载 LLM 配置、主题持久化
...
- **#111** — CLI 补齐缺失命令（eval, consolidate, write rewrite, book update/delete, plan/compose, studio, genre create/copy）
...
- 无数值系统的题材不生成资源账本
- 所有命令支持 `--json` 结构化输出，外部 Agent 和脚本可直接解析
