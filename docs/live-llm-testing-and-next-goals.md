...
截至 2026-07-17，项目本地逻辑、持久化、确定性测试和前后端联动合同已经较稳；真实证据包括历史 DeepSeek 15 章样本、Ark Plan 旧隔离项目 8 章终态，以及恢复合同修复后的 Ark Plan 六章样本：
...
- 恢复合同修复后的 Ark Plan 重跑完成 6/20：六章均 `ready-for-review` 且 Doctor 通过，repair、resync、rewrite、revise 均取得真实收敛证据；第 7 章 Settler 被上游内容安全策略以 HTTP 400 拒绝且未提交。报告可靠失败并清理隔离凭据，135 条 telemetry 为 134 success、1 error、0 retry，共 `1,422,474` tokens。
- provider 内容安全拒绝现已结构化分类；治理 agent 的显式一次性跨 provider fallback、无人值守暂停和 linked stub 故障注入均已完成。下一步是在配置独立 fallback provider 后执行新的唯一付费 20 章复验；不能把六章、历史七章或 stub 结果表述为 20 章通过。
...
### 8.26 审计恢复收敛合同（2026-07-17）

本轮没有调用真实 provider，针对 8.25 的第 8 章失败链完成确定性开发：

- 章节索引新增向后兼容的 `recoveryState`，记录当前正文 fingerprint、结构化 blocking issues、来源 operation 和终止原因；旧书仍可从字符串 audit issues 降级恢复。
- 初次写章在状态校验前保留正文审计问题，repair/resync 恢复为 `audit-failed` 时继续返回原有 category、repairScope 和建议，不再返回空 issues。
- 独立 revise 会重新审计当前正文；若正文无需修改但缺真相快照，终态改为 `state-degraded` 并进入状态恢复，而不是保留无证据的 `audit-failed`。
- 新共享策略限定同一章节 revise/rewrite 各一次、每个正文 fingerprint repair/resync 各一次；rewrite 产生新正文后允许重新结算，但相同正文耗尽恢复动作后可靠暂停。
- Scheduler 与 linked acceptance 使用同一策略。定向 Core 4 个测试文件 123 项通过，`pnpm test:linked` 通过浏览器、Studio API/SSE、Core、持久化和 Doctor 联动。

本轮完整 `pnpm verify` 通过：Core 132 个测试文件、1465 项，Studio 34 个测试文件、407 项 Vitest 与 2 项报告终态 Node 测试，CLI 36 个测试文件、210 项，共 2084 项；hygiene、typecheck、语义审计、build、bundle 和 publish manifest 同时通过。`pnpm stress:unattended` 同时通过 20 章强杀、超时和重启恢复，最终 20/20 为 `ready-for-review`，恢复动作跨 2 次有界运行收敛。

该结果只关闭确定性恢复合同，不证明真实模型第 8 章一定能自动修好。下一有效动作仍是全新隔离项目的唯一 20 章重跑；若新样本在有界动作后暂停，应保留与当前正文 fingerprint 对应的结构化证据，而不是提高重试次数。

### 8.27 Ark Plan 恢复合同复验与内容安全阻断（2026-07-17）

本轮按 8.26 的结论从全新隔离项目执行唯一一次 20 章、每章 1000 字、`report-only` linked acceptance。仍使用 `custom:Ark Plan / deepseek-v4-pro`，总 token 与单次 prompt 不设硬上限，建书最多 2 次、写章最多 3 次；报告使用独占路径。隔离项目从正式配置复制密钥，launcher 终止后自动删除项目和 runtime，终态报告与日志未检出密钥。

- Foundation 首轮 `66/100`，一次有界重生成后以 `85/100` 通过；首次建书尝试即为 ready，前置阶段 `49,454` tokens。
- 第 1-6 章全部为 `ready-for-review`，字数分别为 `1016/1239/1180/1106/1098/1246`，六章均 `doctorVerified=true`。第 2 章一次 repair 后恢复；第 3 章按 `repair-state -> resync -> rewrite` 恢复；第 6 章一次独立 revise 后恢复。新正文 fingerprint、结构化问题和共享有界决策在真实链路生效，没有重复同一外层恢复动作。
- 第 7 章 Planner、Composer、Writer 和 Settler observation 已成功，Settler 最终结算请求被 Ark Plan 以 HTTP 400 拒绝，明确提示输入可能包含敏感信息。该错误被判为不可重试，章节事务没有提交第 7 章，报告在 `chapter-7-pipeline-attempt-1` 可靠失败。
- 全程 telemetry 为 135 条：134 success、1 error、0 provider retry；唯一错误为 `settler/settle`。总 usage `1,422,474` tokens，最大估算 prompt `23,227`，最大单次耗时 `214316 ms`。quality gate 对六个已完成章节全部通过；linked gate 因只观察到 6/20 和第 7 章 provider 拒绝而失败。
- 第 3 章 Planner 三次 hook 身份校验失败后使用 fallback；其他章节也多次出现把既有 hook 当作新 hook、同一 hook 同时 advance/resolve 的有界语义重试。它们未破坏六章终态，但仍是明显的治理成本和质量风险。

本轮关闭的是“恢复收敛合同只有离线证据”的缺口，不关闭 20 章验收。下一有效开发目标不是原样重复 Ark Plan 长跑，而是把内容安全拒绝从通用 invalid-request 中结构化分类，默认不向同一 provider 重试，并为治理 agent 增加显式配置、可审计且保持 operation/telemetry 归属的跨 provider fallback。完成故障注入与 linked stub 回归后，才执行新的唯一 20 章付费验收。

### 8.28 内容安全拒绝分类与治理 fallback（2026-07-17）

本轮没有调用真实 provider。针对 8.27 的唯一错误完成了确定性实现：

- Provider 层新增 `provider-content-policy` 分类，覆盖 Ark 的 `The request failed because the input may contain sensitive information.` 以及常见 content/safety policy/filter 拒绝。分类发生在通用 HTTP 400 包装前；该错误不进入同 provider transport retry。
- 项目配置新增显式 `contentPolicyFallback`。默认治理名单包含 Planner、Composer、Settler、Auditor、Canon Extractor、Claim Validator、Volume Auditor、State Validator 和 Chapter Analyzer，明确排除 Writer 与 Reviser。配置必须给出独立 service、base URL、model 和 API key 环境变量；运行时拒绝与主路由相同的 service 或 endpoint。
- 治理 agent 只执行一次跨 provider fallback。原拒绝与 fallback 调用共用 telemetry sink，因此 `bookId`、`operationId`、预算窗口和报告归属不变；原拒绝按估算 prompt token 计入预算。telemetry 新增 `failureKind`、`route` 和 `fallbackFrom`，不保存失败 prompt 或正文。
- fallback 也失败时不循环。无人值守分类新增 `provider-content-policy`，没有可用 fallback 或 fallback 失败后立即 `pause`，不会在下一调度周期原样重放。
- CLI 新增 `config set-content-policy-fallback`、`remove-content-policy-fallback`，服务密钥仍从项目 secret 注入声明的环境变量，不写入 `inkos.json`。
- linked stub 故障注入真实经过浏览器、Studio API/SSE、Core、持久化和 Doctor：Settler 主路由一次 error，`attemptCount=1/retryCount=0`；fallback 一次 success，两条记录 operationId 相同。报告 `linkedGate=true`、`qualityGate=true`，失败请求估算 prompt `1588` tokens 被纳入总量。
