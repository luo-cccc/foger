...
> 文档状态：真实 provider 测试方法与阶段记录。本文保留 2026-07-09 至 2026-07-15 的实验与收敛记录；当前全项目优先级以[当前架构与开发优先级](current-architecture-and-priorities.md)为准。下文旧 P0/P1 不应被理解为最新排期。
...
截至 2026-07-15，项目本地逻辑、持久化、确定性测试和前后端联动合同已经较稳，剩余质量风险主要集中在真实 LLM 编排、内容质量、耗时和成本：

- 正文生成和状态结算仍必须使用独立 agent/context，不能把 Settler 继续绑定为 Writer 的隐式职责。早期 MiniMax 双路由实验验证了这一点，但它不再是当前默认路由。
- 当前正式运行基线为所有 Agent 统一使用 `openrouter / deepseek/deepseek-v4-flash`；MiniMax 服务仅保留为可选配置，不再通过 `modelOverrides` 自动承接 Writer/Reviser/Length Normalizer。
- 历史双 API 实验曾完成 4 章并保持状态链路完整；最新 OpenRouter-only 五章基线已关闭低分 Foundation、Canon 多 envelope、live runner 未接 Scheduler 和 Reviser prompt 超预算问题，但仍没有五章全部完成的真实证据。
...
- 最新 OpenRouter-only 代码已经取得严格单章联动通过证据；仍需解决的是把 20.9 分钟、132930-token 的整轮成本压到可扩展范围，并依次取得 3 章稳定性和 5 章完整通过证据。
