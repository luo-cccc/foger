...
| `plan -> compose -> write -> audit -> revise -> settle` | 已完成 | 高（确定性）/中（真实模型） | Runner、共享审计合同、resync 兜底和状态门禁有完整回归；历史样本连续提交 15 章，修复后新样本在第 8 章 audit recovery 未收敛，仍缺完整 20 章复验 |
...
- [当前门禁] 先为“rewrite 后 audit-failed”建立确定性 recovery 回归，确认 chapter/index/truth/snapshot 不会在失败恢复中漂移；之后从新隔离项目重新跑唯一的 20 章验收。通过条件不变：20/20 持久化、无 `state-degraded`/`audit-failed` 终态、truth/快照一致、Doctor 全部关联且报告可靠终止。
...
7. [进行中] 为第 8 章真实样本的 `state-degraded -> audit-failed -> rewrite -> audit-failed` 恢复链补齐确定性复现并修复收敛；之后从全新隔离项目重跑唯一的 20 章验收，要求 20/20 持久化、truth/快照一致、Doctor 全部关联且报告可靠收口。通过后再启用 daemon，并推进重依赖延迟加载、大模块拆分和 CI 测试分层。
