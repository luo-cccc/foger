...
当前最主要的工程风险已经从底层持久化和跨入口写入旁路，转移到真实模型长距离连续写作中的状态协议稳定性。2026-07-16 的 DeepSeek 官方 `deepseek-v4-flash` 真实前后端 20 章样本实际持久化了 15 章：第 1-15 章均为 `ready-for-review` 并通过 Doctor，章节索引、manifest、current state 和快照一致停在 15；运行期 LLM telemetry 为 310/310 success、0 retry、`3,848,794` tokens。第 16 章未提交，进程在中间态被外部终止，因此不能表述为“20 章通过”。该样本定位到第 15 章 resync analyzer 把标准伏笔 ID 写成 `H027 (标题)`，后续 Planner 使用规范 ID `H027` 时被 ledger 校验拒绝。当前代码已在 resync 校验和落盘前规范化 Markdown 第一列，并在结构化状态加载时修复、去重 canonical ID；Core 回归覆盖 Markdown、`hooks.json` 和冲突迁移。随后 Ark Plan `deepseek-v4-pro` 从全新隔离项目重跑 20 章，在第 8 章经 `repair-state -> resync -> revise -> rewrite -> repair-state` 后仍为 `audit-failed`，最终报告可靠失败；第 1-7 章均为 `ready-for-review` 且 Doctor 通过。该结果说明 Hook ID 漂移不再是最早阻断点。当前代码已为恢复链增加正文 fingerprint、结构化阻断问题和共享有界策略：同章 revise/rewrite 各一次，每个正文版本 repair/resync 各一次，仍不收敛时可靠暂停；Scheduler 与 linked acceptance 复用同一决策合同。2026-07-17 的修复后 Ark Plan 20 章验收可靠运行至第 7 章：第 1-6 章全部 `ready-for-review` 且 Doctor 通过，第 2 章经 repair 恢复，第 3 章按 `repair -> resync -> rewrite` 恢复，第 6 章经 revise 恢复；第 7 章 Settler 请求被上游内容安全策略以 HTTP 400 拒绝，章节未提交，报告可靠失败并清理隔离凭据。该拒绝现已分类为 `provider-content-policy`；治理 agent 支持显式配置的一次性跨 provider fallback，同 provider 不重试，fallback 失败后无人值守立即暂停。Core、CLI、Studio 和 linked stub 已证明 operationId、telemetry 与预算归属连续；下一步是在独立 fallback provider 配置下运行新的唯一付费 20 章复验。token 继续记录用于容量判断，但不再作为当前功能验收的阻断项。Playwright 隔离 E2E、跨进程竞争和强杀恢复基线仍保持全绿；确定性门禁通过不等于真实 20 章已经完成。
...
| `plan -> compose -> write -> audit -> revise -> settle` | 已完成 | 高（确定性）/中（真实模型） | Runner、共享审计合同、resync 兜底和状态门禁有完整回归；最新样本验证 repair/resync/rewrite/revise 均可收敛，但第 7 章 Settler 被 provider 内容安全策略拒绝，仍缺完整 20 章终态 |
...
- 2026-07-17 完整回归为 Core 132 个测试文件、1465 项通过，Studio 34 个测试文件、407 项 Vitest 与 2 项报告终态 Node 测试通过，CLI 36 个测试文件、210 项通过；当前确定性测试共 2084 项。CLI 集成测试反复启动子进程，仍是主要耗时来源。
...
现有隔离 Studio E2E 已覆盖事务、stub authoring、取消、不落章、重试、`state-degraded -> repair-state -> ready-for-review` 和治理 agent 内容策略拒绝后的一次性跨 provider fallback。确定性测试覆盖长 ID、审计入口一致性、resync 失败回滚、hook 去重和 fallback 失败后 pause。真实 linked acceptance 已取得 15 个连续持久化章节的历史证据，并在最新六章样本验证 repair、resync、rewrite、revise 的共享有界策略；仍缺配置独立 fallback provider 后的完整 20 章终态和可重复的脱敏响应 corpus。联网测试不进入每次提交门禁。
...
写作质量能力的详细缺口、评价合同、人工盲评标准和分阶段完成定义见[写作能力提升开发方案](writing-capability-improvement-plan.md)。当前优先重跑修复后的 20 章正确性验收，并行建立 Book Eval v2 基线；在基线完成前不调整 Writer、Auditor 和 Reviser 的文学质量合同。
...
- [实现完成] linked acceptance 在首次及每个持久化里程碑原子写入报告 checkpoint；新 launcher 会在清理孤儿临时项目之前将遗留 `running` 报告收口为 `interrupted`，并从章节 index、结构化 truth、快照和 runtime LLM JSONL 重建终态摘要、归档中断报告。
...
- [最新真实证据] Ark Plan `deepseek-v4-pro` 的全新 20 章 linked acceptance 可靠运行至第 8 章：第 1-7 章 `ready-for-review` 且 Doctor 通过；第 8 章经历 repair、resync、revise、rewrite 后仍为 `audit-failed`，最终报告为 failed，未启动第 9 章。历史 DeepSeek 15 章样本仍保留为 Hook ID 漂移定位证据。
- [实现完成，待真实验收] 章节索引新增向后兼容的 `recoveryState`，持久化正文 fingerprint、结构化 blocking issues、来源 operation 和终止原因；状态恢复不会再把 `repairScope` 降成日志字符串或为空。
- [实现完成，待真实验收] Scheduler 与 linked acceptance 共享有界恢复策略；同一章节的 revise/rewrite 各最多一次，每个正文 fingerprint 的 repair/resync 各最多一次，证据与正文不匹配时必须重新审计，耗尽后进入可靠 pause。
- [最新真实证据] 2026-07-17 的修复后重跑完成 6/20：六章均 `ready-for-review` 且 Doctor 通过；第 2 章 repair、第 3 章 `repair -> resync -> rewrite`、第 6 章 revise 均按共享策略收敛。第 7 章 `settler/settle` 被 Ark Plan 内容安全策略以 HTTP 400 拒绝，未提交章节；135 条 telemetry 为 134 success、1 error、0 retry，共 `1,422,474` tokens，报告和隔离清理可靠收口。
- [实现完成，待真实复验] provider 内容安全拒绝已分类为 `provider-content-policy`，同 provider 不重试；Planner/Settler/Analyzer 等治理调用可显式配置一次性跨 provider fallback，Writer/Reviser 默认排除。原拒绝与 fallback 保持相同 operation/telemetry/预算归属，fallback 失败后无人值守立即暂停。Core 故障注入、Studio 结构化展示、CLI 配置和 linked stub 均已回归。
- [当前门禁] 为全新隔离项目配置与 Ark Plan 不同 endpoint 的 fallback provider 后，运行新的唯一 20 章验收。通过条件不变：20/20 持久化、无 `state-degraded`/`audit-failed` 终态、truth/快照一致、Doctor 全部关联且报告可靠终止。
...
2026-07-17 当前工作区完整离线验证基线：

- `pnpm typecheck`：通过。
- `pnpm verify`：2084 个测试通过（Core 1465、Studio Vitest 407、Studio 报告终态 Node 测试 2、CLI 210），并通过代码整洁、typecheck、语义审计、build、bundle 和 publish manifest 检查。
- `pnpm build`：通过。
...
- `pnpm release`：上次记录为通过；本轮未重复运行 release。当前 `pnpm verify` 已通过 2084 项测试，release 历史记录中的测试数量不再是当前测试计数。
...
确定性测试、全绿离线门禁和真实多进程压力基准是明显优势，但测试数量仍不能替代真实模型质量证据。事务、锁、进程恢复和 provider 内容安全治理 fallback 的本地可靠性 P0 可以关闭；真实恢复合同已在 repair、resync、rewrite 和 revise 路径取得证据。下一阶段是在独立 fallback provider 配置下重跑 20 章联动验收；之后处理报告终态、大模块拆分和测试分层耗时。
...
7. [provider fallback 已完成，待真实复验] 真实六章已验证 repair、resync、rewrite、revise 的正文 fingerprint、结构化证据和有界动作合同；第 7 章暴露的 Ark Plan 内容安全拒绝现已有独立分类、一次性治理 fallback、同 operation telemetry 和失败后 pause 合同，linked stub 已通过。下一步配置独立 fallback provider 后重跑唯一的 20 章验收；要求 20/20 持久化、truth/快照一致、Doctor 全部关联且报告可靠收口。通过后再启用 daemon，并推进重依赖延迟加载、大模块拆分和 CI 测试分层。
