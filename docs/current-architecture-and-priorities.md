...
当前最主要的工程风险已经从底层持久化和跨入口写入旁路，转移到真实模型质量、成本和 provider 长尾。正式运行基线已切为所有 Agent 统一使用 `openrouter / deepseek/deepseek-v4-flash`，MiniMax override 不再自动启用；live 五章入口也已改为生产 Scheduler 的一次性周期，真实 provider transient、审计修订/重写、状态 repair/resync 和预算暂停都会持久化到无人值守状态。2026-07-14 至 2026-07-15 的 OpenRouter-only 运行已经关闭低分 Foundation 放行、Canon 多/残缺 JSON、模型默认 high reasoning、live loop 绕过 Scheduler、Auditor/Reviser 限额接线、空状态补丁和多处 prompt 超 16000-token 的确定性问题。最新 Studio 真实单章联动已在严格策略下以 `1/1` 通过：Foundation 有界重生成后 `81/100`，章节 `ready-for-review`、`962` 字，linked gate、quality gate 和 Doctor operation 关联全部通过，fallback/timeout 为 0，最大 prompt 估算 `12723` tokens；但整轮耗时 20.9 分钟、总计 `132930` tokens。它证明前端、Studio API、Core、持久化和 Doctor 的同一操作链路已经贯通，不证明五章成本和无人值守稳定性已经达标。跨 agent token 报告和稳定上下文编译缓存已落地并有回归测试，但真实五章基线仍需在单章成本得到控制后验证。Playwright 使用独立临时根目录和动态端口；完整 Studio E2E 为 10/10，`pnpm test:linked` 单独通过；`pnpm stress:process` 通过 8 worker、400 次竞争写入和 30 轮真实强杀/恢复。发布链全绿不等于真实模型主链路已经达到无人值守稳定性。
...
6. [进行中] 真实 3-5 章基线已经形成单章质量、claim 假阳性、Canon fallback、provider 长尾和 token 热点证据；继续条件是 provider 状态、路由或代码场景发生实质变化，不能重复抽样追绿。
