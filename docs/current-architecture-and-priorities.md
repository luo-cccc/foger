...
当前最主要的工程风险已经从底层持久化和跨入口写入旁路，转移到真实模型质量、成本和长期维护：Planner/Settler 仍可能产生可恢复的格式漂移，真实 3-5 章成本基线尚未建立，跨 agent token 预算与稳定上下文缓存仍待验证，Studio server 与 PipelineRunner 也继续承担较宽职责。审计入口合同、确定性 resync 兜底、短稳定 hook ID、跨语言枚举规范化和长操作取消已经关闭。Playwright 使用独立临时根目录和动态端口；完整 Studio E2E 为 9/9，preparing/committed 两个真实子进程强杀/重启 recovery 场景合并连续运行 5 轮共 10/10。`pnpm stress:process` 进一步通过 8 worker、400 次竞争写入和 30 轮真实强杀/恢复；发布链保持全绿，但这不等于真实模型主链路已经达到无人值守稳定性。
...
| 上下文与性能治理 | 可用/持续增强 | 中高 | 已有 Prompt Assembly Trace、per-source 统计、确定性去重、verbatim/semantic/compressible 三层上下文和固定语料成本门禁；仍缺真实多章预算、跨 agent 总预算和稳定缓存基线 |
...
- 当前增量回归为 Core 123 个测试文件、1314 项通过，Studio 33 个测试文件、402 项通过；CLI 与隔离 E2E 的最终发布基线以第 10 节本次 `pnpm release` 结果为准。CLI 集成测试反复启动子进程，仍是主要耗时来源。
- `scripts/live-dual-api-routing.mjs` 已升级到 report v2：同时按 agent+phase、service+model、agent+service+model 汇总耗时、token、状态、尝试与重试，并输出长度偏差、结构化 fallback、repair/resync 恢复路径和可配置质量门禁。尚待一次明确授权的真实 3-5 章运行建立正式基线。
- Composer 已执行 token budget 和 verbatim/semantic/compressible 三层上下文策略；Provider telemetry 与 ChapterTrace 已记录 per-source 字符数、估算 token、tier、fingerprint、selected/compressed 状态和重复 fingerprint 组。尚缺真实多章下的跨 agent 预算与稳定缓存基线。
...
- `packages/studio/src/api/server.ts` 约 4895 行，同时承担路由、配置、密钥、会话、provider 探测和业务调度。
- `packages/core/src/pipeline/runner.ts` 约 4160 行，同时承担多个工作流、恢复、持久化和事件编排。
...
第 3 章完整流程约消耗 17.7 万 token，包含 Planner 重试、Writer、两次长度归一化、两次审计、Reviser、Analyzer 和 State Validator。Studio 已提供 write/draft/rewrite/repair/resync 的统一取消合同和状态恢复提示；当前剩余问题是用 Prompt Assembly Trace 建立每章 agent/phase/source 预算，避免无变化的重复审计、重复 settlement 和上下文膨胀。
...
- [待真实验收] 使用已配置服务执行 3-5 章基准 corpus；真实 API token 成本与 provider 失败样例不得用 stub 结果替代。
...
- [下一步] 使用真实 3-5 章 Prompt Assembly Trace 建立 Planner、Writer、Auditor 和全章总预算，再缓存稳定 foundation、角色和卷级摘要的编译结果。
...
2026-07-12 当前工作区完整发布基线：

- `pnpm typecheck`：通过。
- `pnpm test`：1923 个测试通过（Core 1314、Studio 402、CLI 207）。
- `pnpm build`：通过。
- `pnpm check:studio-bundle`：通过。
- `pnpm verify:publish-manifests`：通过。
- `pnpm audit:semantic-patterns`：0 个候选。
- `pnpm audit --prod`：0 vulnerabilities。
- `pnpm --filter @actalk/inkos-studio test:e2e`：完整套件 9/9；preparing/committed 真实进程 recovery 连续 5 轮共 10/10。
- `pnpm stress:process`：通过；8 worker，book/config 各 200 次竞争 mutation，workflow 20 轮与 chapter 10 轮强杀恢复。
- `pnpm release`：通过，覆盖全仓 typecheck、语义审计、build、bundle、1923 项 Vitest、publish manifest、生产依赖审计和 9 项隔离 E2E。

Playwright E2E 与进程压力测试已具备安全隔离、preparing/committed 真实进程死亡、竞争写入和重复稳定性证据。P1.1 报告和 P1.2 的 Prompt Assembly Trace、三层上下文、确定性去重、职责清理与固定语料门禁均已完成；下一步是在明确成本范围后跑一次真实 3-5 章基线，并据此建立跨 agent token 预算与缓存策略。
...
1923 个确定性测试、全绿发布链和真实多进程压力基准是明显优势，但测试数量仍不能替代真实模型质量证据。事务、锁和进程恢复的本地可靠性 P0 可以关闭；下一阶段应把精力转向 3-5 章真实模型报告、失败样例归档和测试分层耗时。
...
6. [当前开发项] 在明确成本范围后执行真实 3-5 章基线，用 per-source trace 建立 Planner/Writer/Auditor 和全章 token 预算，并归档质量/失败样例。
7. 随后缓存稳定上下文编译结果，再推进重依赖延迟加载、大模块拆分和 CI 测试分层。
