...
当前最主要的工程风险已经从底层持久化和跨入口写入旁路，转移到真实模型质量、成本和长期维护：Planner/Settler 仍可能产生可恢复的格式漂移，真实 3-5 章成本基线尚未建立。2026-07-13 的项目配置真实 API 单章复测完成 18 次调用、138,148 tokens，但 Planner 仍触发三次解析尝试并进入 fallback；这证明错误可诊断，不证明质量已达标。跨 agent token 预算报告和稳定上下文编译缓存的代码已落地，缓存已支持跨进程持久化并有回归测试，但仍待多章运行验证命中率、账单成本和质量边界；Studio server 与 PipelineRunner 也继续承担较宽职责。审计入口合同、确定性 resync 兜底、短稳定 hook ID、跨语言枚举规范化和长操作取消已经关闭。Playwright 使用独立临时根目录和动态端口；完整 Studio E2E 为 10/10，preparing/committed 两个真实子进程强杀/重启 recovery 场景合并连续运行 5 轮共 10/10。`pnpm stress:process` 进一步通过 8 worker、400 次竞争写入和 30 轮真实强杀/恢复；发布链保持全绿，但这不等于真实模型主链路已经达到无人值守稳定性。
...
- 当前增量回归为 Core 128 个测试文件、1372 项通过，Studio 34 个测试文件、405 项通过，CLI 36 个测试文件、209 项通过；本轮确定性测试共 1986 项。CLI 集成测试反复启动子进程，仍是主要耗时来源。
- `scripts/live-dual-api-routing.mjs` 已升级到 report v2：同时按 agent+phase、service+model、agent+service+model 汇总耗时、token、状态、尝试与重试，并输出长度偏差、结构化 fallback、repair/resync 恢复路径、per-agent/per-phase token budget 和可配置质量门禁。尚待一次明确授权的真实 3-5 章运行建立正式基线。
...
- [下一步] 使用真实 3-5 章 Prompt Assembly Trace 验证预算阈值、缓存命中率、供应商账单 usage 与 prose/状态质量，再校准默认门禁。
...
2026-07-14 当前工作区完整离线验证基线：

- `pnpm typecheck`：通过。
- `pnpm verify`：1986 个测试通过（Core 1372、Studio 405、CLI 209），并通过代码整洁、typecheck、语义审计、build、bundle 和 publish manifest 检查。
- `pnpm build`：通过。
...
- `pnpm release`：上次记录为通过；本轮未重复运行 release。当前 `pnpm verify` 已通过 1986 项测试，release 历史记录中的测试数量不再是当前测试计数。

Playwright E2E 与进程压力测试已具备安全隔离、preparing/committed 真实进程死亡、竞争写入和重复稳定性证据。P1.1 报告和 P1.2 的 Prompt Assembly Trace、三层上下文、确定性去重、职责清理、固定语料门禁、跨 agent token budget 报告和稳定上下文编译缓存代码均已完成；下一步是在明确成本范围后跑一次真实 3-5 章基线，校准门禁并归档真实质量/失败样例。
...
1986 个确定性测试、全绿离线门禁和真实多进程压力基准是明显优势，但测试数量仍不能替代真实模型质量证据。事务、锁和进程恢复的本地可靠性 P0 可以关闭；下一阶段应把精力转向 3-5 章真实模型报告、失败样例归档和测试分层耗时。
...
6. [当前开发项] 在明确成本范围后执行真实 3-5 章基线，用 per-source trace 验证 Planner/Writer/Auditor 和全章 token 预算、稳定上下文缓存命中率，并归档质量/失败样例。
7. 随后根据真实基线校准默认预算，再推进重依赖延迟加载、大模块拆分和 CI 测试分层。
