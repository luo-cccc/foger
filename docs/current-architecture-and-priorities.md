...
- 2026-07-15 完整回归为 Core 129 个测试文件、1418 项通过，Studio 34 个测试文件、405 项通过，CLI 36 个测试文件、209 项通过；当前确定性测试共 2032 项。CLI 集成测试反复启动子进程，仍是主要耗时来源。
- `scripts/live-dual-api-routing.mjs` 已升级到 report v2：同时按 agent+phase、service+model、agent+service+model 汇总耗时、token、状态、尝试与重试，并输出长度偏差、结构化 fallback、repair/resync 恢复路径、per-agent/per-phase token budget 和可配置质量门禁。真实 OpenRouter 单章联动已经通过；下一有效样本应先验证 3 章稳定性和成本，再恢复 5 章验收。
...
最新严格 Studio 真实单章联动消耗 `132930` tokens、20.9 分钟，其中建书/Canon `57650`、章节链路 `75280`；最大 prompt 估算 `12723`，没有触发 16000-token 门禁。Studio 已提供 write/draft/rewrite/repair/resync 的统一取消合同和状态恢复提示；当前剩余问题是降低 Foundation/Canon 重试和章节治理长尾，避免无变化的重复审计、重复 settlement 和上下文膨胀。旧的第 3 章约 17.7 万 token 样本保留在真实 LLM 历史记录中，不再作为当前基线。
...
- `pnpm typecheck`：通过。
- `pnpm verify`：2032 个测试通过（Core 1418、Studio 405、CLI 209），并通过代码整洁、typecheck、语义审计、build、bundle 和 publish manifest 检查。
- `pnpm build`：通过。
...
- `pnpm release`：上次记录为通过；本轮未重复运行 release。当前 `pnpm verify` 已通过 2032 项测试，release 历史记录中的测试数量不再是当前测试计数。
...
2032 个确定性测试、全绿离线门禁和真实多进程压力基准是明显优势，但测试数量仍不能替代真实模型质量证据。事务、锁和进程恢复的本地可靠性 P0 可以关闭；下一阶段应把精力转向 3-5 章真实模型报告、失败样例归档和测试分层耗时。
