...
Current quality boundary: local chapter production, structured state, chapter/workflow recovery, and 1,852 deterministic tests have reached a stable usable baseline. The isolated Studio E2E suite is 8/8; its preparing and committed real child-process force-kill/restart recovery cases passed five combined rounds, 10/10 in total. `pnpm stress:process` passed with 8 workers, 400 contended mutations, and 30 force-kill recovery rounds; `pnpm release` is green. Known Studio, CLI, Chat, and sub-agent write bypasses, revise-mode validation, recovery preflights, and cross-process config writes are now converged. See the [current architecture and quality matrix](docs/current-architecture-and-priorities.md).
