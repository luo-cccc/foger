...
- `audit-contamination.mjs`: rejects paid-run book proper nouns in production source, tests, prompts, genre profiles, scripts, and canonical docs. The guard is part of `pnpm verify`; add new paid-run names to `KNOWN_CONTAMINATION` and keep them out of every context an Agent may read. Release notes may retain concise historical conclusions only.
...
Live-provider scripts require explicit credentials and write into ignored temporary directories. `INKOS_MAX_PROMPT_ESTIMATED_TOKENS_PER_CALL` rejects oversized assembled prompts before either provider or stub transport is invoked; report-level token budgets remain post-run aggregate gates. Raw reports are disposable local artifacts and are removed by `pnpm clean`; durable user-visible findings belong in [the release notes](../docs/releases/release-notes.md), without credentials, absolute production paths, generated scripts, or long model transcripts.
